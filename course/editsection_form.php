...
require_once ($CFG->dirroot . '/course/format/unidades/lib.php');
...
class editsection_form extends moodleform
{
    function definition()
    {
        ...
        if ($format_curso == 'unidades')
        {
            if ($seccion > 0)
            {
                ...
                $mform->addElement('textarea', 'observaciones', get_string('observaciones_unidad', 'format_unidades'), array('rows' => "7", 'cols' => "70"));
                $mform->setType('observaciones', PARAM_TEXT);
        ...
        function definition_after_data()
    {
        ...
        if (!empty($CFG->enableavailability))
        {
            ...
            $mform->addElement('date_time_selector', 'availablefrom', get_string('availablefrom', 'condition'), array(
                'optional' => true,
                'defaulttime' => $midnight
            ));
            $mform->addElement('date_time_selector', 'availableuntil', get_string('availableuntil', 'condition'), array(
                'optional' => true,
                'defaulttime' => $midnight
            ));
        ...
        function validation($data, $files)
    {
        ...
        // Conditions: Verify that the user profile field has not been declared more than
        // once
        if (array_key_exists('conditionfieldgroup', $data))
        {
            // Array to store the existing fields
            $arrcurrentfields = array();
            // Error message displayed if any condition is declared more than once. We use
            // lang string because
