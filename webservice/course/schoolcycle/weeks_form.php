...
class semanas_form extends moodleform
{
    private $semanas = 16;
    protected function definition()
    {
        ...
        for($i = 1; $i <= $this->semanas; $i++)
        {
            ...
            $mform->addElement('hidden', "nu_semana_$i");
            $mform->setType("nu_semana_$i", PARAM_INT);

            $fechas = array();
            $fechas[] = $mform->createElement('static', "semana_$i", null, get_string('semana', 'format_unidades') . " $i");
    ...
    public function set_data($datos)
    {
        ...
        $rs = $DB->get_recordset('course_semanas_efectivas', array('nu_carrera' => $datos->nu_carrera, 'nu_periodo' => $datos->id), 'nu_semana');
        foreach ($rs as $semana) 
        {
            ...
            $nu_semana = "nu_semana_$semana->nu_semana";
