...
$seleccionadas = array();
$carreras = $DB->get_records('course_categories');
$role = $DB->get_record('role', array('shortname' => 'coordinador'), '*', MUST_EXIST);

foreach ($carreras as $carrera)
{
    $context = context_coursecat::instance($carrera->id);
    $users = get_role_users($role->id, $context, false, '', 'lastname DESC');

//$handle = fopen($CFG->dataroot . '/carreras' . $carrera->id, 'w+');
//fwrite($handle, 'Usuarios: ' . print_r($users, true) . 'Usuario actual: ' . print_r($USER, true));
//fclose($handle);   
    ...
    foreach ($users as $usuario)
    {
        ...
        {
            $seleccionadas[$carrera->id] = $carrera;
            break;
        }
    }
    
}
//echo print_r($seleccionadas, true);
