...
class settings_navigation extends navigation_node {
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        if($course->format == 'unidades') {
            $modinfo = get_fast_modinfo($course->id);
            $sections = $modinfo->get_section_info_all();
            $seccion = $sections[0];
            $unidad = new Unidad($course, $seccion);
            if ((has_capability('courseformat/unidades:generarpdf', $coursecontext) && $unidad->estado == PLANEACION_APROBADA) || has_capability('courseformat/unidades:superpdf', $coursecontext))
            {
                $reporte = $coursenode->add('Reportes de Gestión del Curso', null, navigation_node::TYPE_CONTAINER);
                $nodo_plan = $reporte->add(get_string('accion_generarpdf', 'format_unidades'), null, navigation_node::TYPE_CONTAINER);
                $nodo_desg = $reporte->add(get_string('accion_desglose_generarpdf', 'format_unidades'), null, navigation_node::TYPE_CONTAINER);
                $url_rubricas = new moodle_url('/lib/itse/reportes/descargar.php', array('id' => $course->id, 'accion' => ACCION_DESCARGA_RUBRICAS));
                $reporte ->add(get_string('accion_rubricas_generarpdf', 'format_unidades'), $url_rubricas, navigation_node::TYPE_SETTING);
                
                $grupos = groups_get_all_groups($course->id);
                foreach ($grupos as $grupo)
                {
                    $nodo_plan->add($grupo->name, Botones_Descarga::boton_planeacion($course->id, $grupo->id), navigation_node::TYPE_SETTING);
                    $nodo_desg->add($grupo->name, Botones_Descarga::boton_desglose($course->id, $grupo->id), navigation_node::TYPE_SETTING);
                }
            }

            if (has_capability('courseformat/cortes:generate', $coursecontext)) //&& $unidad->estado == PLANEACION_APROBADA))
            {
                $cortes = $coursenode->add('Cortes', null, navigation_node::TYPE_CONTAINER);
                for($corte = 1; $corte <= NUMERO_CORTES_SEMESTRE; $corte++)
                {
                    $url = new moodle_url('/course/cortes/genera_corte.php', array('id' => $course->id, 'corte' => $corte, 'sesskey' => sesskey()));
                    if($corte != NUMERO_CORTES_SEMESTRE)
                        $cortes->add("Corte $corte", $url, navigation_node::TYPE_SETTING);
                    else
                        $cortes->add("Corte Final", $url, navigation_node::TYPE_SETTING);
                }
            }
        }
        ...
        if ($reportavailable) {
            ...
            $gradenode = $coursenode->add(get_string('grades'), null, self::TYPE_CONTAINER, null, null, null);
            $gradenode->add(get_string('gradebook', 'format_unidades'), $url, self::TYPE_SETTING, null, 'grades', new pix_icon('i/grades', ''));
            if(has_capability('courseformat/unidades:pdfcalificacion', $coursecontext))
            {
                $grupos = groups_get_all_groups($course->id);
                foreach ($sections as $section)
                {
                    ...
                    {
                        $nodo_unidad = $gradenode->add("Unidad $section->section", null, navigation_node::TYPE_CONTAINER);
                        foreach ($grupos as $grupo)
                        {
                            $url = new moodle_url('/lib/itse/reportes/descargar.php', array('id' => $course->id, 'groupid' => $grupo->id, 'unidad' => $section->section, 'accion' => ACCION_DESCARGA_CALIFICACIONES));
                            $nodo_unidad->add($grupo->name, $url, navigation_node::TYPE_SETTING);
                        }
                    }
                }
            }
    ...
    protected function load_category_settings() {
        ...
        // Administración de ciclo escolar
        if (has_capability('courseformat/periodos:agregar', $this->context)) 
        {
            $url = new moodle_url('/course/schoolcycle/view.php', array('id' => $this->context->instanceid));
            $categorynode->add(get_string('administrarperiodo', 'format_unidades'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/grades', ''));
        }

        // Administración de ciclo escolar
        if (has_capability('courseformat/cortes:view', $this->context)) 
        {
            $cortes = $categorynode->add(get_string('administrarcortes', 'format_unidades'), null, self::TYPE_CONTAINER, null, null, null);
            $url = new moodle_url('/course/cuts/view.php', array('id' => $this->context->instanceid));
            $cortes->add(get_string('fechascortes', 'format_unidades'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/grades', ''));
            $url = new moodle_url('/course/cuts/generate.php', array('id' => $this->context->instanceid));
            $cortes->add(get_string('genera_corte', 'format_unidades'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/cohort', ''));
        }

        // Reporte de Uso de Moodle de Docentes
        if (has_capability('courseformat/semanas:agregar', $this->context)) 
        {
            $url = new moodle_url('/lib/itse/reportes/descarga_est.php', array('id' => $this->context->instanceid, 'accion' => ACCION_DESCARGA_CURSOS_ESTADISTICAS_DOCENTES));
            $categorynode->add(get_string('informe_uso', 'format_unidades'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/cohort', ''));
        }

        // Reporte de las Asistencias por Grupo de la Carrera
        if (has_capability('courseformat/semanas:agregar', $this->context)) 
        {
            $url = new moodle_url('/lib/itse/reportes/descarga_est.php', array('id' => $this->context->instanceid, 'accion' => ACCION_DESCARGA_CURSOS_ESTADISTICAS_ASISTENCIAS_CARRERAS));
            $categorynode->add(get_string('informe_asistencias_carreras', 'format_unidades'), $url, self::TYPE_SETTING, null, null, new pix_icon('i/grades', ''));
        }
