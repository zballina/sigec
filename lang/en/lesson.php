...
/**
 * Strings for component 'lesson', language 'es_mx', branch 'MOODLE_25_STABLE'
 *
 * @package   lesson
...
$string['defaultessayresponse'] = 'Su ensayo será calificado por su maestro.';
...
$string['displayleftif'] = 'Menú de calificación menor a mostrar';
...
$string['essayemailmessage2'] = '<p>Del Ensayo:<blockquote>{$a->question}</blockquote></p><p>Su respuesta:<blockquote><em>{$a->response}</em></blockquote></p><p>Comentarios del evaluador:<blockquote><em>{$a->comment}</em></blockquote></p><p>Usted recibió  {$a->earned} de  {$a->outof}por esta pregunta de ensayo.</p><p>Su calificación para la lección ha sido cambiada a  {$a->newgrade}%.</p>';
...
$string['highscores'] = 'Calificaciones máximas';
$string['hightime'] = 'Tiempo máximo';
$string['importcount'] = 'Importando {$a} preguntas';
$string['importquestions'] = 'Importar preguntas';
$string['importquestions_help'] = '<P>Esta función permite importar preguntas de archivos de texto, subidos mediante un formato. <P>Los formatos permitidos son: <P><B>GIFT</B></P> <ul> <p>GIFT es el formato de importación de preguntas más amigable para Moodle de todos los formatos disponible. Su diseño permite que los profesores pueden escribir las preguntas en un archivo de texto de forma fácil. Permite preguntas de opción múltiple, Falso/Verdadero, Respuesta Corta, Relacionar columnas y numéricas, también permite las preguntas de rellenar huecos insertándolas como _____ . Se pueden mezclar Varios tipos de preguntas en un solo archivo del texto, y este formato también permite líneas de comentarios, los nombres de la pregunta, retroalimentación y los porcentaje-peso (ponderar) calificaciones. Veamos algunos ejemplos:</p> <pre>¿Quién está sepultado en la tumba de Grant?{~Grant ~Jefferson =nadie} Grant está{~sepultado =entumbado ~viviendo} en la tumba de  Grant. Grant está sepultado en la tumba de Grant.{FALSE} ¿Quén está sepultado en la tumba de Grant?{=ninguno =nadie} ¿Cuando nació Ulysses S. Grant?{#1822} </pre> <p align=right><a href="help.php?file=formatgift.html&module=quiz">Más información sobre el formato "GIFT" </a></p> </ul> <P><B>Aiken</B></P> <ul> <p>El formato Aiken es una manera simple de crear preguntas de opción múltiple usando un formato de fácil lectura. Un ejemplo del formato:</p> <pre>¿Qué objetivo tienen los primeros auxilios? A. Salvar la vida, prevenir más lesiones, mantener el buen estado de salud. B. Dar tratamiento médico o sanitario C. prevenir más lesiones D. Ayudar a las victimas que pedir auxilio ANSWER: A </pre> <p align=right><a href="help.php?file=formataiken.html&module=quiz">Más información sobre el formato "Aiken" </a></p> </ul> <P><B>Llenar el hueco</B></P> <UL> <P>Este formato sólo soporta preguntas de opción múltiple. Cada pregunta se separa con un tilde (~), y la respuesta correcta se precede con un signo de igual (=). Un ejemplo: <BLOCKQUOTE>Cuando comenzamos a explorar las partes de nuestro cuerpo nos convertimos en estudiosos de: {=anatomía y fisiología ~reflexología ~la ciencia ~los experimentos}, y en cierto sentido somos estudiantes de la vida. </BLOCKQUOTE> <p align=right><a href="help.php?file=formatmissingword.html&module=quiz">Más información sobre el formato "Llenar el hueco"</a></p> </UL> <P><B>AON</B></P> <UL> <P>Este es el mismo caso de llenar el hueco, excepto que luego de ser importadas, todas las preguntas se convierten en grupos de cuatro preguntas de seleccionar la correcta. </P> <p>Además, las respuestas de opción múltiple son mezcladas aleatoriamente en la importación. <p>Se le llama así en honor a una empresa que impulsó el desarrollo de muchas características para los exámenes. </p> </UL> <P><B>Blackboard</B></P> <UL> <P>Este módulo puede importar preguntas guardadas con la característica de exportar preguntas del programa Blackboard. Se apoya en la capacidad de compilar funciones XML en sus correspondientes PHP.</P> <p align=right><a href="help.php?file=formatblackboard.html&module=quiz">Más información sobre el formato "Blackboard"</a></p> </UL> <P><B>Course Test Manager</B></P> <UL> <P>Este módulo puede importar las preguntas guardadas en una banco de preguntas de Course Test Manager. Tenemos varias formas de acceder al banco de preguntas en formato Access de Microsoft, dependiendo si Moodle está funcionando en un servidor de Windows o de Linux:</P> <p>Desde Windows debemos subir la base de datos como cualquier otro archivo para importar sus datos.</p> <p>Desde Linux, debemos instalar el software ODBC Socket Server, que utiliza XML para transferir datos a Moodle desde el servidor Linux.</p> <p>Por favor, lea todo el archivo de ayuda antes de importar este tipo de formato.</p> <p align=right><a href="help.php?file=formatctm.html&module=quiz">Más información sobre el formato "CTM"</a></p> </UL> <P><B>Personal</B></P> <UL> <P>Si tiene su propio formato que desea importar, puede implementarlo editando mod/quiz/format/custom.php <P>La cantidad de código nuevo necesaria es bastante pequeña - será suficiente con analizar una sola pregunta del texto-. <p align=right><a href="help.php?file=formatcustom.html&module=quiz">Más información sobre el formato "Personal"</a></p> </UL> <P>Se están desarrollando más formatos, incluyendo WebCT, IMS QTI y cualquier otro que los usuarios de Moodle quieran aportar.</p>';
...
$string['lessonnotready'] = 'Esta lección no está lista para tomarse. Por favor, contacte con su {$a}.';
...
* Para aprendizaje auto-dirigido en un tema nuevo
...
$string['noonecheckedthis'] = 'Nadie elegió esta.';
...
$string['nothighscore'] = 'No alcanzó a entrar en la lista {$a} de puntuaciones más altas.';
...
$string['prerequisitelesson'] = 'Lección de pre-requisito';
...
$string['progressbar_help'] = 'Muestra una barra de progreso de finalización de la lección. Por el momento, la barra de progreso tiene más precisión cuando las lecciones son lineales.

Al calcular el porcentaje finalizado (finalizado), las Tablas de ramificación y las páginas de preguntas contestadas correctamente contribuyen al progreso de la lección. Al calcular el número total de páginas de la lección, los conglomerados y las páginas incluídas en los conglomerados sólo cuentan como una página única, excluyéndose las páginas de final de conglomerado y final de tabla de ramificación. El resto de las páginas cuentan para calcular el total de páginas de la lección.

Nota: los estilos por defecto de la barra de finalización no impresionan ;) Todos los estilos (e.g.: colores, imágenes de fondo, etc.) de la barra de progreso pueden modificarse en mod/lesson/styles.php.';
...
$string['youmadehighscore'] = 'Ha salido en la lista de las {$a} puntuaciones más altas';
