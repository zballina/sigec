...
/**
 * Strings for component 'auth_ldap', language 'es_mx', branch 'MOODLE_25_STABLE'
 *
 * @package   auth_ldap
...
$string['auth_ldapdescription'] = 'Este método proporciona autenticación contra un servidor LDAP externo. Si el nombredeusuario y contraseña facilitados son válidos, Moodle crea una nueva entrada para el usuario en su base de datos. Este módulo puede leer atributos de usuario desde LDAP y prerrellenar los campos requeridos en Moodle. Para los ingresos sólo se comprueba el usuario y la contraseña.';
...
$string['auth_ldap_graceattr_desc'] = 'Opcional: Anula el atributo gracelogin (ingreso libre)';
$string['auth_ldap_gracelogin_key'] = 'Atributo de ingreso libre';
$string['auth_ldap_gracelogins_desc'] = 'Activar el soporte de entrada libre LDAP. Una ves que la contraseña ha caducado, el usuario puede ingresar hasta que la cuenta de acceso libre llega a cero. Si se activa esta opción se mostrará un mensaje de ingreso libre en el caso de que la contraseña haya caducado.';
...
$string['auth_ldap_user_exists'] = 'Ya existe ese nombredeusuario LDAP';
$string['auth_ldap_user_settings'] = 'Ajustes de búsqueda de usuario';
$string['auth_ldap_user_type'] = 'Seleccione cómo se almacenarán los usuarios en LDAP. Este ajuste también especifica cómo funcionarán la caducidad del acceso, los ingresos libres y la creación de usuarios.';
...
$string['auth_ntlmsso_enabled'] = 'Seleccione Sí para intentar  (entrada única) Single Sign On con el dominio NTLM. <strong>Nota:</strong> esto requiere un ajuste adicional en el servidor web para trabajar; vea <a href="http://docs.moodle.org/en/NTLM_authentication">http://docs.moodle.org/en/NTLM_authentication</a>';
...
$string['auth_ntlmsso_maybeinvalidformat'] = 'No se pudo extraer el nombredeusuario del encabezado de REMOTE_USER (USUARIO_REMOTO). ¿Está correcto el formato configurado?';
$string['auth_ntlmsso_missing_username'] = 'Usted necesita especificar al menos %username% en el formato de nombredeusuario remoto';
$string['auth_ntlmsso_remoteuserformat'] = 'Si Usted ha elegido \'NTLM\' en \'Tipo de Autenticación\', Usted puede especificar aquí el formato de nombredeusuario remoto. Si lo deja vacío, se usará el formato de DOMAINusername (nombredeusuarioDELDOMINIO). Usted puede usar el remplazable opcional <b>%domain%</b> para especificar donde aparece el nombre del dominio, y el remplazable obligatorio <b>%username%</b> para especificar donde aparece el nombredeusuario. <br /><br />Algunos de los formatos ampliamente usados son <tt>%domain%%username%</tt> (MS Windows por defecto), <tt>%domain%/%username%</tt>, <tt>%domain%+%username%</tt> y simplemente <tt>%username%</tt> (si no hubiera la parte del dominio).';
$string['auth_ntlmsso_remoteuserformat_key'] = 'Formato de nombredeusuario remoto';
...
$string['needbcmath'] = 'Necesita la extensión BCMath para usar ingresos de gracia con Active Directory';
...
$string['pagedresultsnotsupp'] = 'LDAP envió resultados no soportados (sea que su versión PHP carezca de soporte o que Usted haya configurado Moodle para usar la versión 2 del protocolo LDAP)';
...
$string['updatepasserrorexpiregrace'] = 'Error en user_update_password{} al modificar el periodo de caducidad y/o los ingresos libres.  Código de error: {$a->errno}. Cadena de texto del error: {$a->errstring}';
