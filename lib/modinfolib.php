...
class cm_info implements IteratorAggregate {
    ...
    public function get_formatted_content($options = array()) {
        ...
        if (empty($this->content)) {
            return $this->get_extra_info_mod();
        }
        ...
        $info = $this->get_extra_info_mod();
        $info .= format_text($this->content, FORMAT_HTML, $options);
        return $info;
    }
    ...
    public function get_extra_info_mod()
    {
        global $DB, $CFG;
        ...
        {
            if($this->modname == 'practicalab' || $this->modname == 'sesionclase' || $this->modname == 'sesiontarea')
            {
                $i = 0;
                $sesion = new SesionClase($this->instance, $this->modname, $this->id);
                $programacion = $sesion->programacion(0);
                foreach ($programacion as $prog_grupo)
                {
                    if($prog_grupo->fprogramada > 0)
                    {
                        $fprogramada = $prog_grupo->fprogramada;
                        $tiempo = $prog_grupo->hprogramada > 0 ? format_time($prog_grupo->hprogramada): 'Sin tiempo';
//                        $d = userdate($fprogramada, "%d/%m/%y %h:%m");
                        $d = userdate($fprogramada);
                        $nombre = groups_get_group_name($prog_grupo->groupid);
                        $fprog .= " $nombre: $d ($tiempo) " . $sesion->string_estado($prog_grupo->groupid, true) . '<br>';
                        $i++;
                    } 
                }
                if($i == 0)
                    $fprog = ' (Actividad no programada)<br>';
            }
            else if($this->modname == 'assign')
            {
                $fprogramada = $DB->get_field($this->modname, 'allowsubmissionsfromdate', array('id' => $this->instance), MUST_EXIST);
                if($fprogramada != 0)
//                    $fprog = userdate($fprogramada, "%d/%m/%y") . '<br>';
                    $fprog = userdate($fprogramada) . '<br>';
            }
            else if($this->modname == 'quiz')
            {
                $fprogramada = $DB->get_field($this->modname, 'timeopen', array('id' => $this->instance), MUST_EXIST);
                if($fprogramada != 0)
//                    $fprog = userdate($fprogramada, "%d/%m/%y") . '<br>';
                    $fprog = userdate($fprogramada) . '<br>';
            }
