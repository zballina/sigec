 ...
 * @return bool
 */
function sesiontareasubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('sesiontarea_submission', array('id'=>$options->itemid))) {
    ...
    if ($sesiontareament->get_instance()->id != $submission->sesiontareament) {
        throw new comment_exception('invalidcontext');
    }
    if (!has_capability('mod/sesiontarea:grade', $context)) {
        if (!has_capability('mod/sesiontarea:submit', $context)) {
            throw new comment_exception('nopermissiontocomment');
        } else if ($sesiontareament->get_instance()->teamsubmission) {
            $group = $sesiontareament->get_submission_group($USER->id);
            $groupid = 0;
            if ($group) {
                $groupid = $group->id;
            }
 ...
 * @return array
 */
function sesiontareasubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if ($sesiontareament->get_instance()->teamsubmission &&
        !$sesiontareament->can_view_group_submission($submission->groupid)) {
        return array('post' => false, 'view' => false);
    }

    if (!$sesiontareament->get_instance()->teamsubmission &&
        !$sesiontareament->can_view_submission($submission->userid)) {
        return array('post' => false, 'view' => false);
    }
 ...
 */
function sesiontareasubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($sesiontareament->is_blind_marking() && !empty($comments)) {
        // Blind marking is being used, may need to map unique anonymous ids to the comments.
        $usermappings = array();
        $hiddenuserstr = trim(get_string('hiddenuser', 'sesiontarea'));
        ...
        foreach ($comments as $comment) {
            ...
            if (empty($usermappings[$comment->userid])) {
                ...
                $anonid = $sesiontareament->get_uniqueid_for_user($comment->userid);
                ...
                $usermappings[$comment->userid]->avatar = $sesiontareament->get_renderer()->user_picture($commenter,
 ...
 */
function sesiontareasubmission_comments_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_comments_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('sesiontarea_submission', array('id' => $submissionid));
