 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_sesiontarea_grading_batch_operations_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        // Visible elements.
        $options = array();
        $options['lock'] = get_string('locksubmissions', 'sesiontarea');
        $options['unlock'] = get_string('unlocksubmissions', 'sesiontarea');
        if ($instance['submissiondrafts']) {
            $options['reverttodraft'] = get_string('reverttodraft', 'sesiontarea');
        }
        if ($instance['duedate']) {
            $options['grantextension'] = get_string('grantextension', 'sesiontarea');
        }
        if ($instance['attemptreopenmethod'] == SESIONTAREA_ATTEMPT_REOPEN_METHOD_MANUAL) {
            $options['addattempt'] = get_string('addattempt', 'sesiontarea');
        }
        ...
        $objs = array();
        $objs[] =& $mform->createElement('select', 'operation', get_string('chooseoperation', 'sesiontarea'), $options);
        $objs[] =& $mform->createElement('submit', 'submit', get_string('go'));
        $batchdescription = get_string('batchoperationsdescription', 'sesiontarea');
