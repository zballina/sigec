 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sesiontareafeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $scaleoptions = null;
        if ($sesiontareament->get_instance()->grade < 0) {
            $scale = $DB->get_record('scale', array('id'=>-($sesiontareament->get_instance()->grade)));
            if ($scale) {
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'sesiontareafeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($sesiontareament->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'sesiontarea') . $sesiontareament->get_uniqueid_for_user($user->id);
            }

            $usergrade = $sesiontareament->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $grade) {
                ...
                $skip = true;
            } else if ($sesiontareament->grading_disabled($user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($sesiontareament->get_instance()->grade > -1) &&
                      (($grade < 0) || ($grade > $sesiontareament->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }
            ...
            if (!$skip) {
                $update = true;
                $updates[] = get_string('gradeupdate', 'sesiontareafeedback_offline',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'sesiontareafeedback_offline',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'sesiontareafeedback_offline'));
        }
