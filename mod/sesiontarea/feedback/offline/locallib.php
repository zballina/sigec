 ...
 *
 *
 * @package   sesiontareafeedback_offline
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sesiontarea_feedback_offline extends sesiontarea_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'sesiontareafeedback_offline');
    }
    ...
    /**
     * Loop through uploaded grades and update the grades for this sesiontareament
     *
    ...
    public function process_import_grades($draftid, $importid, $ignoremodified) {
        ...
        $gradeimporter = new sesiontareafeedback_offline_grade_importer($importid, $this->sesiontareament);
        ...
        if (!$gradeimporter->init()) {
            $thisurl = new moodle_url('/mod/sesiontarea/view.php', array('action'=>'viewpluginpage',
                                                                     'pluginsubtype'=>'sesiontareafeedback',
                                                                     'plugin'=>'offline',
                                                                     'pluginaction'=>'uploadgrades',
                                                                     'id'=>$sesiontareament->get_course_module()->id));
            print_error('invalidgradeimport', 'sesiontareafeedback_offline', $thisurl);
            return;
        }
        // Does this sesiontareament use a scale?
        $scaleoptions = null;
        if ($this->sesiontareament->get_instance()->grade < 0) {
            $scale = $DB->get_record('scale', array('id'=>-($this->sesiontareament->get_instance()->grade)));
            if ($scale) {
                $scaleoptions = explode(',', $scale->scale);
            }
        }
        // We may need to upgrade the gradebook comments after this update.
        $adminconfig = $this->sesiontareament->get_admin_config();
        ...
        while ($record = $gradeimporter->next()) {
            ...
            $usergrade = $this->sesiontareament->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $record->grade) {
                ...
                $skip = true;
            } else if ($this->sesiontareament->grading_disabled($record->user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($this->sesiontareament->get_instance()->grade > -1) &&
                      (($record->grade < 0) || ($record->grade > $this->sesiontareament->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }

            if (!$skip) {
                $grade = $this->sesiontareament->get_user_grade($record->user->id, true);

                $grade->grade = $record->grade;
                $grade->grader = $USER->id;
                if ($this->sesiontareament->update_grade($grade)) {
                    $this->sesiontareament->notify_grade_modified($grade);
                    $this->sesiontareament->add_to_log('grade submission', $this->sesiontareament->format_grade_for_log($grade));
                    $updatecount += 1;
                }
            }
                ...
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        ...
                        if ($plugin->set_editor_text($field, $newvalue, $grade->id)) {
                            $logdesc = get_string('feedbackupdate', 'sesiontareafeedback_offline',
                                                  array('field'=>$description,
                                                        'student'=>$userdesc,
                                                        'text'=>$newvalue));

                            $this->sesiontareament->add_to_log('save grading feedback', $logdesc);
                        }
                        ...
                        if (($plugin->get_subtype() . '_' . $plugin->get_type()) == $gradebookplugin) {
                            ...
                            $this->sesiontareament->update_grade($grade);
                        }
                    }
                }
            }
        ...
        $o .= $renderer->render(new sesiontarea_header($this->sesiontareament->get_instance(),
                                                  $this->sesiontareament->get_context(),
                                                  false,
                                                  $this->sesiontareament->get_course_module()->id,
                                                  get_string('importgrades', 'sesiontareafeedback_offline')));
        $o .= $renderer->box(get_string('updatedgrades', 'sesiontareafeedback_offline', $updatecount));
    ...
    public function upload_grades() {
        ...
        require_capability('mod/sesiontarea:grade', $this->sesiontareament->get_context());
        require_once($CFG->dirroot . '/mod/sesiontarea/feedback/offline/uploadgradesform.php');
        require_once($CFG->dirroot . '/mod/sesiontarea/feedback/offline/importgradesform.php');
        require_once($CFG->dirroot . '/mod/sesiontarea/feedback/offline/importgradeslib.php');
        require_once($CFG->libdir . '/csvlib.class.php');

        $mform = new sesiontareafeedback_offline_upload_grades_form(null,
        ...
        if ($mform->is_cancelled()) {
            ...
            $importid = csv_import_reader::get_new_iid('sesiontareafeedback_offline');
            $gradeimporter = new sesiontareafeedback_offline_grade_importer($importid, $this->sesiontareament);
            ...
            $mform = new sesiontareafeedback_offline_import_grades_form(null, array('sesiontareament'=>$this->sesiontareament,
                                                                       'csvdata'=>$csvdata,
            ...
            $o .= $renderer->render(new sesiontarea_header($this->sesiontareament->get_instance(),
                                                            ...
                                                            false,
                                                            $this->sesiontareament->get_course_module()->id,
                                                            get_string('confirmimport', 'sesiontareafeedback_offline')));
            $o .= $renderer->render(new sesiontarea_form('confirmimport', $mform));
            $o .= $renderer->render_footer();
        } else if ($confirm) {
            ...
            $o .= $renderer->render(new sesiontarea_header($this->sesiontareament->get_instance(),
                                                            ...
                                                            false,
                                                            $this->sesiontareament->get_course_module()->id,
                                                            get_string('uploadgrades', 'sesiontareafeedback_offline')));
            $o .= $renderer->render(new sesiontarea_form('batchuploadfiles', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    public function download_grades() {
        ...
        $groupmode = groups_get_activity_groupmode($this->sesiontareament->get_course_module());
        // All users.
        $groupid = 0;
        $groupname = '';
        if ($groupmode) {
            $groupid = groups_get_activity_group($this->sesiontareament->get_course_module(), true);
            $groupname = groups_get_group_name($groupid) . '-';
        }
        $filename = clean_filename(get_string('offlinegradingworksheet', 'sesiontareafeedback_offline') . '-' .
                                   $this->sesiontareament->get_course()->shortname . '-' .
                                   $this->sesiontareament->get_instance()->name . '-' .
                                   $groupname .
                                   $this->sesiontareament->get_course_module()->id);

        $table = new sesiontarea_grading_table($this->sesiontareament, 0, '', 0, false, $filename);
    ...
    public function get_grading_actions() {
        return array('uploadgrades'=>get_string('uploadgrades', 'sesiontareafeedback_offline'),
                    'downloadgrades'=>get_string('downloadgrades', 'sesiontareafeedback_offline'));
    }
    ...
    public function is_enabled() {
        $gradingmanager = get_grading_manager($this->sesiontareament->get_context(), 'mod_sesiontarea', 'submissions');
