 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sesiontareafeedback_offline_grade_importer {
    ...
    /** @var sesiontareament $sesiontareament - the sesiontareament class */
    private $sesiontareament;
     ...
     * @param sesiontarea $sesiontareament The current sesiontareament
     */
    public function __construct($importid, sesiontarea $sesiontareament) {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'sesiontareafeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'sesiontarea');
        $strmodified = get_string('lastmodifiedgrade', 'sesiontarea');
        ...
        $this->validusers = $this->sesiontareament->list_participants($groupid, false);
        return true;
    }
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'sesiontarea')));
            if ($userid = $this->sesiontareament->get_user_id_for_uniqueid($id)) {
