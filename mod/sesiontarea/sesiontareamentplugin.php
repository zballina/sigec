...
/**
 * This file contains the functions for sesiontarea_plugin abstract class
 *
 *
...
/**
 * Abstract class for sesiontarea_plugin (submission/feedback).
 *
 * @package   mod_sesiontarea
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class sesiontarea_plugin {

    /** @var sesiontarea $sesiontareament the sesiontareament record that contains the global
     *              settings for this sesiontarea instance
     */
    protected $sesiontareament;
    /** @var string $type sesiontareament plugin type */
    private $type = '';
     ...
     *
     * @param sesiontarea $sesiontareament
     * @param string $type
     */
    public final function __construct(sesiontarea $sesiontareament, $type) {
     ...
     * form for display in the settings page for the sesiontareament.
    ...
    /**
     * The sesiontareament subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'sesiontareasubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - sesiontarea_submission or sesiontarea_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this sesiontareament.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/sesiontarea/' . substr($this->get_subtype(), strlen('sesiontarea')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('sesiontareament'=>$this->sesiontareament->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('sesiontarea_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('sesiontarea_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->sesiontareament = $this->sesiontareament->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('sesiontarea_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            if (!$this->sesiontareament->has_instance()) {
                return false;
            }
            $sesiontareament = $this->sesiontareament->get_instance();
            if ($sesiontareament) {
                $dbparams = array('sesiontareament'=>$sesiontareament->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('sesiontarea_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('sesiontareament'=>$this->sesiontareament->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('sesiontarea_plugin_config', $dbparams);
     ...
     * and version.
     *
     * @param string $type The old sesiontareament subtype
     * @param int $version The old sesiontareament version
    ...
    /**
     * Upgrade the settings from the old sesiontareament to the new one
     *
     * @param context $oldcontext The context for the old sesiontareament module
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldsesiontareament, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_sesiontarea', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade sesiontarea_submission or sesiontarea_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_sesiontarea', $params);
        return false;
    }
    ...
    /**
     * The sesiontareament has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this sesiontareament plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade sesiontarea_submission or sesiontarea_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'sesiontareasubmission') {
            if ($itemid) {
                $record = $DB->get_record('sesiontarea_submission', array('id'=>$itemid), 'userid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
                if (!$this->sesiontareament->can_view_submission($record->userid)) {
                    return null;
                }
            }
        } else {
        ...
        if (!($storedfile = $fs->get_file($this->sesiontareament->get_context()->id,
    ...
    /**
     * This allows a plugin to render a page in the context of the sesiontareament
     *
     * If the plugin creates a link to the sesiontareament view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=sesiontareafeedback|sesiontareasubmission
