...
/**
 * Extends table_sql to provide a table of sesiontareament submissions
 *
 * @package   mod_sesiontarea
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sesiontarea_grading_table extends table_sql implements renderable {
    /** @var sesiontarea $sesiontareament */
    private $sesiontareament = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the sesiontareament class that is displaying this table
     *
     * @param sesiontarea $sesiontareament The sesiontareament class
     ...
     */
    public function __construct(sesiontarea $sesiontareament,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        global $CFG, $PAGE, $DB;
        parent::__construct('mod_sesiontarea_grading');
        $this->sesiontareament = $sesiontareament;
        ...
        $urlparams = array('action'=>'grading', 'id'=>$sesiontareament->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/sesiontarea/view.php', $urlparams);
        ...
        $users = array_keys( $sesiontareament->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['sesiontareamentid1'] = (int)$this->sesiontareament->get_instance()->id;
        $params['sesiontareamentid2'] = (int)$this->sesiontareament->get_instance()->id;
        $params['sesiontareamentid3'] = (int)$this->sesiontareament->get_instance()->id;
        $params['sesiontareamentid4'] = (int)$this->sesiontareament->get_instance()->id;
        $params['sesiontareamentid5'] = (int)$this->sesiontareament->get_instance()->id;

        $extrauserfields = get_extra_user_fields($this->sesiontareament->get_context());
        ...
        $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                 ...
                                 WHERE mxs.sesiontareament = :sesiontareamentid4 GROUP BY mxs.userid';
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {sesiontarea_grades} mxg
                            WHERE mxg.sesiontareament = :sesiontareamentid5 GROUP BY mxg.userid';
        $from = '{user} u
                         ...
                         LEFT JOIN {sesiontarea_submission} s ON
                            u.id = s.userid AND
                            s.sesiontareament = :sesiontareamentid1 AND
                            s.attemptnumber = smx.maxattempt
                         LEFT JOIN {sesiontarea_grades} g ON
                            u.id = g.userid AND
                            g.sesiontareament = :sesiontareamentid2 AND
                            g.attemptnumber = gmx.maxattempt
                         LEFT JOIN {sesiontarea_user_flags} uf ON u.id = uf.userid AND uf.sesiontareament = :sesiontareamentid3';
        ...
        if ($this->sesiontareament->is_any_submission_plugin_enabled()) {
            if ($filter == SESIONTAREA_FILTER_SUBMITTED) {
                $where .= ' AND s.timecreated > 0 ';
            }
            if ($filter == SESIONTAREA_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified > g.timemodified OR g.timemodified IS NULL))';
                $params['submitted'] = SESIONTAREA_SUBMISSION_STATUS_SUBMITTED;
            }
            if (strpos($filter, SESIONTAREA_FILTER_SINGLE_USER) === 0) {
        ...
        // User picture.
        if (!$this->sesiontareament->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'sesiontarea');
            }
        ...
        // Submission status.
        if ($sesiontareament->is_any_submission_plugin_enabled()) {
            $columns[] = 'status';
            $headers[] = get_string('status');
        }

        // Team submission columns.
        if ($sesiontareament->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'sesiontarea');

            $columns[] = 'teamstatus';
            $headers[] = get_string('teamsubmissionstatus', 'sesiontarea');
        }
        ...
        if ($this->is_downloading()) {
            if ($this->sesiontareament->get_instance()->grade >= 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'sesiontarea');
            } else {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'sesiontarea');
            }
        }
        if (!$this->is_downloading()) {
        ...
        if ($sesiontareament->is_any_submission_plugin_enabled()) {
            ...
            $headers[] = get_string('lastmodifiedsubmission', 'sesiontarea');

            foreach ($this->sesiontareament->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    if ($plugin->is_visible() && $plugin->is_enabled()) {
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'sesiontarea');

        // Feedback plugins.
        foreach ($this->sesiontareament->get_feedback_plugins() as $plugin) {
            if ($this->is_downloading()) {
                if ($plugin->is_visible() && $plugin->is_enabled()) {
                    foreach ($plugin->get_editor_fields() as $field => $description) {
                        $index = 'plugin' . count($this->plugincache);
        ...
        $this->gradinginfo = grade_get_grades($this->sesiontareament->get_course()->id,
                                              'mod',
                                              'sesiontarea',
                                              $this->sesiontareament->get_instance()->id,
                                              $users);
        $this->hasgrantextension = has_capability('mod/sesiontarea:grantextension',
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this sesiontareament.
     *
     * @return string
     */
    public function col_recordid(stdClass $row) {
        return get_string('hiddenuser', 'sesiontarea') .
               $this->sesiontareament->get_uniqueid_for_user($row->userid);
    }
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id'=>-($this->sesiontareament->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->sesiontareament->get_instance()->grade >= 0) {
                ...
                // This is a custom scale.
                $scale = $this->sesiontareament->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->sesiontareament->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        return get_string('defaultteam', 'sesiontarea');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->sesiontareament->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->sesiontareament->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_teamstatus(stdClass $row) {
        ...
        return get_string('submissionstatus_' . $status, 'sesiontarea');
    }
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            $courseid = $this->sesiontareament->get_course()->id;
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'sesiontarea', fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this sesiontareament.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        return format_float($this->sesiontareament->get_instance()->grade, 2);
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        $separator = '';
        $grade = '';
        $gradingdisabled = $this->sesiontareament->grading_disabled($row->id);

        if (!$this->is_downloading()) {
            $name = fullname($row);
            if ($this->sesiontareament->is_blind_marking()) {
                $name = get_string('hiddenuser', 'sesiontarea') .
                        $this->sesiontareament->get_uniqueid_for_user($row->userid);
            }
            $icon = $this->output->pix_icon('gradefeedback',
                                            get_string('gradeuser', 'sesiontarea', $name),
                                            'mod_sesiontarea');
    ...
    public function col_status(stdClass $row) {
        ...
        $instance = $this->sesiontareament->get_instance();

        if ($this->sesiontareament->is_any_submission_plugin_enabled()) {

            $o .= $this->output->container(get_string('submissionstatus_' . $row->status, 'sesiontarea'),
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'sesiontarea');
                $o .= $this->output->container($lockedstr, 'lockedsubmission');
            }
            if ($row->grade !== null && $row->grade >= 0) {
                $o .= $this->output->container(get_string('graded', 'sesiontarea'), 'submissiongraded');
            }
            if (!$row->timesubmitted) {
                $now = time();
                ...
                if ($due && ($now > $due)) {
                    $overduestr = get_string('overdue', 'sesiontarea', format_time($now - $due));
                    $o .= $this->output->container($overduestr, 'overduesubmission');
                }
            }
            if ($row->extensionduedate) {
                $userdate = userdate($row->extensionduedate);
                $extensionstr = get_string('userextensiondate', 'sesiontarea', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'sesiontarea');
        }
        $actions[$url->out(false)] = $description;

        // Hide for offline sesiontareaments.
        if ($this->sesiontareament->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == SESIONTAREA_SUBMISSION_STATUS_DRAFT ||
                    !$this->sesiontareament->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'sesiontarea');
                    $actions[$url->out(false)] = $description;
                } else {
                    $urlparams = array('id' => $this->sesiontareament->get_course_module()->id,
                                       'userid'=>$row->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/sesiontarea/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'sesiontarea');
                    $actions[$url->out(false)] = $description;
                }
            }

            if (($this->sesiontareament->get_instance()->duedate ||
                   $this->sesiontareament->get_instance()->cutoffdate) &&
                ...
                $urlparams = array('id' => $this->sesiontareament->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/sesiontarea/view.php', $urlparams);
                $description = get_string('grantextension', 'sesiontarea');
                $actions[$url->out(false)] = $description;
            }
        }
        if ($row->status == SESIONTAREA_SUBMISSION_STATUS_SUBMITTED &&
                $this->sesiontareament->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->sesiontareament->get_course_module()->id,
                               'userid'=>$row->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/sesiontarea/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'sesiontarea');
            $actions[$url->out(false)] = $description;
        }

        $ismanual = $this->sesiontareament->get_instance()->attemptreopenmethod == SESIONTAREA_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != SESIONTAREA_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->sesiontareament->get_instance()->maxattempts == SESIONTAREA_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->sesiontareament->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->sesiontareament->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/sesiontarea/view.php', $urlparams);
            $description = get_string('addattempt', 'sesiontarea');
            $actions[$url->out(false)] = $description;
        }
     ...
     *
     * @param sesiontarea_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(sesiontarea_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('sesiontarea')), 'sesiontarea');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'sesiontareasubmission') {
                if ($this->sesiontareament->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == SESIONTAREA_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->sesiontareament->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->sesiontareament = $this->sesiontareament->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->sesiontareament = $this->sesiontareament->get_instance()->id;
     ...
     *
     * @return string the sesiontareament name
     */
    public function get_sesiontareament_name() {
        return $this->sesiontareament->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->sesiontareament->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->sesiontareament->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->sesiontareament->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this sesiontareament accept submissions
     */
    public function submissions_enabled() {
        return $this->sesiontareament->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->sesiontareament->get_course_context();
