...
/**
 * This file contains the classes for the admin settings of the sesiontarea module.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sesiontarea_admin_page_manage_sesiontarea_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/sesiontarea/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'sesiontarea'),
                            $url);
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sesiontarea_plugin_manager {
    ...
    /**
     * Constructor for this sesiontareament plugin manager
     * @param string $subtype - either sesiontareasubmission or sesiontareafeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/sesiontarea/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'delete', 'settings'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'sesiontarea'),
                get_string('version'), get_string('hideshow', 'sesiontarea'),
        ...
        $shortsubtype = substr($this->subtype, strlen('sesiontarea'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/sesiontarea/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'sesiontarea'));
    }
    ...
    public function delete_plugin($plugin) {
        ...
        if ($confirm) {
            ...
            $DB->delete_records('sesiontarea_plugin_config', array('plugin'=>$plugin, 'subtype'=>$this->subtype));

            // Then the tables themselves.
            $shortsubtype = substr($this->subtype, strlen('sesiontarea'));
            $installxml = $CFG->dirroot . '/mod/sesiontarea/' . $shortsubtype . '/' . $plugin . '/db/install.xml';
    ...
    private function view_plugin_deleted($plugin) {
        ...
        echo $OUTPUT->heading(get_string('deletingplugin', 'sesiontarea', $pluginname));
        echo $this->error;
        $messageparams = array('name'=>$pluginname,
                               'directory'=>('/mod/sesiontarea/' . $this->subtype . '/'.$plugin));
    ...
    private function view_confirm_delete($plugin) {
        ...
        $this->view_header();
        $pluginname = get_string('pluginname', $this->subtype . '_' . $plugin);
        echo $OUTPUT->heading(get_string('deletepluginareyousure', 'sesiontarea', $pluginname));
        $urlparams = array('action' => 'delete', 'plugin'=>$plugin, 'confirm' => 1);
        $confirmurl = new moodle_url($this->pageurl, $urlparams);
        echo $OUTPUT->confirm(get_string('deletepluginareyousuremessage', 'sesiontarea', $pluginname),
                $confirmurl,
                $this->pageurl);
        $this->view_footer();
    }
     ...
     * @return None
     */
    public static function add_admin_sesiontarea_plugin_settings($subtype,
        ...
        foreach ($pluginsbyname as $pluginname => $plugin) {
            ...
            if ($admin->fulltree) {
                ...
                include($CFG->dirroot . "/mod/sesiontarea/$shortsubtype/$plugin/settings.php");
            }
