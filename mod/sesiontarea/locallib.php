...
/**
 * This file contains the definition for the class sesiontareament
 *
...
define('SESIONTAREA_SUBMISSION_STATUS_REOPENED', 'reopened');
define('SESIONTAREA_SUBMISSION_STATUS_DRAFT', 'draft');
define('SESIONTAREA_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('SESIONTAREA_FILTER_SUBMITTED', 'submitted');
define('SESIONTAREA_FILTER_SINGLE_USER', 'singleuser');
define('SESIONTAREA_FILTER_REQUIRE_GRADING', 'require_grading');

// Reopen attempt methods.
define('SESIONTAREA_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('SESIONTAREA_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('SESIONTAREA_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('SESIONTAREA_UNLIMITED_ATTEMPTS', -1);

require_once($CFG->libdir . '/accesslib.php');
require_once($CFG->libdir . '/formslib.php');
require_once($CFG->dirroot . '/repository/lib.php');
require_once($CFG->dirroot . '/mod/sesiontarea/mod_form.php');
require_once($CFG->libdir . '/gradelib.php');
require_once($CFG->dirroot . '/grade/grading/lib.php');
require_once($CFG->dirroot . '/mod/sesiontarea/feedbackplugin.php');
require_once($CFG->dirroot . '/mod/sesiontarea/submissionplugin.php');
require_once($CFG->dirroot . '/mod/sesiontarea/renderable.php');
require_once($CFG->dirroot . '/mod/sesiontarea/gradingtable.php');
require_once($CFG->libdir . '/eventslib.php');
require_once($CFG->libdir . '/portfolio/caller.php');
require_once($CFG->dirroot . '/mod/assign/locallib.php');

/**
 * Standard base class for mod_sesiontarea (sesiontareament types).
 *
 * @package   mod_sesiontarea
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class sesiontarea {
    ...
    /** @var stdClass the sesiontareament record that contains the global settings for this sesiontarea instance */
    private $instance;

    /** @var context the context of the course module for this sesiontarea instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this sesiontarea instance belongs to */
    private $course;

    /** @var stdClass the admin config for all sesiontarea instances  */
    private $adminconfig;

    /** @var sesiontarea_renderer the custom renderer for this module */
    private $output;

    /** @var stdClass the course module for this sesiontarea instance */
    ...
    /**
     * Constructor for the base sesiontarea class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('sesiontareasubmission');
        $this->feedbackplugins = $this->load_plugins('sesiontareafeedback');
    }
    ...
    /**
     * Based on the current sesiontareament settings should we display the intro.
     *
    ...
    /**
     * Does an sesiontareament have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype sesiontareasubmission | sesiontareafeedback
     * @param string $type
     * @return mixed sesiontarea_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('sesiontarea'));
     ...
     * @return mixed sesiontarea_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('sesiontareafeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed sesiontarea_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('sesiontareasubmission', $type);
    }
    ...
    protected function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('sesiontarea'));
                $pluginclass = 'sesiontarea_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof sesiontarea_plugin) {
    ...
    /**
     * Display the sesiontareament, used by view.php
     *
     * The sesiontareament is displayed differently depending on your role,
     * the settings for the sesiontareament and the status of the sesiontareament.
     *
    ...
    public function view($action='') {
        ...
        if ($action == 'savesubmission') {
            ...
            if (optional_param('saveandshownext', null, PARAM_RAW)) {
                ...
                $action = 'grade';
                if ($this->process_save_grade($mform)) {
                    $message = get_string('gradingchangessaved', 'sesiontarea');
        ...
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/sesiontarea/view.php', $nextpageparams);
        ...
        } else if ($action == 'viewpluginsesiontareafeedback') {
            $o .= $this->view_plugin_content('sesiontareafeedback');
        } else if ($action == 'viewpluginsesiontareasubmission') {
            $o .= $this->view_plugin_content('sesiontareasubmission');
     ...
     *             when upgrading an old sesiontareament to a new one (the plugins get called manually)
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->aprendizaje = $formdata->aprendizaje;
        $update->aprendizajeformat = $formdata->aprendizajeformat;
        $update->ensenanza = $formdata->ensenanza;
        $update->ensenanzaformat = $formdata->ensenanzaformat;
        $update->portafolio = $formdata->portafolio;
        $update->portafolioformat = $formdata->portafolioformat;
        ...
        $update->attemptreopenmethod = SESIONTAREA_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('sesiontarea', $update);
        $this->instance = $DB->get_record('sesiontarea', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        $DB->update_record('sesiontarea', $update);

        return $returnid;
    }

    /**
     * Delete all grades from the gradebook for this sesiontareament.
     *
     * @return bool
     */
    protected function delete_grades() {
        ...
        $result = grade_update('mod/sesiontarea',
                               $this->get_course()->id,
                               'mod',
                               'sesiontarea',
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this sesiontareament.
        $fs = get_file_storage();
        ...
        $DB->delete_records('sesiontarea_submission', array('sesiontareament'=>$this->get_instance()->id));
        $DB->delete_records('sesiontarea_grades', array('sesiontareament'=>$this->get_instance()->id));
        $DB->delete_records('sesiontarea_plugin_config', array('sesiontareament'=>$this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('sesiontarea', array('id'=>$this->get_instance()->id));
     ...
     * sesiontareament submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'sesiontarea');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_sesiontarea_submissions)) {
            // Delete files associated with this sesiontareament.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'sesiontarea'),
            ...
            $sesiontareassql = 'SELECT a.id
                             FROM {sesiontarea} a
                           WHERE a.course=:course';
            $params = array('course'=>$data->courseid);

            $DB->delete_records_select('sesiontarea_submission', "sesiontareament IN ($sesiontareassql)", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('sesiontarea_grades', "sesiontareament IN ($sesiontareassql)", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/sesiontarea/lib.php');
                sesiontarea_reset_gradebook($data->courseid);
            }
        }
        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            shift_course_mod_dates('sesiontarea',
     ...
     *
     * @param sesiontarea_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(sesiontarea_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this sesiontareament.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this sesiontareament
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/sesiontarea/lib.php');
        $sesiontarea = clone $this->get_instance();
        $sesiontarea->cmidnumber = $coursemoduleid;
        $param = null;
        if ($reset) {
            $param = 'reset';
        }

        return sesiontarea_grade_item_update($sesiontarea, $param);
    }
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('sesiontarea');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this sesiontareament.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            ...
            $params = array('modulename'=>'sesiontarea', 'instance'=>$instance->id);
            ...
            if ($event->id) {
                ...
                $event->description = format_module_intro('sesiontarea', $instance, $coursemoduleid);
            ...
            $DB->delete_records('event', array('modulename'=>'sesiontarea', 'instance'=>$instance->id));
        }
    }
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('sesiontarea', $update);
        $this->instance = $DB->get_record('sesiontarea', array('id'=>$update->id), '*', MUST_EXIST);

        // Load the sesiontareament so the plugins have access to it.
     ...
     *
     * @param sesiontarea_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(sesiontarea_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
        if ($plugin->is_visible()) {
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'sesiontarea'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'sesiontarea'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);

        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'sesiontarea'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'sesiontarea'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    protected function get_module_name() {
        ...
        self::$modulename = get_string('modulename', 'sesiontarea');
        return self::$modulename;
    }
    ...
    protected function get_module_name_plural() {
        ...
        self::$modulenameplural = get_string('modulenameplural', 'sesiontarea');
        return self::$modulenameplural;
    }

    /**
     * Has this sesiontareament been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this sesiontareament
     *
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('sesiontarea', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
            throw new coding_exception('Improper use of the sesiontareament class. ' .
                                       'Cannot load the sesiontareament record.');
        }
        return $this->instance;
    }
    ...
    public function get_course_module() {
        ...
        if ($this->context->contextlevel == CONTEXT_MODULE) {
            $this->coursemodule = get_coursemodule_from_id('sesiontarea',
        ...
        if ($this->get_instance()->grade >= 0) {
            ...
            if ($editing && $this->get_instance()->grade > 0) {
                ...
                $o .= '<label class="accesshide" for="quickgrade_' . $userid . '">' .
                       get_string('usergrade', 'sesiontarea') .
                       '</label>';
                $o .= '<input type="text"
    ...
    public function list_participants($currentgroup, $idsonly) {
        if ($idsonly) {
            return get_enrolled_users($this->context, 'mod/sesiontarea:submit', $currentgroup, 'u.id');
        } else {
            return get_enrolled_users($this->context, 'mod/sesiontarea:submit', $currentgroup);
        }
    }

    /**
     * Load a count of valid teams for this sesiontareament.
     *
    ...
    public function count_participants($currentgroup) {
        return count_enrolled_users($this->context, 'mod/sesiontarea:submit', $currentgroup);
    }
    ...
    public function count_submissions_need_grading() {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group sesiontareament because the submission is shared.
            return 0;
        }

        $currentgroup = groups_get_activity_group($this->get_course_module(), true);
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/sesiontarea:submit', $currentgroup, false);

        $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                 FROM {sesiontarea_submission} mxs
                                 WHERE mxs.sesiontareament = :sesiontareaid2 GROUP BY mxs.userid';
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {sesiontarea_grades} mxg
                            WHERE mxg.sesiontareament = :sesiontareaid3 GROUP BY mxg.userid';

        $params['sesiontareaid'] = $this->get_instance()->id;
        $params['sesiontareaid2'] = $this->get_instance()->id;
        $params['sesiontareaid3'] = $this->get_instance()->id;
        $params['submitted'] = SESIONTAREA_SUBMISSION_STATUS_SUBMITTED;
    ...
    public function count_grades() {
        ...
        $sql = 'SELECT COUNT(g.userid)
                   FROM {sesiontarea_grades} g
                   JOIN(' . $esql . ') e ON e.id = g.userid
                   WHERE g.sesiontareament = :sesiontareaid';
    ...
    public function count_submissions_with_status($status) {
        ...
        if ($this->get_instance()->teamsubmission) {
            $maxattemptsql = 'SELECT mxs.groupid, MAX(mxs.attemptnumber) AS maxattempt
                              FROM {sesiontarea_submission} mxs
                              WHERE mxs.sesiontareament = :sesiontareaid2 GROUP BY mxs.groupid';
    ...
    protected function get_grading_userid_list() {
        $filter = get_user_preferences('sesiontarea_filter', '');
        $table = new sesiontarea_grading_table($this, 0, $filter, 0, false);
    ...
    protected function pack_files($filesforzipping) {
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'sesiontareament_');
        // Zip files.
    ...
    /**
     * Finds all sesiontareament notifications that have yet to be mailed out, and mails them.
     *
    ...
    public static function cron() {
        ...
        $sql = 'SELECT a.course, a.name, a.blindmarking, a.revealidentities,
                 ...
                 FROM {sesiontarea} a
                 JOIN {sesiontarea_grades} g ON g.sesiontareament = a.id
                 LEFT JOIN {sesiontarea_user_flags} uf ON uf.sesiontareament = a.id AND uf.userid = g.userid
        ...
        mtrace('Processing ' . count($submissions) . ' sesiontareament submissions ...');
        ...
        foreach ($submissions as $submission) {
            ...
            mtrace("Processing sesiontareament submission $submission->id ...");
            ...
            if (!array_key_exists($submission->sesiontareament, $modcache)) {
                $mod = get_coursemodule_from_instance('sesiontarea', $submission->sesiontareament, $course->id);
                if (empty($mod)) {
                    mtrace('Could not find course module for sesiontareament id ' . $submission->sesiontareament);
                    continue;
                }
                $modcache[$submission->sesiontareament] = $mod;
            } else {
                $mod = $modcache[$submission->sesiontareament];
            }
            // Context lookups are already cached.
            $contextmodule = context_module::instance($mod->id);

            if (!$mod->visible) {
                // Hold mail notification for hidden sesiontareaments until later.
                continue;
            }

            // Need to send this to the student.
            $messagetype = 'feedbackavailable';
            $eventtype = 'sesiontarea_notification';
            $updatetime = $submission->lastmodified;
            $modulename = get_string('modulename', 'sesiontarea');

            $uniqueid = 0;
            if ($submission->blindmarking && !$submission->revealidentities) {
                $uniqueid = self::get_uniqueid_for_user_static($submission->sesiontareament, $user->id);
            }
            $showusers = $submission->blindmarking && !$submission->revealidentities;
            self::send_sesiontareament_notification($grader,
                                               $user,
                                               $messagetype,
                                               $eventtype,
                                               $updatetime,
            ...
            $flags = $DB->get_record('sesiontarea_user_flags', array('userid'=>$user->id, 'sesiontareament'=>$submission->sesiontareament));
            if ($flags) {
                $flags->mailed = 1;
                $DB->update_record('sesiontarea_user_flags', $flags);
            } else {
                $flags = new stdClass();
                $flags->userid = $user->id;
                $flags->sesiontareament = $submission->sesiontareament;
                $flags->mailed = 1;
        ...
        mtrace('Done processing ' . count($submissions) . ' sesiontareament submissions');
    ...
    /**
     * Update user flags for this user in this sesiontareament.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->sesiontareament <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('sesiontarea_user_flags', $flags);
        return $result;
    }

    /**
     * Update a grade in the grade table for the sesiontareament and in the gradebook.
     *
     * @param stdClass $grade a grade record keyed on id
     * @return bool true for success
     */
    public function update_grade($grade) {
        ...
        $result = $DB->update_record('sesiontarea_grades', $grade);
    ...
    protected function view_grant_extension($mform) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/mod/sesiontarea/extensionform.php');
        ...
        $header = new sesiontarea_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'sesiontarea'));
        ...
        if (!$mform) {
            $formparams = array($this->get_course_module()->id,
                                ...
                                $this->get_instance(),
                                $data);
            $mform = new mod_sesiontarea_extension_form(null, $formparams);
        }
        $o .= $this->get_renderer()->render(new sesiontarea_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the sesiontareament.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
    ...
    public function get_submission_group_members_who_have_not_submitted($groupid, $onlyids) {
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == SESIONTAREA_SUBMISSION_STATUS_SUBMITTED) {
                unset($members[$id]);
            } else {
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'sesiontarea') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('sesiontareament'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        $submission = null;
        $submissions = $DB->get_records('sesiontarea_submission', $params, 'attemptnumber DESC', '*', 0, 1);
        if ($submissions) {
        ...
        if ($create) {
            ...
            $submission->sesiontareament = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            $submission->status = SESIONTAREA_SUBMISSION_STATUS_DRAFT;
    ...
    /**
     * View a summary listing of all sesiontareaments in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('sesiontarea', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new sesiontarea_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['sesiontarea'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            if (has_capability('mod/sesiontarea:grade', $context)) {
                $submitted = $sesiontareament->count_submissions_with_status(SESIONTAREA_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/sesiontarea:submit', $context)) {
                $usersubmission = $sesiontareament->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'sesiontarea');
                } else {
                    $submitted = get_string('submissionstatus_', 'sesiontarea');
                }
            }
            $grading_info = grade_get_grades($course->id, 'mod', 'sesiontarea', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_sesiontarea_info($cm->id, $cm->name, $sectionname, $timedue, $submitted, $grade);
    ...
    /**
     * This is used for team sesiontareaments to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_TEXT);
        $item = null;
        if ($pluginsubtype == 'sesiontareasubmission') {
            ...
            if ($item->userid != $USER->id) {
                require_capability('mod/sesiontarea:grade', $this->context);
            }
            $o .= $this->get_renderer()->render(new sesiontarea_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new sesiontarea_submission_plugin_submission($plugin,
                                                              $item,
                                                              sesiontarea_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            $logmessage = get_string('viewsubmissionforuser', 'sesiontarea', $item->userid);
            ...
            $o .= $this->get_renderer()->render(new sesiontarea_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new sesiontarea_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              sesiontarea_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));
            $logmessage = get_string('viewfeedbackforuser', 'sesiontarea', $item->userid);
     ...
     * @param sesiontarea_plugin $plugin - The sesiontareament plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        ...
        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'sesiontarea');
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component) {
        ...
        if ($CFG->enableportfolios) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('sesiontarea_portfolio_caller', $portfolioparams, 'mod_sesiontarea');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new sesiontarea_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'sesiontarea')));
        $gradingresult = new sesiontarea_gradingmessage(get_string('savegradingresult', 'sesiontarea'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new sesiontarea_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'sesiontarea')));
        $gradingresult = new sesiontarea_gradingmessage(get_string('quickgradingresult', 'sesiontarea'),
                                                   $message,
                                                   $this->get_course_module()->id);
        $o .= $this->get_renderer()->render($gradingresult);
        $o .= $this->view_footer();
    ...
    /**
     * Does this user have grade permission for this sesiontareament?
     *
     * @return bool
     */
    protected function can_grade() {
        // Permissions check.
        if (!has_capability('mod/sesiontarea:grade', $this->context)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all sesiontareament submissions.
     *
    ...
    protected function download_submissions() {
        ...
        $students = get_enrolled_users($this->context, "mod/sesiontarea:submit");
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->get_instance()->teamsubmission) {
                    ...
                    if ($submissiongroup) {
                        ...
                        $groupname = get_string('defaultteam', 'sesiontarea') . '-';
                    }
                } else {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'sesiontarea'));
        ...
        if (count($filesforzipping) == 0) {
            $header = new sesiontarea_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'sesiontarea'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'sesiontarea'));
            $url = new moodle_url('/mod/sesiontarea/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            $this->add_to_log('download all submissions', get_string('downloadall', 'sesiontarea'));
     ...
     * @param string $url The url to the sesiontarea module instance.
     * @return void
     */
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_sesiontarea');
        return $this->output;
    }
     ...
     *
     * For team sesiontareaments there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('sesiontareament'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('sesiontareament'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('sesiontarea_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('sesiontareament'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('sesiontarea_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->sesiontareament = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('sesiontarea_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('sesiontarea_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->sesiontareament   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('sesiontarea_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('sesiontareament'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('sesiontarea_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grade_page($mform) {
        ...
        require_once($CFG->dirroot . '/mod/sesiontarea/gradeform.php');

        // Need submit permission to submit an sesiontareament.
        require_capability('mod/sesiontarea:grade', $this->context);

        $header = new sesiontarea_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'sesiontarea'));
        $o .= $this->get_renderer()->render($header);
        ...
        $cache = cache::make_from_params(cache_store::MODE_SESSION, 'mod_sesiontarea', 'useridlist');
        if (!$userid) {
        ...
        if ($this->can_view_submission($userid)) {
            ...
            if ($teamsubmission) {
                $showsubmit = $showedit &&
                              $teamsubmission &&
                              ($teamsubmission->status == SESIONTAREA_SUBMISSION_STATUS_DRAFT);
            } else {
                $showsubmit = $showedit &&
                              $submission &&
                              ($submission->status == SESIONTAREA_SUBMISSION_STATUS_DRAFT);
            ...
            $submissionstatus = new sesiontarea_submission_status($instance->allowsubmissionsfromdate,
                                                             ...
                                                             $this->get_course_module()->id,
                                                             $this->get_course()->id,
                                                             sesiontarea_submission_status::GRADER_VIEW,
                                                             $showedit,
                                                             $showsubmit,
                                                             $viewfullnames,
                                                             $extensionduedate,
        ...
        if ($attemptnumber != -1) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'sesiontarea', $params);
        ...
        if (!$mform) {
            ...
            $mform = new mod_sesiontarea_grade_form(null,
                                               $formparams,
                                               'post',
                                               '',
        ...
        $o .= $this->get_renderer()->render(new sesiontarea_form('gradingform', $mform));

        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new sesiontarea_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    protected function view_reveal_identities_confirm() {
        ...
        require_capability('mod/sesiontarea:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/sesiontarea/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/sesiontarea/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'sesiontarea'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();
        $this->add_to_log('view', get_string('viewrevealidentitiesconfirm', 'sesiontarea'));
        return $o;
    }
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/sesiontarea/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this sesiontareament.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/sesiontarea/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/sesiontarea/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/sesiontarea/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'sesiontarea');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/sesiontarea/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'sesiontarea');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/sesiontarea:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/sesiontarea/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'sesiontarea');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/sesiontarea/view.php' .
                           '?id=' .  $cmid .
                           '&plugin=' . $plugin->get_type() .
                           '&pluginsubtype=sesiontareafeedback' .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'sesiontarea'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_sesiontarea', 'submissions');

        $perpage = get_user_preferences('sesiontarea_perpage', 10);
        $filter = get_user_preferences('sesiontarea_filter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller);
        $quickgrading = get_user_preferences('sesiontarea_quickgrading', false);
        ...
        $gradingoptionsform = new mod_sesiontarea_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_sesiontarea_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new sesiontarea_grading_table($this, $perpage, $filter, 0, true);
            $table = $this->get_renderer()->render($gradingtable);
            $quickformparams = array('cm'=>$this->get_course_module()->id, 'gradingtable'=>$table);
            $quickgradingform = new mod_sesiontarea_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new sesiontarea_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new sesiontarea_grading_table($this, $perpage, $filter, 0, false);
        ...
        if (count($users) != 0) {
            ...
            $sesiontareaform = new sesiontarea_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($sesiontareaform);
        }
        $sesiontareaform = new sesiontarea_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_sesiontarea.init_grading_options');
        $o .= $this->get_renderer()->render($sesiontareaform);
        return $o;
    }
    ...
    protected function view_grading_page() {
        ...
        $logmessage = get_string('viewsubmissiongradingtable', 'sesiontarea');
    ...
    /**
     * Message for students when sesiontareament submissions have been closed.
     *
     * @return string
     */
    protected function view_student_error_message() {
        ...
        $o = '';
        // Need submit permission to submit an sesiontareament.
        require_capability('mod/sesiontarea:submit', $this->context);

        $header = new sesiontarea_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('editsubmission', 'sesiontarea'));
        $o .= $this->get_renderer()->render($header);

        $o .= $this->get_renderer()->notification(get_string('submissionsclosed', 'sesiontarea'));
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/sesiontarea/submission_form.php');
        // Need submit permission to submit an sesiontareament.
        require_capability('mod/sesiontarea:submit', $this->context);
        ...
        if (!$mform) {
            $mform = new mod_sesiontarea_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new sesiontarea_form('editsubmissionform', $mform));

        $o .= $this->view_footer();
        $this->add_to_log('view submit sesiontareament form', get_string('viewownsubmissionform', 'sesiontarea'));

        return $o;
    }

    /**
     * See if this sesiontareament has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/sesiontarea:submit', $this->context)) {
            return true;
        }
        if (has_capability('mod/sesiontarea:grade', $this->context)) {
            return true;
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_sesiontarea_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/sesiontarea/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_sesiontarea_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $submitforgradingpage = new sesiontarea_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        $logmessage = get_string('viewownsubmissionform', 'sesiontarea');
        $this->add_to_log('view confirm submit sesiontareament form', $logmessage);
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            $showedit = has_capability('mod/sesiontarea:submit', $this->context) &&
            ...
            $gradingmanager = get_grading_manager($this->context, 'mod_sesiontarea', 'submissions');
            ...
            if ($teamsubmission && ($teamsubmission->status != SESIONTAREA_SUBMISSION_STATUS_DRAFT)) {
                $showsubmit = false;
            }
            if ($submission && ($submission->status != SESIONTAREA_SUBMISSION_STATUS_DRAFT)) {
                $showsubmit = false;
            }
            if (!$this->get_instance()->submissiondrafts) {
                $showsubmit = false;
            ...
            $submissionstatus = new sesiontarea_submission_status($instance->allowsubmissionsfromdate,
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $this->get_course()->id,
                                                              sesiontarea_submission_status::STUDENT_VIEW,
                                                              $showedit,
                                                              $showsubmit,
                                                              $viewfullnames,
                                                              $extensionduedate,
            ...
            $cangrade = has_capability('mod/sesiontarea:grade', $this->get_context());
            ...
            if ((!empty($gradebookgrade->grade) && ($cangrade || !$gradebookgrade->hidden)) ||
                ...
                $feedbackstatus = new sesiontarea_feedback_status($gradefordisplay,
                                                      $gradeddate,
                                                      $grader,
                                                      $this->get_feedback_plugins(),
                                                      $grade,
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('sesiontarea_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('sesiontarea_submission', $params, 'attemptnumber ASC');
    ...
    protected function view_submission_page() {
        ...
        $o = '';
        $o .= $this->get_renderer()->render(new sesiontarea_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id));

        if ($this->can_grade()) {
            $draft = SESIONTAREA_SUBMISSION_STATUS_DRAFT;
            $submitted = SESIONTAREA_SUBMISSION_STATUS_SUBMITTED;
            if ($instance->teamsubmission) {
                $summary = new sesiontarea_grading_summary($this->count_teams(),
                ...
                $summary = new sesiontarea_grading_summary($this->count_participants(0),
                                                      $instance->submissiondrafts,
                                                      $this->count_submissions_with_status($draft),
                                                      $this->is_any_submission_plugin_enabled(),
                                                      $this->count_submissions_with_status($submitted),
        ...
        $o .= $this->view_footer();
        $this->add_to_log('view', get_string('viewownsubmissionstatus', 'sesiontarea'));
        return $o;
    }
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        $sesiontarea = clone $this->get_instance();
        $sesiontarea->cmidnumber = $this->get_course_module()->id;

        return sesiontarea_grade_item_update($sesiontarea, $gradebookgrade);
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this sesiontareament qualifies as submitted or draft.
        $team = $this->get_submission_group_members($submission->groupid, true);

        $allsubmitted = true;
        $anysubmitted = false;
        $result = true;
        if ($submission->status != SESIONTAREA_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                $membersubmission = $this->get_user_submission($member->id, false, $submission->attemptnumber);

                if (!$membersubmission || $membersubmission->status != SESIONTAREA_SUBMISSION_STATUS_SUBMITTED) {
                    $allsubmitted = false;
                    if ($anysubmitted) {
                        break;
                    }
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('sesiontarea_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = SESIONTAREA_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = SESIONTAREA_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('sesiontarea_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('sesiontarea_submission', $submission) && $result;
        }
    ...
    /**
     * Is this sesiontareament open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the sesiontareament locked?
     *
    ...
    public function submissions_open($userid = 0) {
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == SESIONTAREA_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the sesiontareament.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        $files = $fs->get_area_files($this->get_context()->id,
                                     ...
                                     'timemodified',
                                     false);
        return $this->get_renderer()->sesiontarea_files($this->context, $submissionid, $area, $component);
    ...
    protected function get_graders($userid) {
        $potentialgraders = get_enrolled_users($this->context, 'mod/sesiontarea:grade');
     ...
     * @param string $sesiontareamentname
     */
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext  = format_string($course->shortname, true, $formatparams) .
                     ...
                     $modulename .
                     ' -> ' .
                     format_string($sesiontareamentname, true, $formatparams) . "\n";
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'sesiontarea', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/sesiontarea/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/sesiontarea/view.php?id=' . $coursemodule->id . '">' .
                     format_string($sesiontareamentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'sesiontarea', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_sesiontareament_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        $info = new stdClass();
        if ($blindmarking) {
            $info->username = get_string('participant', 'sesiontarea') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'sesiontarea');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->sesiontareament = format_string($sesiontareamentname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/sesiontarea/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'sesiontarea', $info);
        ...
        $eventdata->component       = 'mod_sesiontarea';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->sesiontareament;
    ...
    public function send_notification($userfrom,
                                      ...
                                      $eventtype,
                                      $updatetime) {
        self::send_sesiontareament_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    protected function process_submit_for_grading($mform) {
        ...
        if (!$mform->is_cancelled()) {
            ...
            if ($submission->status != SESIONTAREA_SUBMISSION_STATUS_SUBMITTED) {
    ...
    protected function process_save_extension(& $mform) {
        ...
        // Need submit permission to submit an sesiontareament.
        require_capability('mod/sesiontarea:grantextension', $this->context);
        ...
        $mform = new mod_sesiontarea_extension_form(null, array($this->get_course_module()->id,
                                                           $userid,
                                                           $batchusers,
                                                           $this->get_instance(),
                                                           null));
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'sesiontarea');
        }
        ...
        $params['sesiontareaid1'] = $this->get_instance()->id;
        $params['sesiontareaid2'] = $this->get_instance()->id;

        // Check them all for currency.
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {sesiontarea_grades} mxg
                            WHERE mxg.sesiontareament = :sesiontareaid1 GROUP BY mxg.userid';
        ...
        $sql = 'SELECT u.id as userid, g.grade as grade, g.timemodified as lastmodified
                ...
                LEFT JOIN {sesiontarea_grades} g ON
                    u.id = g.userid AND
                    g.sesiontareament = :sesiontareaid2 AND
                    g.attemptnumber = gmx.maxattempt
                WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'sesiontarea');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('sesiontareafeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/sesiontarea',
                                          $this->course->id,
                                          'mod',
                                          'sesiontarea',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'sesiontarea');
    }
    ...
    protected function process_reveal_identities() {
        ...
        require_capability('mod/sesiontarea:revealidentities', $this->context);
        if (!confirm_sesskey()) {
            return false;
        }

        // Update the sesiontareament record.
        $update = new stdClass();
        $update->id = $this->get_instance()->id;
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $grades = $DB->get_records('sesiontarea_grades', array('sesiontareament'=>$this->get_instance()->id));
        ...
        $this->add_to_log('reveal identities', get_string('revealidentities', 'sesiontarea'));
    }
    ...
    protected function process_save_grading_options() {
        ...
        $mform = new mod_sesiontarea_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('sesiontarea_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('sesiontarea_filter', $formdata->filter);
            }
            if ($showquickgrading) {
                set_user_preference('sesiontarea_quickgrading', isset($formdata->quickgrading));
            }
        }
    }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'sesiontarea', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'sesiontarea');
        }
        return $info;
    }
    ...
    protected function format_submission_for_log(stdClass $submission) {
        $info = '';
        $info .= get_string('submissionstatus', 'sesiontarea') .
                 ': ' .
                 get_string('submissionstatus_' . $submission->status, 'sesiontarea') .
                 '. <br>';
    ...
    /**
     * Copy the current sesiontareament submission from the last submitted attempt.
     *
    ...
    protected function process_copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != SESIONTAREA_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'sesiontarea');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'sesiontarea');
            return false;
        }
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted sesiontareaments
    ...
    /**
     * Save sesiontareament submission.
     *
     * @param  moodleform $mform
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        if ($data = $mform->get_data()) {
            ...
            // Get the flags to check if it is locked.
            if ($flags && $flags->locked) {
                print_error('submissionslocked', 'sesiontarea');
                return true;
            }
            ...
            if ($pluginerror || $allempty) {
                if ($allempty) {
                    $notices[] = get_string('submissionempty', 'mod_sesiontarea');
                }
                return false;
            }
     ...
     * This is specific to the sesiontareament, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'sesiontarea', $this->get_instance()->grade);
                $gradingelement = $mform->addElement('text', 'grade', $name);
                $mform->addHelpButton('grade', 'gradeoutofhelp', 'sesiontarea');
        ...
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'sesiontarea'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'sesiontarea', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'sesiontarea'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'sesiontarea'));
            $mform->setDefault('applytoall', 1);
        }

        // Do not show if we are editing a previous attempt.
        if ($attemptnumber == -1 && $this->get_instance()->attemptreopenmethod != SESIONTAREA_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'sesiontarea'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'sesiontarea');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'sesiontarea'), $attemptreopenmethod);
            ...
            if ($maxattempts == SESIONTAREA_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'sesiontarea');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'sesiontarea'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'sesiontarea'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == SESIONTAREA_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == SESIONTAREA_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'sesiontarea'));
        ...
        $buttonarray=array();
        $name = get_string('savechanges', 'sesiontarea');
        $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
        if (!$last) {
            $name = get_string('savenext', 'sesiontarea');
        ...
        if ($rownum > 0) {
            $name = get_string('previous', 'sesiontarea');
            $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
        }

        if (!$last) {
            $name = get_string('nosavebutnext', 'sesiontarea');
    ...
    protected function process_save_grade(&$mform) {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            $maxattemptsreached = !empty($submission) &&
                                  ...
                                  $instance->maxattempts != SESIONTAREA_UNLIMITED_ATTEMPTS;
            $shouldreopen = false;
            if ($instance->attemptreopenmethod == SESIONTAREA_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
            ...
            if ($instance->attemptreopenmethod == SESIONTAREA_ATTEMPT_REOPEN_METHOD_MANUAL &&
     ...
     * @return bool
     */
    public static function can_upgrade_sesiontareament($type, $version) {
        $sesiontareament = new sesiontarea(null, null, null);
        return $sesiontareament->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an sesiontareament from the 2.2 module.
     *
     * @param string $type The plugin type
     * @param int $version The plugin version
     * @return bool
    ...
    /**
     * Copy all the files from the old sesiontareament files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old sesiontareament context id
     * @param int $oldcomponent The old sesiontareament component ('sesiontareament')
     * @param int $oldfilearea The old sesiontareament filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new sesiontareament context id
     * @param int $newcomponent The new sesiontareament component ('sesiontareament')
     * @param int $newfilearea The new sesiontareament filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group sesiontareament
     * at most 1 time.
     *
    ...
    protected function process_add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == SESIONTAREA_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != SESIONTAREA_UNLIMITED_ATTEMPTS &&
        ...
        $submission->status = SESIONTAREA_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        global $DB, $CFG;
        $grades = array();
        $sesiontareamentid = $this->get_instance()->id;
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('sesiontareafeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {sesiontarea_grades} mxg
                            WHERE mxg.sesiontareament = :sesiontareaid2 GROUP BY mxg.userid';

        // When the gradebook asks us for grades - only return the last attempt for each user.
        $params = array('sesiontareaid1'=>$sesiontareamentid,
                        'sesiontareaid2'=>$sesiontareamentid,
                        'sesiontareaid3'=>$sesiontareamentid,
                        'sesiontareaid4'=>$sesiontareamentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                LEFT JOIN ( ' . $grademaxattempt . ' ) gmx ON u.id = gmx.userid
                                                LEFT JOIN {sesiontarea_submission} s
                                                    ON u.id = s.userid and s.sesiontareament = :sesiontareaid3 AND
                                                    s.attemptnumber = smx.maxattempt
                                                JOIN {sesiontarea_grades} g
                                                    ON u.id = g.userid and g.sesiontareament = :sesiontareaid4 AND
                                                    g.attemptnumber = gmx.maxattempt' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - sesiontarea them a random id.
     *
     * @param int $sesiontareaid The sesiontareaid to lookup
     */
    public static function allocate_unique_ids($sesiontareaid) {
        global $DB;

        $cm = get_coursemodule_from_instance('sesiontarea', $sesiontareaid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/sesiontarea:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        $record = new stdClass();
        $record->sesiontareament = $sesiontareaid;
        foreach ($users as $user) {
            $record = $DB->get_record('sesiontarea_user_mapping',
                                      array('sesiontareament'=>$sesiontareaid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->userid = $user->id;
                $DB->insert_record('sesiontarea_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this sesiontareament.
     *
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($sesiontareaid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('sesiontareament'=>$sesiontareaid, 'userid'=>$userid);
        if ($record = $DB->get_record('sesiontarea_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($sesiontareaid);
        ...
        return $DB->insert_record('sesiontarea_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this sesiontareament.
     *
     * @param int $sesiontareaid The id of the sesiontareament this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($sesiontareaid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('sesiontarea_user_mapping',
                                      array('sesiontareament'=>$sesiontareaid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
...
/**
 * Portfolio caller class for mod_sesiontarea.
 *
 * @package   mod_sesiontarea
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sesiontarea_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the sesiontareament we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('sesiontarea', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the sesiontareament type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_sesiontarea');
            }
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'sesiontareament.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'sesiontarea' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return sesiontarea_submission_plugin
     */
    protected function get_submission_plugin() {
        ...
        return $sesiontareament->get_submission_plugin_by_type($this->plugin);
    }
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/sesiontarea:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'sesiontarea');
    }
