...
/**
 * This file contains a renderer for the asistenciasment class
 *
...
/**
 * A custom renderer class that extends the plugin_renderer_base and is used by the asistencias module.
 *
 * @package mod_asistencias
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_asistencias_renderer extends plugin_renderer_base {

    /**
     * Rendering asistenciasment files
     *
     ...
     * @return string
     */
    public function asistencias_files(context $context, $userid, $filearea, $component) {
        return $this->render(new asistencias_files($context, $userid, $filearea, $component));
    }

    /**
     * Rendering asistenciasment files
     *
     * @param asistencias_files $tree
     * @return string
     */
    public function render_asistencias_files(asistencias_files $tree) {
        $this->htmlid = 'asistencias_files_tree_'.uniqid();
        $this->page->requires->js_init_call('M.mod_asistencias.init_tree', array(true, $this->htmlid));
     ...
     * @param asistencias_gradingmessage $result The result to render
     * @return string
     */
    public function render_asistencias_gradingmessage(asistencias_gradingmessage $result) {
     ...
     * @param asistencias_form $form The form to render
     * @return string
     */
    public function render_asistencias_form(asistencias_form $form) {
        $o = '';
     ...
     *
     * @param asistencias_user_summary $summary The user summary to render
     * @return string
     */
    public function render_asistencias_user_summary(asistencias_user_summary $summary) {
        $o = '';
        ...
        if ($summary->blindmarking) {
            $o .= get_string('hiddenuser', 'asistencias') . $summary->uniqueidforuser;
        } else {
     ...
     *
     * @param asistencias_submit_for_grading_page $page
     * @return string
     */
    public function render_asistencias_submit_for_grading_page($page) {
        $o = '';

        $o .= $this->output->container_start('submitforgrading');
        $o .= $this->output->heading(get_string('submitasistenciasment', 'asistencias'), 3);
        $o .= $this->output->spacer(array('height'=>30));

        $cancelurl = new moodle_url('/mod/asistencias/view.php', array('id' => $page->coursemoduleid));
        if (count($page->notifications)) {
            // At least one of the submission plugins is not ready for submission.

            $o .= $this->output->heading(get_string('submissionnotready', 'asistencias'), 4);
     ...
     * Render the header.
     *
     * @param asistencias_header $header
     * @return string
     */
    public function render_asistencias_header(asistencias_header $header) {
        $o = '';
        ...
        $this->page->set_title(get_string('pluginname', 'asistencias'));
        $this->page->set_heading($header->asistencias->name);

        $o .= $this->output->header();
        if ($header->preface) {
            $o .= $header->preface;
        }
        $heading = format_string($header->asistencias->name, false, array('context' => $header->context));
        $o .= $this->output->heading($heading);

        if ($header->showintro) {
            $o .= $this->output->box_start('generalbox boxaligncenter', 'intro');
            $o .= format_module_intro('asistencias', $header->asistencias, $header->coursemoduleid);
            $o .= $this->output->box_end();
        }
     ...
     *
     * @param asistencias_grading_summary $summary
     * @return string
     */
    public function render_asistencias_grading_summary(asistencias_grading_summary $summary) {
        // Create a table for the data.
        $o = '';
        $o .= $this->output->container_start('gradingsummary');
        $o .= $this->output->heading(get_string('gradingsummary', 'asistencias'), 3);
        $o .= $this->output->box_start('boxaligncenter gradingsummarytable');
        $t = new html_table();

        // Status.
        if ($summary->teamsubmission) {
            $this->add_table_row_tuple($t, get_string('numberofteams', 'asistencias'),
                                       $summary->participantcount);
        } else {
            $this->add_table_row_tuple($t, get_string('numberofparticipants', 'asistencias'),
                                       $summary->participantcount);
        }

        // Drafts.
        if ($summary->submissiondraftsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofdraftsubmissions', 'asistencias'),
                                       $summary->submissiondraftscount);
        }

        // Submitted for grading.
        if ($summary->submissionsenabled) {
            $this->add_table_row_tuple($t, get_string('numberofsubmittedasistenciasments', 'asistencias'),
                                       $summary->submissionssubmittedcount);
            if (!$summary->teamsubmission) {
                $this->add_table_row_tuple($t, get_string('numberofsubmissionsneedgrading', 'asistencias'),
        ...
        if ($summary->duedate) {
            ...
            $this->add_table_row_tuple($t, get_string('duedate', 'asistencias'),
                                       userdate($duedate));

            // Time remaining.
            $due = '';
            if ($duedate - $time <= 0) {
                $due = get_string('asistenciasmentisdue', 'asistencias');
            } else {
                $due = format_time($duedate - $time);
            }
            $this->add_table_row_tuple($t, get_string('timeremaining', 'asistencias'), $due);

            if ($duedate < $time) {
                $cutoffdate = $summary->cutoffdate;
                if ($cutoffdate) {
                    if ($cutoffdate > $time) {
                        $late = get_string('latesubmissionsaccepted', 'asistencias');
                    } else {
                        $late = get_string('nomoresubmissionsaccepted', 'asistencias');
                    }
                    $this->add_table_row_tuple($t, get_string('latesubmissions', 'asistencias'), $late);
                }
            }
        ...
        $url = new moodle_url('/mod/asistencias/view.php', $urlparams);
        $o .= $this->output->action_link($url, get_string('viewgrading', 'asistencias'));
        $o .= $this->output->container_end();
     ...
     *
     * @param asistencias_feedback_status $status
     * @return string
     */
    public function render_asistencias_feedback_status(asistencias_feedback_status $status) {
        global $DB, $CFG;
        $o = '';

        $o .= $this->output->container_start('feedback');
        $o .= $this->output->heading(get_string('feedback', 'asistencias'), 3);
        ...
        if (isset($status->gradefordisplay)) {
            ...
            // Grade date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedon', 'asistencias'));
        ...
        if ($status->grader) {
            // Grader.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradedby', 'asistencias'));
        ...
        foreach ($status->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() &&
                ...
                $displaymode = asistencias_feedback_plugin_feedback::SUMMARY;
                $pluginfeedback = new asistencias_feedback_plugin_feedback($plugin,
                                                                      $status->grade,
                                                                      $displaymode,
     ...
     *
     * @param asistencias_submission_status $status
     * @return string
     */
    public function render_asistencias_submission_status(asistencias_submission_status $status) {
        $o = '';
        $o .= $this->output->container_start('submissionstatustable');
        $o .= $this->output->heading(get_string('submissionstatusheading', 'asistencias'), 3);
        $time = time();
        ...
        if ($status->allowsubmissionsfromdate &&
            ...
            if ($status->alwaysshowdescription) {
                ...
                $o .= get_string('allowsubmissionsfromdatesummary', 'asistencias', $date);
            } else {
                $date = userdate($status->allowsubmissionsfromdate);
                $o .= get_string('allowsubmissionsanddescriptionfromdatesummary', 'asistencias', $date);
            }
            $o .= $this->output->box_end();
        }
        ...
        if ($status->teamsubmissionenabled) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('submissionteam', 'asistencias'));
            $group = $status->submissiongroup;
            if ($group) {
                $cell2 = new html_table_cell(format_string($group->name, false, $status->context));
            } else {
                $cell2 = new html_table_cell(get_string('defaultteam', 'asistencias'));
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
        }

        if ($status->attemptreopenmethod != ASISTENCIAS_ATTEMPT_REOPEN_METHOD_NONE) {
            $currentattempt = 1;
            if (!$status->teamsubmissionenabled) {
            ...
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('attemptnumber', 'asistencias'));
            $maxattempts = $status->maxattempts;
            if ($maxattempts == ASISTENCIAS_UNLIMITED_ATTEMPTS) {
                $message = get_string('currentattempt', 'asistencias', $currentattempt);
            } else {
                $message = get_string('currentattemptof', 'asistencias', array('attemptnumber'=>$currentattempt,
        ...
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('submissionstatus', 'asistencias'));
        if (!$status->teamsubmissionenabled) {
            if ($status->submission) {
                $statusstr = get_string('submissionstatus_' . $status->submission->status, 'asistencias');
                $cell2 = new html_table_cell($statusstr);
                $cell2->attributes = array('class'=>'submissionstatus' . $status->submission->status);
            } else {
                if (!$status->submissionsenabled) {
                    $cell2 = new html_table_cell(get_string('noonlinesubmissions', 'asistencias'));
                } else {
                    $cell2 = new html_table_cell(get_string('noattempt', 'asistencias'));
                }
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
            ...
            if ($status->teamsubmission) {
                ...
                $submissionsummary = get_string('submissionstatus_' . $teamstatus, 'asistencias');
                $groupid = 0;
                ...
                foreach ($members as $member) {
                    ...
                    $url = new moodle_url('/user/view.php', $urlparams);
                    if ($status->view == asistencias_submission_status::GRADER_VIEW && $status->blindmarking) {
                ...
                if (count($userslist) > 0) {
                    ...
                    $formatteduserstr = get_string('userswhoneedtosubmit', 'asistencias', $userstr);
                ...
                $cell2 = new html_table_cell(get_string('nosubmission', 'asistencias'));
                if (!$status->submissionsenabled) {
                    $cell2 = new html_table_cell(get_string('noonlinesubmissions', 'asistencias'));
                } else {
                    $cell2 = new html_table_cell(get_string('nosubmission', 'asistencias'));
        ...
        if ($status->locked) {
            ...
            $cell2 = new html_table_cell(get_string('submissionslocked', 'asistencias'));
        ...
        // Grading status.
        $row = new html_table_row();
        $cell1 = new html_table_cell(get_string('gradingstatus', 'asistencias'));

        if ($status->graded) {
            $cell2 = new html_table_cell(get_string('graded', 'asistencias'));
            $cell2->attributes = array('class'=>'submissiongraded');
        } else {
            $cell2 = new html_table_cell(get_string('notgraded', 'asistencias'));
        ...
        if ($duedate > 0) {
            // Due date.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('duedate', 'asistencias'));
            $cell2 = new html_table_cell(userdate($duedate));
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;

            if ($status->view == asistencias_submission_status::GRADER_VIEW) {
                if ($status->cutoffdate) {
                    // Cut off date.
                    $row = new html_table_row();
                    $cell1 = new html_table_cell(get_string('cutoffdate', 'asistencias'));
            ...
            if ($status->extensionduedate) {
                // Extension date.
                $row = new html_table_row();
                $cell1 = new html_table_cell(get_string('extensionduedate', 'asistencias'));
            ...
            // Time remaining.
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timeremaining', 'asistencias'));
            if ($duedate - $time <= 0) {
                if (!$status->submission ||
                        $status->submission->status != ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED) {
                    if ($status->submissionsenabled) {
                        $overduestr = get_string('overdue', 'asistencias', format_time($time - $duedate));
                        $cell2 = new html_table_cell($overduestr);
                        $cell2->attributes = array('class'=>'overdue');
                    } else {
                        $cell2 = new html_table_cell(get_string('duedatereached', 'asistencias'));
                    }
                } else {
        ...
        if ($status->view == asistencias_submission_status::GRADER_VIEW) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('editingstatus', 'asistencias'));
            if ($status->canedit) {
                $cell2 = new html_table_cell(get_string('submissioneditable', 'asistencias'));
                $cell2->attributes = array('class'=>'submissioneditable');
            } else {
                $cell2 = new html_table_cell(get_string('submissionnoteditable', 'asistencias'));
        ...
        if (!empty($status->gradingcontrollerpreview)) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('gradingmethodpreview', 'asistencias'));
        ...
        if ($submission) {
            $row = new html_table_row();
            $cell1 = new html_table_cell(get_string('timemodified', 'asistencias'));
            ...
            foreach ($status->submissionplugins as $plugin) {
                ...
                if ($plugin->is_enabled() &&
                    ...
                    $row = new html_table_row();
                    $cell1 = new html_table_cell($plugin->get_name());
                    $displaymode = asistencias_submission_plugin_submission::SUMMARY;
                    $pluginsubmission = new asistencias_submission_plugin_submission($plugin,
                                                                                $submission,
                                                                                $displaymode,
                                                                                $status->coursemoduleid,
                                                                                $status->returnaction,
        ...
        // Links.
        if ($status->view == asistencias_submission_status::STUDENT_VIEW) {
            if ($status->canedit) {
                if (!$submission) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/asistencias/view.php', $urlparams),
                                                       get_string('addsubmission', 'asistencias'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'asistencias');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                } else if ($submission->status == ASISTENCIAS_SUBMISSION_STATUS_REOPENED) {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editprevioussubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/asistencias/view.php', $urlparams),
                                                       get_string('addnewattemptfromprevious', 'asistencias'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattemptfromprevious_help', 'asistencias');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/asistencias/view.php', $urlparams),
                                                       get_string('addnewattempt', 'asistencias'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('addnewattempt_help', 'asistencias');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
                } else {
                    $o .= $this->output->box_start('generalbox submissionaction');
                    $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/asistencias/view.php', $urlparams),
                                                       get_string('editsubmission', 'asistencias'), 'get');
                    $o .= $this->output->box_start('boxaligncenter submithelp');
                    $o .= get_string('editsubmission_help', 'asistencias');
                    $o .= $this->output->box_end();
                    $o .= $this->output->box_end();
            ...
            if ($status->cansubmit) {
                ...
                $o .= $this->output->box_start('generalbox submissionaction');
                $o .= $this->output->single_button(new moodle_url('/mod/asistencias/view.php', $urlparams),
                                                   get_string('submitasistenciasment', 'asistencias'), 'get');
                $o .= $this->output->box_start('boxaligncenter submithelp');
                $o .= get_string('submitasistenciasment_help', 'asistencias');
                $o .= $this->output->box_end();
                $o .= $this->output->box_end();
            }
        }
    ...
    /**
     * Output the attempt history for this asistenciasment
     *
     * @param asistencias_attempt_history $history
     * @return string
     */
    public function render_asistencias_attempt_history(asistencias_attempt_history $history) {
        $o = '';

        $submittedstr = get_string('submitted', 'asistencias');
        $gradestr = get_string('grade');
        $gradedonstr = get_string('gradedon', 'asistencias');
        $gradedbystr = get_string('gradedby', 'asistencias');
        ...
        $o .= $this->heading(get_string('attempthistory', 'asistencias'), 3);
        ...
        foreach ($history->submissions as $i => $submission) {
            ...
            if ($submission) {
                ...
                $submissionsummary = get_string('nosubmission', 'asistencias');
            }

            $attemptsummaryparams = array('attemptnumber'=>$submission->attemptnumber+1,
                                          'submissionsummary'=>$submissionsummary);
            $o .= $this->heading(get_string('attemptheading', 'asistencias', $attemptsummaryparams), 4);

            $t = new html_table();

            if ($submission) {
                $cell1 = new html_table_cell(get_string('submissionstatus', 'asistencias'));
                $cell2 = new html_table_cell(get_string('submissionstatus_' . $submission->status, 'asistencias'));
                ...
                foreach ($history->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $pluginsubmission = new asistencias_submission_plugin_submission($plugin,
                                                                                    $submission,
                                                                                    asistencias_submission_plugin_submission::SUMMARY,
            ...
            if ($grade) {
                // Heading 'feedback'.
                $title = get_string('feedback', 'asistencias', $i);
                ...
                if ($history->cangrade) {
                    ...
                    $icon = new pix_icon('gradefeedback',
                                            get_string('editattemptfeedback', 'asistencias', $grade->attemptnumber+1),
                                            'mod_asistencias');
                ...
                foreach ($history->feedbackplugins as $plugin) {
                    if ($plugin->is_enabled() &&
                        ...
                        $cell1 = new html_table_cell($plugin->get_name());
                        $pluginfeedback = new asistencias_feedback_plugin_feedback(
                            $plugin, $grade, asistencias_feedback_plugin_feedback::SUMMARY, $history->coursemoduleid,
        ...
        $this->page->requires->yui_module('moodle-mod_asistencias-history', 'Y.one("#' . $containerid . '").history');
     ...
     *
     * @param asistencias_submission_plugin_submission $submissionplugin
     * @return string
     */
    public function render_asistencias_submission_plugin_submission(asistencias_submission_plugin_submission $submissionplugin) {
        $o = '';

        if ($submissionplugin->view == asistencias_submission_plugin_submission::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewsubmission', 'asistencias');
                $icon = $this->output->pix_icon('t/preview', $previewstr);

                $expandstr = get_string('viewfull', 'asistencias');
                ...
                $this->page->requires->js_init_call('M.mod_asistencias.init_plugin_summary', $jsparams);
            ...
            if ($showviewlink) {
                ...
                $o .= $this->output->pix_icon('t/switch_minus',
                                              get_string('viewsummary', 'asistencias'),
                                              null,
                                              array('class'=>$classes));
                $o .= $submissionplugin->plugin->view($submissionplugin->submission);
                $o .= $this->output->box_end();
            }
        } else if ($submissionplugin->view == asistencias_submission_plugin_submission::FULL) {
     ...
     *
     * @param asistencias_grading_table $table
     * @return string
     */
    public function render_asistencias_grading_table(asistencias_grading_table $table) {
        $o = '';
        $o .= $this->output->box_start('boxaligncenter gradingtable');

        $this->page->requires->js_init_call('M.mod_asistencias.init_grading_table', array());
        $this->page->requires->string_for_js('nousersselected', 'asistencias');
        $this->page->requires->string_for_js('batchoperationconfirmgrantextension', 'asistencias');
        $this->page->requires->string_for_js('batchoperationconfirmlock', 'asistencias');
        $this->page->requires->string_for_js('batchoperationconfirmreverttodraft', 'asistencias');
        $this->page->requires->string_for_js('batchoperationconfirmunlock', 'asistencias');
        $this->page->requires->string_for_js('batchoperationconfirmaddattempt', 'asistencias');
        $this->page->requires->string_for_js('editaction', 'asistencias');
     ...
     *
     * @param asistencias_feedback_plugin_feedback $feedbackplugin
     * @return string
     */
    public function render_asistencias_feedback_plugin_feedback(asistencias_feedback_plugin_feedback $feedbackplugin) {
        $o = '';

        if ($feedbackplugin->view == asistencias_feedback_plugin_feedback::SUMMARY) {
            $showviewlink = false;
            ...
            $link = '';
            if ($showviewlink) {
                $previewstr = get_string('viewfeedback', 'asistencias');
                $icon = $this->output->pix_icon('t/preview', $previewstr);
        ...
        } else if ($feedbackplugin->view == asistencias_feedback_plugin_feedback::FULL) {
     ...
     *
     * @param asistencias_course_index_summary $indexsummary
     * @return string
     */
    public function render_asistencias_course_index_summary(asistencias_course_index_summary $indexsummary) {
        $o = '';
        ...
        $strsubmission = get_string('submission', 'asistencias');
        ...
        $currentsection = '';
        foreach ($indexsummary->asistenciasments as $info) {
            $params = array('id' => $info['cmid']);
            $link = html_writer::link(new moodle_url('/mod/asistencias/view.php', $params),
                                      $info['cmname']);
     ...
     * @return string
     */
    protected function htmllize_tree(asistencias_files $tree, $dir) {
        global $CFG;
        $yuiconfig = array();
        $yuiconfig['type'] = 'html';
     ...
     * @param int $rowsperpage How many asistenciasments to render in a page
