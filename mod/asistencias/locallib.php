...
/**
 * This file contains the definition for the class asistenciasment
 *
 * This class provides all the functionality for the new asistencias module.
 *
...
define('ASISTENCIAS_SUBMISSION_STATUS_REOPENED', 'reopened');
define('ASISTENCIAS_SUBMISSION_STATUS_DRAFT', 'draft');
define('ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('ASISTENCIAS_FILTER_SUBMITTED', 'submitted');
define('ASISTENCIAS_FILTER_SINGLE_USER', 'singleuser');
define('ASISTENCIAS_FILTER_REQUIRE_GRADING', 'require_grading');

// Reopen attempt methods.
define('ASISTENCIAS_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('ASISTENCIAS_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('ASISTENCIAS_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('ASISTENCIAS_UNLIMITED_ATTEMPTS', -1);

require_once($CFG->libdir . '/accesslib.php');
require_once($CFG->libdir . '/formslib.php');
require_once($CFG->dirroot . '/repository/lib.php');
require_once($CFG->dirroot . '/mod/asistencias/mod_form.php');
require_once($CFG->libdir . '/gradelib.php');
require_once($CFG->dirroot . '/grade/grading/lib.php');
require_once($CFG->dirroot . '/mod/asistencias/feedbackplugin.php');
require_once($CFG->dirroot . '/mod/asistencias/submissionplugin.php');
require_once($CFG->dirroot . '/mod/asistencias/renderable.php');
require_once($CFG->dirroot . '/mod/asistencias/gradingtable.php');
require_once($CFG->libdir . '/eventslib.php');
require_once($CFG->libdir . '/portfolio/caller.php');

/**
 * Standard base class for mod_asistencias (asistenciasment types).
 *
 * @package   mod_asistencias
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class asistencias {
    ...
    /** @var stdClass the asistenciasment record that contains the global settings for this asistencias instance */
    private $instance;

    /** @var context the context of the course module for this asistencias instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this asistencias instance belongs to */
    private $course;

    /** @var stdClass the admin config for all asistencias instances  */
    private $adminconfig;

    /** @var asistencias_renderer the custom renderer for this module */
    private $output;

    /** @var stdClass the course module for this asistencias instance */
    ...
    /**
     * Constructor for the base asistencias class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('asistenciassubmission');
        $this->feedbackplugins = $this->load_plugins('asistenciasfeedback');
    }
    ...
    /**
     * Based on the current asistenciasment settings should we display the intro.
     *
    ...
    /**
     * Does an asistenciasment have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype asistenciassubmission | asistenciasfeedback
     * @param string $type
     * @return mixed asistencias_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('asistencias'));
     ...
     * @return mixed asistencias_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('asistenciasfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed asistencias_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('asistenciassubmission', $type);
    }
    ...
    protected function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('asistencias'));
                $pluginclass = 'asistencias_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof asistencias_plugin) {
    ...
    /**
     * Display the asistenciasment, used by view.php
     *
     * The asistenciasment is displayed differently depending on your role,
     * the settings for the asistenciasment and the status of the asistenciasment.
     *
    ...
    public function view($action='') {
        ...
        if ($action == 'savesubmission') {
            ...
            if (optional_param('saveandshownext', null, PARAM_RAW)) {
                ...
                $action = 'grade';
                if ($this->process_save_grade($mform)) {
                    $message = get_string('gradingchangessaved', 'asistencias');
        ...
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/asistencias/view.php', $nextpageparams);
        ...
        } else if ($action == 'viewpluginasistenciasfeedback') {
            $o .= $this->view_plugin_content('asistenciasfeedback');
        } else if ($action == 'viewpluginasistenciassubmission') {
            $o .= $this->view_plugin_content('asistenciassubmission');
     ...
     *             when upgrading an old asistenciasment to a new one (the plugins get called manually)
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->attemptreopenmethod = ASISTENCIAS_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('asistencias', $update);
        $this->instance = $DB->get_record('asistencias', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        $DB->update_record('asistencias', $update);

        return $returnid;
    }

    /**
     * Delete all grades from the gradebook for this asistenciasment.
     *
     * @return bool
     */
    protected function delete_grades() {
        ...
        $result = grade_update('mod/asistencias',
                               $this->get_course()->id,
                               'mod',
                               'asistencias',
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this asistenciasment.
        $fs = get_file_storage();
        ...
        $DB->delete_records('asistencias_submission', array('asistenciasment'=>$this->get_instance()->id));
        $DB->delete_records('asistencias_grades', array('asistenciasment'=>$this->get_instance()->id));
        $DB->delete_records('asistencias_plugin_config', array('asistenciasment'=>$this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('asistencias', array('id'=>$this->get_instance()->id));
     ...
     * asistenciasment submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'asistencias');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_asistencias_submissions)) {
            // Delete files associated with this asistenciasment.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'asistencias'),
            ...
            $asistenciasssql = 'SELECT a.id
                             FROM {asistencias} a
                           WHERE a.course=:course';
            $params = array('course'=>$data->courseid);

            $DB->delete_records_select('asistencias_submission', "asistenciasment IN ($asistenciasssql)", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('asistencias_grades', "asistenciasment IN ($asistenciasssql)", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/asistencias/lib.php');
                asistencias_reset_gradebook($data->courseid);
            }
        }
        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            shift_course_mod_dates('asistencias',
     ...
     *
     * @param asistencias_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(asistencias_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this asistenciasment.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this asistenciasment
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/asistencias/lib.php');
        $asistencias = clone $this->get_instance();
        $asistencias->cmidnumber = $coursemoduleid;
        $param = null;
        if ($reset) {
            $param = 'reset';
        }

        return asistencias_grade_item_update($asistencias, $param);
    }
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('asistencias');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this asistenciasment.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            ...
            $params = array('modulename'=>'asistencias', 'instance'=>$instance->id);
            ...
            if ($event->id) {
                ...
                $event->description = format_module_intro('asistencias', $instance, $coursemoduleid);
            ...
            $DB->delete_records('event', array('modulename'=>'asistencias', 'instance'=>$instance->id));
        }
    }
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('asistencias', $update);
        $this->instance = $DB->get_record('asistencias', array('id'=>$update->id), '*', MUST_EXIST);

        // Load the asistenciasment so the plugins have access to it.
     ...
     *
     * @param asistencias_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(asistencias_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
        if ($plugin->is_visible()) {
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'asistencias'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'asistencias'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);

        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'asistencias'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'asistencias'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    protected function get_module_name() {
        ...
        self::$modulename = get_string('modulename', 'asistencias');
        return self::$modulename;
    }
    ...
    protected function get_module_name_plural() {
        ...
        self::$modulenameplural = get_string('modulenameplural', 'asistencias');
        return self::$modulenameplural;
    }

    /**
     * Has this asistenciasment been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this asistenciasment
     *
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('asistencias', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
            throw new coding_exception('Improper use of the asistenciasment class. ' .
                                       'Cannot load the asistenciasment record.');
        }
        return $this->instance;
    }
    ...
    public function get_course_module() {
        ...
        if ($this->context->contextlevel == CONTEXT_MODULE) {
            $this->coursemodule = get_coursemodule_from_id('asistencias',
        ...
        if ($this->get_instance()->grade >= 0) {
            ...
            if ($editing && $this->get_instance()->grade > 0) {
                ...
                $o .= '<label class="accesshide" for="quickgrade_' . $userid . '">' .
                       get_string('usergrade', 'asistencias') .
                       '</label>';
                $o .= '<input type="text"
    ...
    public function list_participants($currentgroup, $idsonly) {
        if ($idsonly) {
            return get_enrolled_users($this->context, 'mod/asistencias:submit', $currentgroup, 'u.id');
        } else {
            return get_enrolled_users($this->context, 'mod/asistencias:submit', $currentgroup);
        }
    }

    /**
     * Load a count of valid teams for this asistenciasment.
     *
    ...
    public function count_participants($currentgroup) {
        return count_enrolled_users($this->context, 'mod/asistencias:submit', $currentgroup);
    }
    ...
    public function count_submissions_need_grading() {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group asistenciasment because the submission is shared.
            return 0;
        }

        $currentgroup = groups_get_activity_group($this->get_course_module(), true);
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/asistencias:submit', $currentgroup, false);

        $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                 FROM {asistencias_submission} mxs
                                 WHERE mxs.asistenciasment = :asistenciasid2 GROUP BY mxs.userid';
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {asistencias_grades} mxg
                            WHERE mxg.asistenciasment = :asistenciasid3 GROUP BY mxg.userid';

        $params['asistenciasid'] = $this->get_instance()->id;
        $params['asistenciasid2'] = $this->get_instance()->id;
        $params['asistenciasid3'] = $this->get_instance()->id;
        $params['submitted'] = ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED;

        $sql = 'SELECT COUNT(s.userid)
                   FROM {asistencias_submission} s
                   LEFT JOIN ( ' . $submissionmaxattempt . ' ) smx ON s.userid = smx.userid
                   LEFT JOIN ( ' . $grademaxattempt . ' ) gmx ON s.userid = gmx.userid
                   LEFT JOIN {asistencias_grades} g ON
                        s.asistenciasment = g.asistenciasment AND
                        s.userid = g.userid AND
                        g.attemptnumber = gmx.maxattempt
                   JOIN(' . $esql . ') e ON e.id = s.userid
                   WHERE
                        s.attemptnumber = smx.maxattempt AND
                        s.asistenciasment = :asistenciasid AND
    ...
    public function count_grades() {
        ...
        $sql = 'SELECT COUNT(g.userid)
                   FROM {asistencias_grades} g
                   JOIN(' . $esql . ') e ON e.id = g.userid
                   WHERE g.asistenciasment = :asistenciasid';
    ...
    public function count_submissions_with_status($status) {
        ...
        if ($this->get_instance()->teamsubmission) {
            $maxattemptsql = 'SELECT mxs.groupid, MAX(mxs.attemptnumber) AS maxattempt
                              FROM {asistencias_submission} mxs
                              WHERE mxs.asistenciasment = :asistenciasid2 GROUP BY mxs.groupid';
    ...
    protected function get_grading_userid_list() {
        $filter = get_user_preferences('asistencias_filter', '');
        $table = new asistencias_grading_table($this, 0, $filter, 0, false);
    ...
    protected function pack_files($filesforzipping) {
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'asistenciasment_');
        // Zip files.
    ...
    /**
     * Finds all asistenciasment notifications that have yet to be mailed out, and mails them.
     *
    ...
    public static function cron() {
        ...
        $sql = 'SELECT a.course, a.name, a.blindmarking, a.revealidentities,
                 ...
                 FROM {asistencias} a
                 JOIN {asistencias_grades} g ON g.asistenciasment = a.id
                 LEFT JOIN {asistencias_user_flags} uf ON uf.asistenciasment = a.id AND uf.userid = g.userid
        ...
        mtrace('Processing ' . count($submissions) . ' asistenciasment submissions ...');
        ...
        foreach ($submissions as $submission) {
            ...
            mtrace("Processing asistenciasment submission $submission->id ...");
            ...
            if (!array_key_exists($submission->asistenciasment, $modcache)) {
                $mod = get_coursemodule_from_instance('asistencias', $submission->asistenciasment, $course->id);
                if (empty($mod)) {
                    mtrace('Could not find course module for asistenciasment id ' . $submission->asistenciasment);
                    continue;
                }
                $modcache[$submission->asistenciasment] = $mod;
            } else {
                $mod = $modcache[$submission->asistenciasment];
            }
            // Context lookups are already cached.
            $contextmodule = context_module::instance($mod->id);

            if (!$mod->visible) {
                // Hold mail notification for hidden asistenciasments until later.
                continue;
            }

            // Need to send this to the student.
            $messagetype = 'feedbackavailable';
            $eventtype = 'asistencias_notification';
            $updatetime = $submission->lastmodified;
            $modulename = get_string('modulename', 'asistencias');

            $uniqueid = 0;
            if ($submission->blindmarking && !$submission->revealidentities) {
                $uniqueid = self::get_uniqueid_for_user_static($submission->asistenciasment, $user->id);
            }
            $showusers = $submission->blindmarking && !$submission->revealidentities;
            self::send_asistenciasment_notification($grader,
                                               $user,
                                               $messagetype,
                                               $eventtype,
                                               $updatetime,
            ...
            $flags = $DB->get_record('asistencias_user_flags', array('userid'=>$user->id, 'asistenciasment'=>$submission->asistenciasment));
            if ($flags) {
                $flags->mailed = 1;
                $DB->update_record('asistencias_user_flags', $flags);
            } else {
                $flags = new stdClass();
                $flags->userid = $user->id;
                $flags->asistenciasment = $submission->asistenciasment;
                $flags->mailed = 1;
        ...
        mtrace('Done processing ' . count($submissions) . ' asistenciasment submissions');
    ...
    /**
     * Update user flags for this user in this asistenciasment.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->asistenciasment <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('asistencias_user_flags', $flags);
        return $result;
    }

    /**
     * Update a grade in the grade table for the asistenciasment and in the gradebook.
     *
     * @param stdClass $grade a grade record keyed on id
     * @return bool true for success
     */
    public function update_grade($grade) {
        ...
        $result = $DB->update_record('asistencias_grades', $grade);
    ...
    protected function view_grant_extension($mform) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/mod/asistencias/extensionform.php');
        ...
        $header = new asistencias_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'asistencias'));
        ...
        if (!$mform) {
            $formparams = array($this->get_course_module()->id,
                                ...
                                $this->get_instance(),
                                $data);
            $mform = new mod_asistencias_extension_form(null, $formparams);
        }
        $o .= $this->get_renderer()->render(new asistencias_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the asistenciasment.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
    ...
    public function get_submission_group_members_who_have_not_submitted($groupid, $onlyids) {
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED) {
                unset($members[$id]);
            } else {
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'asistencias') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('asistenciasment'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        $submission = null;
        $submissions = $DB->get_records('asistencias_submission', $params, 'attemptnumber DESC', '*', 0, 1);
        if ($submissions) {
        ...
        if ($create) {
            ...
            $submission->asistenciasment = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            $submission->status = ASISTENCIAS_SUBMISSION_STATUS_DRAFT;
    ...
    /**
     * View a summary listing of all asistenciasments in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        $strplural = get_string('modulenameplural', 'asistencias');

        if (!$cms = get_coursemodules_in_course('asistencias', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new asistencias_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['asistencias'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            $asistenciasment = new asistencias($context, $cm, $course);

            if (has_capability('mod/asistencias:grade', $context)) {
                $submitted = $asistenciasment->count_submissions_with_status(ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/asistencias:submit', $context)) {
                $usersubmission = $asistenciasment->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'asistencias');
                } else {
                    $submitted = get_string('submissionstatus_', 'asistencias');
                }
            }
            $grading_info = grade_get_grades($course->id, 'mod', 'asistencias', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_asistencias_info($cm->id, $cm->name, $sectionname, $timedue, $submitted, $grade);
    ...
    /**
     * This is used for team asistenciasments to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_TEXT);
        $item = null;
        if ($pluginsubtype == 'asistenciassubmission') {
            ...
            if ($item->userid != $USER->id) {
                require_capability('mod/asistencias:grade', $this->context);
            }
            $o .= $this->get_renderer()->render(new asistencias_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new asistencias_submission_plugin_submission($plugin,
                                                              $item,
                                                              asistencias_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            $logmessage = get_string('viewsubmissionforuser', 'asistencias', $item->userid);
            ...
            $o .= $this->get_renderer()->render(new asistencias_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new asistencias_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              asistencias_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));
            $logmessage = get_string('viewfeedbackforuser', 'asistencias', $item->userid);
     ...
     * @param asistencias_plugin $plugin - The asistenciasment plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        ...
        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'asistencias');
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component) {
        ...
        if ($CFG->enableportfolios) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('asistencias_portfolio_caller', $portfolioparams, 'mod_asistencias');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new asistencias_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'asistencias')));
        $gradingresult = new asistencias_gradingmessage(get_string('savegradingresult', 'asistencias'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new asistencias_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'asistencias')));
        $gradingresult = new asistencias_gradingmessage(get_string('quickgradingresult', 'asistencias'),
                                                   $message,
                                                   $this->get_course_module()->id);
        $o .= $this->get_renderer()->render($gradingresult);
        $o .= $this->view_footer();
    ...
    /**
     * Does this user have grade permission for this asistenciasment?
     *
     * @return bool
     */
    protected function can_grade() {
        // Permissions check.
        if (!has_capability('mod/asistencias:grade', $this->context)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all asistenciasment submissions.
     *
    ...
    protected function download_submissions() {
        ...
        $students = get_enrolled_users($this->context, "mod/asistencias:submit");
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->get_instance()->teamsubmission) {
                    ...
                    if ($submissiongroup) {
                        ...
                        $groupname = get_string('defaultteam', 'asistencias') . '-';
                    }
                } else {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'asistencias'));
        ...
        if (count($filesforzipping) == 0) {
            $header = new asistencias_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'asistencias'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'asistencias'));
            $url = new moodle_url('/mod/asistencias/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            $this->add_to_log('download all submissions', get_string('downloadall', 'asistencias'));
     ...
     * @param string $url The url to the asistencias module instance.
     * @return void
     */
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_asistencias');
        return $this->output;
    }
     ...
     *
     * For team asistenciasments there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('asistenciasment'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('asistenciasment'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('asistencias_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('asistenciasment'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('asistencias_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->asistenciasment = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('asistencias_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('asistencias_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->asistenciasment   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('asistencias_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('asistenciasment'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('asistencias_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grade_page($mform) {
        ...
        require_once($CFG->dirroot . '/mod/asistencias/gradeform.php');

        // Need submit permission to submit an asistenciasment.
        require_capability('mod/asistencias:grade', $this->context);

        $header = new asistencias_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'asistencias'));
        $o .= $this->get_renderer()->render($header);
        ...
        $cache = cache::make_from_params(cache_store::MODE_SESSION, 'mod_asistencias', 'useridlist');
        if (!$userid) {
        ...
        if ($user) {
            ...
            $usersummary = new asistencias_user_summary($user,
                                                   $this->get_course()->id,
                                                   $viewfullnames,
        ...
        if ($this->can_view_submission($userid)) {
            ...
            if ($teamsubmission) {
                $showsubmit = $showedit &&
                              $teamsubmission &&
                              ($teamsubmission->status == ASISTENCIAS_SUBMISSION_STATUS_DRAFT);
            } else {
                $showsubmit = $showedit &&
                              $submission &&
                              ($submission->status == ASISTENCIAS_SUBMISSION_STATUS_DRAFT);
            ...
            $submissionstatus = new asistencias_submission_status($instance->allowsubmissionsfromdate,
                                                             ...
                                                             $this->get_course_module()->id,
                                                             $this->get_course()->id,
                                                             asistencias_submission_status::GRADER_VIEW,
                                                             $showedit,
                                                             $showsubmit,
                                                             $viewfullnames,
                                                             $extensionduedate,
        ...
        if ($attemptnumber != -1) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'asistencias', $params);
        ...
        if (!$mform) {
            ...
            $mform = new mod_asistencias_grade_form(null,
                                               $formparams,
                                               'post',
                                               '',
        ...
        $o .= $this->get_renderer()->render(new asistencias_form('gradingform', $mform));

        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new asistencias_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    protected function view_reveal_identities_confirm() {
        ...
        require_capability('mod/asistencias:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/asistencias/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/asistencias/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'asistencias'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();
        $this->add_to_log('view', get_string('viewrevealidentitiesconfirm', 'asistencias'));
        return $o;
    }
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/asistencias/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this asistenciasment.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/asistencias/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/asistencias/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/asistencias/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'asistencias');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/asistencias/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'asistencias');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/asistencias:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/asistencias/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'asistencias');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/asistencias/view.php' .
                           '?id=' .  $cmid .
                           '&plugin=' . $plugin->get_type() .
                           '&pluginsubtype=asistenciasfeedback' .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'asistencias'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_asistencias', 'submissions');

        $perpage = get_user_preferences('asistencias_perpage', 10);
        $filter = get_user_preferences('asistencias_filter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller);
        $quickgrading = get_user_preferences('asistencias_quickgrading', false);
        ...
        $gradingoptionsform = new mod_asistencias_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_asistencias_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new asistencias_grading_table($this, $perpage, $filter, 0, true);
            $table = $this->get_renderer()->render($gradingtable);
            $quickformparams = array('cm'=>$this->get_course_module()->id, 'gradingtable'=>$table);
            $quickgradingform = new mod_asistencias_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new asistencias_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new asistencias_grading_table($this, $perpage, $filter, 0, false);
        ...
        if (count($users) != 0) {
            ...
            $asistenciasform = new asistencias_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($asistenciasform);
        }
        $asistenciasform = new asistencias_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_asistencias.init_grading_options');
        $o .= $this->get_renderer()->render($asistenciasform);
        return $o;
    }
    ...
    protected function view_grading_page() {
        ...
        $logmessage = get_string('viewsubmissiongradingtable', 'asistencias');
    ...
    /**
     * Message for students when asistenciasment submissions have been closed.
     *
     * @return string
     */
    protected function view_student_error_message() {
        ...
        $o = '';
        // Need submit permission to submit an asistenciasment.
        require_capability('mod/asistencias:submit', $this->context);

        $header = new asistencias_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('editsubmission', 'asistencias'));
        $o .= $this->get_renderer()->render($header);

        $o .= $this->get_renderer()->notification(get_string('submissionsclosed', 'asistencias'));
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/asistencias/submission_form.php');
        // Need submit permission to submit an asistenciasment.
        require_capability('mod/asistencias:submit', $this->context);
        ...
        if (!$mform) {
            $mform = new mod_asistencias_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new asistencias_form('editsubmissionform', $mform));

        $o .= $this->view_footer();
        $this->add_to_log('view submit asistenciasment form', get_string('viewownsubmissionform', 'asistencias'));

        return $o;
    }

    /**
     * See if this asistenciasment has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/asistencias:submit', $this->context)) {
            return true;
        }
        if (has_capability('mod/asistencias:grade', $this->context)) {
            return true;
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_asistencias_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/asistencias/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_asistencias_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $submitforgradingpage = new asistencias_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        $logmessage = get_string('viewownsubmissionform', 'asistencias');
        $this->add_to_log('view confirm submit asistenciasment form', $logmessage);
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            $showedit = has_capability('mod/asistencias:submit', $this->context) &&
            ...
            $gradingmanager = get_grading_manager($this->context, 'mod_asistencias', 'submissions');
            ...
            if ($teamsubmission && ($teamsubmission->status != ASISTENCIAS_SUBMISSION_STATUS_DRAFT)) {
                $showsubmit = false;
            }
            if ($submission && ($submission->status != ASISTENCIAS_SUBMISSION_STATUS_DRAFT)) {
                $showsubmit = false;
            }
            if (!$this->get_instance()->submissiondrafts) {
                $showsubmit = false;
            ...
            $submissionstatus = new asistencias_submission_status($instance->allowsubmissionsfromdate,
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $this->get_course()->id,
                                                              asistencias_submission_status::STUDENT_VIEW,
                                                              $showedit,
                                                              $showsubmit,
                                                              $viewfullnames,
                                                              $extensionduedate,
            ...
            $cangrade = has_capability('mod/asistencias:grade', $this->get_context());
            ...
            if ((!empty($gradebookgrade->grade) && ($cangrade || !$gradebookgrade->hidden)) ||
                ...
                $feedbackstatus = new asistencias_feedback_status($gradefordisplay,
                                                      $gradeddate,
                                                      $grader,
                                                      $this->get_feedback_plugins(),
                                                      $grade,
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('asistencias_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('asistencias_submission', $params, 'attemptnumber ASC');
    ...
    protected function view_submission_page() {
        ...
        $o = '';
        $o .= $this->get_renderer()->render(new asistencias_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id));

        if ($this->can_grade()) {
            $draft = ASISTENCIAS_SUBMISSION_STATUS_DRAFT;
            $submitted = ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED;
            if ($instance->teamsubmission) {
                $summary = new asistencias_grading_summary($this->count_teams(),
                ...
                $summary = new asistencias_grading_summary($this->count_participants(0),
                                                      $instance->submissiondrafts,
                                                      $this->count_submissions_with_status($draft),
                                                      $this->is_any_submission_plugin_enabled(),
                                                      $this->count_submissions_with_status($submitted),
        ...
        $o .= $this->view_footer();
        $this->add_to_log('view', get_string('viewownsubmissionstatus', 'asistencias'));
        return $o;
    }
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        $asistencias = clone $this->get_instance();
        $asistencias->cmidnumber = $this->get_course_module()->id;

        return asistencias_grade_item_update($asistencias, $gradebookgrade);
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this asistenciasment qualifies as submitted or draft.
        $team = $this->get_submission_group_members($submission->groupid, true);

        $allsubmitted = true;
        $anysubmitted = false;
        $result = true;
        if ($submission->status != ASISTENCIAS_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                $membersubmission = $this->get_user_submission($member->id, false, $submission->attemptnumber);

                if (!$membersubmission || $membersubmission->status != ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED) {
                    $allsubmitted = false;
                    if ($anysubmitted) {
                        break;
                    }
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('asistencias_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = ASISTENCIAS_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('asistencias_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('asistencias_submission', $submission) && $result;
        }
    ...
    /**
     * Is this asistenciasment open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the asistenciasment locked?
     *
    ...
    public function submissions_open($userid = 0) {
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the asistenciasment.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        $files = $fs->get_area_files($this->get_context()->id,
                                     ...
                                     'timemodified',
                                     false);
        return $this->get_renderer()->asistencias_files($this->context, $submissionid, $area, $component);
    ...
    protected function get_graders($userid) {
        $potentialgraders = get_enrolled_users($this->context, 'mod/asistencias:grade');
     ...
     * @param string $asistenciasmentname
     */
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext  = format_string($course->shortname, true, $formatparams) .
                     ...
                     $modulename .
                     ' -> ' .
                     format_string($asistenciasmentname, true, $formatparams) . "\n";
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'asistencias', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/asistencias/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/asistencias/view.php?id=' . $coursemodule->id . '">' .
                     format_string($asistenciasmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'asistencias', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_asistenciasment_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        $info = new stdClass();
        if ($blindmarking) {
            $info->username = get_string('participant', 'asistencias') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'asistencias');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->asistenciasment = format_string($asistenciasmentname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/asistencias/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'asistencias', $info);
        ...
        $eventdata->modulename       = 'asistencias';
        ...
        $eventdata->component       = 'mod_asistencias';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->asistenciasment;
    ...
    public function send_notification($userfrom,
                                      ...
                                      $eventtype,
                                      $updatetime) {
        self::send_asistenciasment_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    protected function process_submit_for_grading($mform) {
        ...
        if (!$mform->is_cancelled()) {
            ...
            if ($submission->status != ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED) {
    ...
    protected function process_save_extension(& $mform) {
        ...
        // Need submit permission to submit an asistenciasment.
        require_capability('mod/asistencias:grantextension', $this->context);
        ...
        $mform = new mod_asistencias_extension_form(null, array($this->get_course_module()->id,
                                                           $userid,
                                                           $batchusers,
                                                           $this->get_instance(),
                                                           null));
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'asistencias');
        }
        ...
        $params['asistenciasid1'] = $this->get_instance()->id;
        $params['asistenciasid2'] = $this->get_instance()->id;

        // Check them all for currency.
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {asistencias_grades} mxg
                            WHERE mxg.asistenciasment = :asistenciasid1 GROUP BY mxg.userid';
        ...
        $sql = 'SELECT u.id as userid, g.grade as grade, g.timemodified as lastmodified
                ...
                LEFT JOIN {asistencias_grades} g ON
                    u.id = g.userid AND
                    g.asistenciasment = :asistenciasid2 AND
                    g.attemptnumber = gmx.maxattempt
                WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'asistencias');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('asistenciasfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/asistencias',
                                          $this->course->id,
                                          'mod',
                                          'asistencias',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'asistencias');
    }
    ...
    protected function process_reveal_identities() {
        ...
        require_capability('mod/asistencias:revealidentities', $this->context);
        if (!confirm_sesskey()) {
            return false;
        }

        // Update the asistenciasment record.
        $update = new stdClass();
        $update->id = $this->get_instance()->id;
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $grades = $DB->get_records('asistencias_grades', array('asistenciasment'=>$this->get_instance()->id));
        ...
        $this->add_to_log('reveal identities', get_string('revealidentities', 'asistencias'));
    }
    ...
    protected function process_save_grading_options() {
        ...
        $mform = new mod_asistencias_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('asistencias_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('asistencias_filter', $formdata->filter);
            }
            if ($showquickgrading) {
                set_user_preference('asistencias_quickgrading', isset($formdata->quickgrading));
            }
        }
    }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'asistencias', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'asistencias');
        }
        return $info;
    }
    ...
    protected function format_submission_for_log(stdClass $submission) {
        $info = '';
        $info .= get_string('submissionstatus', 'asistencias') .
                 ': ' .
                 get_string('submissionstatus_' . $submission->status, 'asistencias') .
                 '. <br>';
    ...
    /**
     * Copy the current asistenciasment submission from the last submitted attempt.
     *
    ...
    protected function process_copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != ASISTENCIAS_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'asistencias');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'asistencias');
            return false;
        }
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted asistenciasments
    ...
    /**
     * Save asistenciasment submission.
     *
     * @param  moodleform $mform
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        if ($data = $mform->get_data()) {
            ...
            // Get the flags to check if it is locked.
            if ($flags && $flags->locked) {
                print_error('submissionslocked', 'asistencias');
                return true;
            }
            ...
            if ($pluginerror || $allempty) {
                if ($allempty) {
                    $notices[] = get_string('submissionempty', 'mod_asistencias');
                }
                return false;
            }
     ...
     * This is specific to the asistenciasment, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'asistencias', $this->get_instance()->grade);
                $gradingelement = $mform->addElement('text', 'grade', $name);
                $mform->addHelpButton('grade', 'gradeoutofhelp', 'asistencias');
        ...
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'asistencias'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'asistencias', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'asistencias'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            $mform->addElement('header', 'groupsubmissionsettings', get_string('groupsubmissionsettings', 'asistencias'));
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'asistencias'));
            $mform->setDefault('applytoall', 1);
        }

        // Do not show if we are editing a previous attempt.
        if ($attemptnumber == -1 && $this->get_instance()->attemptreopenmethod != ASISTENCIAS_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'asistencias'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'asistencias');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'asistencias'), $attemptreopenmethod);
            ...
            if ($maxattempts == ASISTENCIAS_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'asistencias');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'asistencias'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'asistencias'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == ASISTENCIAS_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == ASISTENCIAS_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'asistencias'));
        ...
        $buttonarray=array();
        $name = get_string('savechanges', 'asistencias');
        $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
        if (!$last) {
            $name = get_string('savenext', 'asistencias');
        ...
        if ($rownum > 0) {
            $name = get_string('previous', 'asistencias');
            $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
        }

        if (!$last) {
            $name = get_string('nosavebutnext', 'asistencias');
    ...
    protected function process_save_grade(&$mform) {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            $maxattemptsreached = !empty($submission) &&
                                  ...
                                  $instance->maxattempts != ASISTENCIAS_UNLIMITED_ATTEMPTS;
            $shouldreopen = false;
            if ($instance->attemptreopenmethod == ASISTENCIAS_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
            ...
            if ($instance->attemptreopenmethod == ASISTENCIAS_ATTEMPT_REOPEN_METHOD_MANUAL &&
     ...
     * @return bool
     */
    public static function can_upgrade_asistenciasment($type, $version) {
        $asistenciasment = new asistencias(null, null, null);
        return $asistenciasment->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an asistenciasment from the 2.2 module.
     *
     * @param string $type The plugin type
     * @param int $version The plugin version
     * @return bool
    ...
    /**
     * Copy all the files from the old asistenciasment files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old asistenciasment context id
     * @param int $oldcomponent The old asistenciasment component ('asistenciasment')
     * @param int $oldfilearea The old asistenciasment filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new asistenciasment context id
     * @param int $newcomponent The new asistenciasment component ('asistenciasment')
     * @param int $newfilearea The new asistenciasment filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group asistenciasment
     * at most 1 time.
     *
    ...
    protected function process_add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == ASISTENCIAS_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != ASISTENCIAS_UNLIMITED_ATTEMPTS &&
        ...
        $submission->status = ASISTENCIAS_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        global $DB, $CFG;
        $grades = array();
        $asistenciasmentid = $this->get_instance()->id;
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('asistenciasfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {asistencias_grades} mxg
                            WHERE mxg.asistenciasment = :asistenciasid2 GROUP BY mxg.userid';

        // When the gradebook asks us for grades - only return the last attempt for each user.
        $params = array('asistenciasid1'=>$asistenciasmentid,
                        'asistenciasid2'=>$asistenciasmentid,
                        'asistenciasid3'=>$asistenciasmentid,
                        'asistenciasid4'=>$asistenciasmentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                LEFT JOIN ( ' . $grademaxattempt . ' ) gmx ON u.id = gmx.userid
                                                LEFT JOIN {asistencias_submission} s
                                                    ON u.id = s.userid and s.asistenciasment = :asistenciasid3 AND
                                                    s.attemptnumber = smx.maxattempt
                                                JOIN {asistencias_grades} g
                                                    ON u.id = g.userid and g.asistenciasment = :asistenciasid4 AND
                                                    g.attemptnumber = gmx.maxattempt' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - asistencias them a random id.
     *
     * @param int $asistenciasid The asistenciasid to lookup
     */
    public static function allocate_unique_ids($asistenciasid) {
        global $DB;

        $cm = get_coursemodule_from_instance('asistencias', $asistenciasid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/asistencias:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        $record = new stdClass();
        $record->asistenciasment = $asistenciasid;
        foreach ($users as $user) {
            $record = $DB->get_record('asistencias_user_mapping',
                                      array('asistenciasment'=>$asistenciasid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->userid = $user->id;
                $DB->insert_record('asistencias_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this asistenciasment.
     *
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($asistenciasid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('asistenciasment'=>$asistenciasid, 'userid'=>$userid);
        if ($record = $DB->get_record('asistencias_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($asistenciasid);
        ...
        return $DB->insert_record('asistencias_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this asistenciasment.
     *
     * @param int $asistenciasid The id of the asistenciasment this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($asistenciasid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('asistencias_user_mapping',
                                      array('asistenciasment'=>$asistenciasid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
...
/**
 * Portfolio caller class for mod_asistencias.
 *
 * @package   mod_asistencias
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the asistenciasment we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('asistencias', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the asistenciasment type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_asistencias');
            }
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'asistenciasment.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'asistencias' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return asistencias_submission_plugin
     */
    protected function get_submission_plugin() {
        ...
        $asistenciasment = new asistencias($context, null, null);
        return $asistenciasment->get_submission_plugin_by_type($this->plugin);
    }
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/asistencias:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'asistencias');
    }
