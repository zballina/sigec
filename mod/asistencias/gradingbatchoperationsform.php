 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_asistencias_grading_batch_operations_form extends moodleform {
    /**
    ...
    public function definition() {
        ...
        // Visible elements.
        $options = array();
        $options['lock'] = get_string('locksubmissions', 'asistencias');
        $options['unlock'] = get_string('unlocksubmissions', 'asistencias');
        if ($instance['submissiondrafts']) {
            $options['reverttodraft'] = get_string('reverttodraft', 'asistencias');
        }
        if ($instance['duedate']) {
            $options['grantextension'] = get_string('grantextension', 'asistencias');
        }
        if ($instance['attemptreopenmethod'] == ASISTENCIAS_ATTEMPT_REOPEN_METHOD_MANUAL) {
            $options['addattempt'] = get_string('addattempt', 'asistencias');
        }
        ...
        $objs = array();
        $objs[] =& $mform->createElement('select', 'operation', get_string('chooseoperation', 'asistencias'), $options);
        $objs[] =& $mform->createElement('submit', 'submit', get_string('go'));
        $batchdescription = get_string('batchoperationsdescription', 'asistencias');
