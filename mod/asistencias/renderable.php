...
/**
 * This file contains the definition for the renderable classes for the asistenciasment
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_submit_for_grading_page implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_gradingmessage implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_form implements renderable {
    /** @var moodleform $form is the edit submission form */
    public $form = null;
    /** @var string $classname is the name of the class to asistencias to the container */
    public $classname = '';
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_user_summary implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_feedback_plugin_feedback implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;

    /** @var asistencias_submission_plugin $plugin */
    public $plugin = null;
    /** @var stdClass $grade */
    public $grade = null;
    /** @var string $view */
     ...
     *
     * @param asistencias_feedback_plugin $plugin
     ...
     */
    public function __construct(asistencias_feedback_plugin $plugin,
                                stdClass $grade,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_submission_plugin_submission implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;
    ...
    /**
     * Constructor
     * @param asistencias_submission_plugin $plugin
     ...
     */
    public function __construct(asistencias_submission_plugin $plugin,
                                stdClass $submission,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_feedback_status implements renderable {
    ...
    /** @var stdClass asistencias_grade record */
    public $grade = null;
    /** @var int coursemoduleid */
    public $coursemoduleid = 0;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_submission_status implements renderable {
...
/**
 * Used to output the attempt history for a particular asistenciasment.
 *
 * @package mod_asistencias
 * @copyright 2012 Davo Smith, Synergy Learning
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_attempt_history implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_header implements renderable {
    /** @var stdClass the asistencias record  */
    public $asistencias = null;
     ...
     * Constructor
     *
     * @param stdClass $asistencias  - the asistencias database record
     ...
     */
    public function __construct(stdClass $asistencias,
                                $context,
                                $showintro,
                                $coursemoduleid,
                                $subpage='',
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_grading_summary implements renderable {
    /** @var int participantcount - The number of users who can submit to this asistenciasment */
    ...
    /** @var int duedate - The asistenciasment due date (if one is set) */
    public $duedate = 0;
    /** @var int cutoffdate - The asistenciasment cut off date (if one is set) */
    public $cutoffdate = 0;
    /** @var int coursemoduleid - The asistenciasment course module id */
    public $coursemoduleid = 0;
    /** @var boolean teamsubmission - Are team submissions enabled for this asistenciasment */
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_course_index_summary implements renderable {
    /** @var array asistenciasments - A list of course module info and submission counts or statuses */
    public $asistenciasments = array();
     ...
     * @param int $timedue - The due date for the asistenciasment - may be 0 if no duedate
     * @param string $submissioninfo - A string with either the number of submitted asistenciasments, or the
     ...
     */
    public function add_asistencias_info($cmid, $cmname, $sectionname, $timedue, $submissioninfo, $gradeinfo) {
        $this->asistenciasments[] = array('cmid'=>$cmid,
                               'cmname'=>$cmname,
...
/**
 * An asistencias file class that extends rendererable class and is used by the asistencias module.
 *
 * @package   mod_asistencias
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_files implements renderable {
    /** @var context $context */
    public $context;
    /** @var string $context */
    public $dir;
    ...
    public function __construct(context $context, $sid, $filearea, $component) {
        ...
        if (!empty($CFG->enableportfolios)) {
            ...
            if (count($files) >= 1 &&
                    has_capability('mod/asistencias:exportownsubmission', $this->context)) {
                $button = new portfolio_add_button();
                $callbackparams = array('cmid' => $this->cm->id,
                                        'sid' => $sid,
                                        'area' => $filearea,
                                        'component' => $component);
                $button->set_callback_options('asistencias_portfolio_caller',
                                              $callbackparams,
                                              'mod_asistencias');
        ...
        if (!empty($CFG->enableplagiarism)) {
            ...
            $asistenciasment = new asistencias($this->context, null, null);
            foreach ($files as $file) {
                ...
                $linkparams = array('userid' => $sid,
                                    ...
                                    'cmid' => $this->cm->id,
                                    'course' => $this->course,
                                    'asistenciasment' => $asistenciasment->get_instance());
    ...
    public function preprocess($dir, $filearea, $component) {
        ...
        foreach ($dir['files'] as $file) {
            ...
            if (!empty($CFG->enableportfolios)) {
                $button = new portfolio_add_button();
                if (has_capability('mod/asistencias:exportownsubmission', $this->context)) {
