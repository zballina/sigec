...
/**
 * Extends table_sql to provide a table of asistenciasment submissions
 *
 * @package   mod_asistencias
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_grading_table extends table_sql implements renderable {
    /** @var asistencias $asistenciasment */
    private $asistenciasment = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the asistenciasment class that is displaying this table
     *
     * @param asistencias $asistenciasment The asistenciasment class
     ...
     */
    public function __construct(asistencias $asistenciasment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        global $CFG, $PAGE, $DB;
        parent::__construct('mod_asistencias_grading');
        $this->asistenciasment = $asistenciasment;
        ...
        $urlparams = array('action'=>'grading', 'id'=>$asistenciasment->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/asistencias/view.php', $urlparams);
        $this->define_baseurl($url);

        // Do some business - then set the sql.
        $currentgroup = groups_get_activity_group($asistenciasment->get_course_module(), true);

        if ($rowoffset) {
            $this->rownum = $rowoffset - 1;
        }

        $users = array_keys( $asistenciasment->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['asistenciasmentid1'] = (int)$this->asistenciasment->get_instance()->id;
        $params['asistenciasmentid2'] = (int)$this->asistenciasment->get_instance()->id;
        $params['asistenciasmentid3'] = (int)$this->asistenciasment->get_instance()->id;
        $params['asistenciasmentid4'] = (int)$this->asistenciasment->get_instance()->id;
        $params['asistenciasmentid5'] = (int)$this->asistenciasment->get_instance()->id;

        $extrauserfields = get_extra_user_fields($this->asistenciasment->get_context());
        ...
        $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                 ...
                                 WHERE mxs.asistenciasment = :asistenciasmentid4 GROUP BY mxs.userid';
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {asistencias_grades} mxg
                            WHERE mxg.asistenciasment = :asistenciasmentid5 GROUP BY mxg.userid';
        $from = '{user} u
                         ...
                         LEFT JOIN {asistencias_submission} s ON
                            u.id = s.userid AND
                            s.asistenciasment = :asistenciasmentid1 AND
                            s.attemptnumber = smx.maxattempt
                         LEFT JOIN {asistencias_grades} g ON
                            u.id = g.userid AND
                            g.asistenciasment = :asistenciasmentid2 AND
                            g.attemptnumber = gmx.maxattempt
                         LEFT JOIN {asistencias_user_flags} uf ON u.id = uf.userid AND uf.asistenciasment = :asistenciasmentid3';
        ...
        if ($this->asistenciasment->is_any_submission_plugin_enabled()) {
            if ($filter == ASISTENCIAS_FILTER_SUBMITTED) {
                $where .= ' AND s.timecreated > 0 ';
            }
            if ($filter == ASISTENCIAS_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified > g.timemodified OR g.timemodified IS NULL))';
                $params['submitted'] = ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED;
            }
            if (strpos($filter, ASISTENCIAS_FILTER_SINGLE_USER) === 0) {
        ...
        // User picture.
        if (!$this->asistenciasment->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'asistencias');
            }
        ...
        // Submission status.
        if ($asistenciasment->is_any_submission_plugin_enabled()) {
            $columns[] = 'status';
            $headers[] = get_string('status');
        }

        // Team submission columns.
        if ($asistenciasment->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'asistencias');

            $columns[] = 'teamstatus';
            $headers[] = get_string('teamsubmissionstatus', 'asistencias');
        }
        ...
        if ($this->is_downloading()) {
            if ($this->asistenciasment->get_instance()->grade >= 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'asistencias');
            } else {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'asistencias');
            }
        }
        if (!$this->is_downloading()) {
        ...
        if ($asistenciasment->is_any_submission_plugin_enabled()) {
            ...
            $headers[] = get_string('lastmodifiedsubmission', 'asistencias');

            foreach ($this->asistenciasment->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    if ($plugin->is_visible() && $plugin->is_enabled()) {
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'asistencias');

        // Feedback plugins.
        foreach ($this->asistenciasment->get_feedback_plugins() as $plugin) {
            if ($this->is_downloading()) {
                if ($plugin->is_visible() && $plugin->is_enabled()) {
                    foreach ($plugin->get_editor_fields() as $field => $description) {
                        $index = 'plugin' . count($this->plugincache);
        ...
        $this->gradinginfo = grade_get_grades($this->asistenciasment->get_course()->id,
                                              'mod',
                                              'asistencias',
                                              $this->asistenciasment->get_instance()->id,
                                              $users);
        $this->hasgrantextension = has_capability('mod/asistencias:grantextension',
                                                  $this->asistenciasment->get_context());
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this asistenciasment.
     *
     * @return string
     */
    public function col_recordid(stdClass $row) {
        return get_string('hiddenuser', 'asistencias') .
               $this->asistenciasment->get_uniqueid_for_user($row->userid);
    }
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id'=>-($this->asistenciasment->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->asistenciasment->get_instance()->grade >= 0) {
                ...
                // This is a custom scale.
                $scale = $this->asistenciasment->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->asistenciasment->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        return get_string('defaultteam', 'asistencias');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->asistenciasment->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->asistenciasment->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_teamstatus(stdClass $row) {
        ...
        return get_string('submissionstatus_' . $status, 'asistencias');
    }
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            $courseid = $this->asistenciasment->get_course()->id;
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'asistencias', fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this asistenciasment.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        return format_float($this->asistenciasment->get_instance()->grade, 2);
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        $separator = '';
        $grade = '';
        $gradingdisabled = $this->asistenciasment->grading_disabled($row->id);

        if (!$this->is_downloading()) {
            $name = fullname($row);
            if ($this->asistenciasment->is_blind_marking()) {
                $name = get_string('hiddenuser', 'asistencias') .
                        $this->asistenciasment->get_uniqueid_for_user($row->userid);
            }
            $icon = $this->output->pix_icon('gradefeedback',
                                            get_string('gradeuser', 'asistencias', $name),
                                            'mod_asistencias');
            $urlparams = array('id' => $this->asistenciasment->get_course_module()->id,
                               'rownum'=>$this->rownum,
                               'action'=>'grade');
    ...
    public function col_status(stdClass $row) {
        ...
        $instance = $this->asistenciasment->get_instance();

        if ($this->asistenciasment->is_any_submission_plugin_enabled()) {

            $o .= $this->output->container(get_string('submissionstatus_' . $row->status, 'asistencias'),
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'asistencias');
                $o .= $this->output->container($lockedstr, 'lockedsubmission');
            }
            if ($row->grade !== null && $row->grade >= 0) {
                $o .= $this->output->container(get_string('graded', 'asistencias'), 'submissiongraded');
            }
            if (!$row->timesubmitted) {
                $now = time();
                ...
                if ($due && ($now > $due)) {
                    $overduestr = get_string('overdue', 'asistencias', format_time($now - $due));
                    $o .= $this->output->container($overduestr, 'overduesubmission');
                }
            }
            if ($row->extensionduedate) {
                $userdate = userdate($row->extensionduedate);
                $extensionstr = get_string('userextensiondate', 'asistencias', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'asistencias');
        }
        $actions[$url->out(false)] = $description;

        // Hide for offline asistenciasments.
        if ($this->asistenciasment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == ASISTENCIAS_SUBMISSION_STATUS_DRAFT ||
                    !$this->asistenciasment->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'asistencias');
                    $actions[$url->out(false)] = $description;
                } else {
                    $urlparams = array('id' => $this->asistenciasment->get_course_module()->id,
                                       'userid'=>$row->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/asistencias/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'asistencias');
                    $actions[$url->out(false)] = $description;
                }
            }

            if (($this->asistenciasment->get_instance()->duedate ||
                   $this->asistenciasment->get_instance()->cutoffdate) &&
                ...
                $urlparams = array('id' => $this->asistenciasment->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/asistencias/view.php', $urlparams);
                $description = get_string('grantextension', 'asistencias');
                $actions[$url->out(false)] = $description;
            }
        }
        if ($row->status == ASISTENCIAS_SUBMISSION_STATUS_SUBMITTED &&
                $this->asistenciasment->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->asistenciasment->get_course_module()->id,
                               'userid'=>$row->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/asistencias/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'asistencias');
            $actions[$url->out(false)] = $description;
        }

        $ismanual = $this->asistenciasment->get_instance()->attemptreopenmethod == ASISTENCIAS_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != ASISTENCIAS_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->asistenciasment->get_instance()->maxattempts == ASISTENCIAS_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->asistenciasment->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->asistenciasment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/asistencias/view.php', $urlparams);
            $description = get_string('addattempt', 'asistencias');
            $actions[$url->out(false)] = $description;
        }
     ...
     *
     * @param asistencias_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(asistencias_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('asistencias')), 'asistencias');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'asistenciassubmission') {
                if ($this->asistenciasment->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == ASISTENCIAS_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->asistenciasment->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->asistenciasment = $this->asistenciasment->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->asistenciasment = $this->asistenciasment->get_instance()->id;
     ...
     *
     * @return string the asistenciasment name
     */
    public function get_asistenciasment_name() {
        return $this->asistenciasment->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->asistenciasment->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->asistenciasment->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->asistenciasment->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this asistenciasment accept submissions
     */
    public function submissions_enabled() {
        return $this->asistenciasment->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->asistenciasment->get_course_context();
