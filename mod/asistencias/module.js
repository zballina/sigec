...
M.mod_asistencias.init_tree = function(Y, expand_all, htmlid) {
...
M.mod_asistencias.init_grading_table = function(Y) {
    Y.use('node', function(Y) {
        ...
        batchform.on('submit', function(e) {
            ...
            if (selectedusers.length == 0) {
                alert(M.str.asistencias.nousersselected);
                e.preventDefault();
            } else {
                ...
                if (action.indexOf(prefix) == 0) {
                    ...
                    confirmmessage = eval('M.str.asistenciasfeedback_' + plugin + '.batchoperationconfirm' + action);
                } else {
                    confirmmessage = eval('M.str.asistencias.batchoperationconfirm' + operation.get('value'));
                }
...
M.mod_asistencias.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_asistencias.init_grade_change = function(Y) {
    ...
    if (gradenode) {
        ...
        gradenode.on('change', function() {
            if (gradenode.get('value') != originalvalue) {
                alert(M.str.mod_asistencias.changegradewarning);
            }
        });
    }
};

M.mod_asistencias.init_plugin_summary = function(Y, subtype, type, submissionid) {
