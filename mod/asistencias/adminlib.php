...
/**
 * This file contains the classes for the admin settings of the asistencias module.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_admin_page_manage_asistencias_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/asistencias/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'asistencias'),
                            $url);
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class asistencias_plugin_manager {
    ...
    /**
     * Constructor for this asistenciasment plugin manager
     * @param string $subtype - either asistenciassubmission or asistenciasfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/asistencias/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'delete', 'settings'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'asistencias'),
                get_string('version'), get_string('hideshow', 'asistencias'),
        ...
        $shortsubtype = substr($this->subtype, strlen('asistencias'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/asistencias/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'asistencias'));
    }
    ...
    public function delete_plugin($plugin) {
        ...
        if ($confirm) {
            ...
            $DB->delete_records('asistencias_plugin_config', array('plugin'=>$plugin, 'subtype'=>$this->subtype));

            // Then the tables themselves.
            $shortsubtype = substr($this->subtype, strlen('asistencias'));
            $installxml = $CFG->dirroot . '/mod/asistencias/' . $shortsubtype . '/' . $plugin . '/db/install.xml';
    ...
    private function view_plugin_deleted($plugin) {
        ...
        echo $OUTPUT->heading(get_string('deletingplugin', 'asistencias', $pluginname));
        echo $this->error;
        $messageparams = array('name'=>$pluginname,
                               'directory'=>('/mod/asistencias/' . $this->subtype . '/'.$plugin));
    ...
    private function view_confirm_delete($plugin) {
        ...
        $this->view_header();
        $pluginname = get_string('pluginname', $this->subtype . '_' . $plugin);
        echo $OUTPUT->heading(get_string('deletepluginareyousure', 'asistencias', $pluginname));
        $urlparams = array('action' => 'delete', 'plugin'=>$plugin, 'confirm' => 1);
        $confirmurl = new moodle_url($this->pageurl, $urlparams);
        echo $OUTPUT->confirm(get_string('deletepluginareyousuremessage', 'asistencias', $pluginname),
                $confirmurl,
                $this->pageurl);
        $this->view_footer();
    }
     ...
     * @return None
     */
    public static function add_admin_asistencias_plugin_settings($subtype,
        ...
        foreach ($pluginsbyname as $pluginname => $plugin) {
            ...
            if ($admin->fulltree) {
                ...
                include($CFG->dirroot . "/mod/asistencias/$shortsubtype/$plugin/settings.php");
            }
