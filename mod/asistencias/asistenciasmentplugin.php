...
/**
 * This file contains the functions for asistencias_plugin abstract class
 *
 *
...
/**
 * Abstract class for asistencias_plugin (submission/feedback).
 *
 * @package   mod_asistencias
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class asistencias_plugin {

    /** @var asistencias $asistenciasment the asistenciasment record that contains the global
     *              settings for this asistencias instance
     */
    protected $asistenciasment;
    /** @var string $type asistenciasment plugin type */
    private $type = '';
     ...
     *
     * @param asistencias $asistenciasment
     * @param string $type
     */
    public final function __construct(asistencias $asistenciasment, $type) {
     ...
     * form for display in the settings page for the asistenciasment.
    ...
    /**
     * The asistenciasment subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'asistenciassubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - asistencias_submission or asistencias_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this asistenciasment.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/asistencias/' . substr($this->get_subtype(), strlen('asistencias')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('asistenciasment'=>$this->asistenciasment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('asistencias_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('asistencias_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->asistenciasment = $this->asistenciasment->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('asistencias_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            if (!$this->asistenciasment->has_instance()) {
                return false;
            }
            $asistenciasment = $this->asistenciasment->get_instance();
            if ($asistenciasment) {
                $dbparams = array('asistenciasment'=>$asistenciasment->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('asistencias_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('asistenciasment'=>$this->asistenciasment->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('asistencias_plugin_config', $dbparams);
    ...
    /**
     * Return true if this plugin can upgrade an old Moodle 2.2 asistenciasment of this type
     * and version.
     *
     * @param string $type The old asistenciasment subtype
     * @param int $version The old asistenciasment version
    ...
    /**
     * Upgrade the settings from the old asistenciasment to the new one
     *
     * @param context $oldcontext The context for the old asistenciasment module
     * @param stdClass $oldasistenciasment The data record for the old asistenciasment
     * @param string $log Record upgrade messages in the log
     * @return bool true or false - false will trigger a rollback
     */
    public function upgrade_settings(context $oldcontext, stdClass $oldasistenciasment, & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_asistencias', $params);
        return false;
    }

    /**
     * Upgrade the submission from the old asistenciasment to the new one
     *
     ...
     * @param stdClass $submissionorgrade asistencias_submission or asistencias_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
     * @return boolean true or false - false will trigger a rollback
     */
    public function upgrade(context $oldcontext,
                            stdClass $oldasistenciasment,
                            stdClass $oldsubmissionorgrade,
                            stdClass $submissionorgrade,
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_asistencias', $params);
        return false;
    }
    ...
    /**
     * The asistenciasment has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this asistenciasment plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade asistencias_submission or asistencias_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'asistenciassubmission') {
            if ($itemid) {
                $record = $DB->get_record('asistencias_submission', array('id'=>$itemid), 'userid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
                if (!$this->asistenciasment->can_view_submission($record->userid)) {
                    return null;
                }
            }
        } else {
        ...
        if (!($storedfile = $fs->get_file($this->asistenciasment->get_context()->id,
    ...
    /**
     * This allows a plugin to render a page in the context of the asistenciasment
     *
     * If the plugin creates a link to the asistenciasment view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=asistenciasfeedback|asistenciassubmission
