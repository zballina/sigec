...
/**
 * This file contains the definition for the class practicalabment
 *
...
define('PRACTICALAB_SUBMISSION_STATUS_REOPENED', 'reopened');
define('PRACTICALAB_SUBMISSION_STATUS_DRAFT', 'draft');
define('PRACTICALAB_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('PRACTICALAB_FILTER_SUBMITTED', 'submitted');
define('PRACTICALAB_FILTER_SINGLE_USER', 'singleuser');
define('PRACTICALAB_FILTER_REQUIRE_GRADING', 'require_grading');

// Reopen attempt methods.
define('PRACTICALAB_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('PRACTICALAB_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('PRACTICALAB_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('PRACTICALAB_UNLIMITED_ATTEMPTS', -1);

require_once($CFG->libdir . '/accesslib.php');
require_once($CFG->libdir . '/formslib.php');
require_once($CFG->dirroot . '/repository/lib.php');
require_once($CFG->dirroot . '/mod/practicalab/mod_form.php');
require_once($CFG->libdir . '/gradelib.php');
require_once($CFG->dirroot . '/grade/grading/lib.php');
require_once($CFG->dirroot . '/mod/practicalab/feedbackplugin.php');
...
/**
 * Standard base class for mod_practicalab (practicalabment types).
 *
 * @package   mod_practicalab
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class practicalab {
    ...
    /** @var stdClass the practicalabment record that contains the global settings for this practicalab instance */
    private $instance;

    /** @var context the context of the course module for this practicalab instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this practicalab instance belongs to */
    private $course;

    /** @var stdClass the admin config for all practicalab instances  */
    private $adminconfig;

    /** @var practicalab_renderer the custom renderer for this module */
    private $output;

    /** @var stdClass the course module for this practicalab instance */
    ...
    /**
     * Constructor for the base practicalab class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('practicalabsubmission');
        $this->feedbackplugins = $this->load_plugins('practicalabfeedback');
    }
    ...
    /**
     * Based on the current practicalabment settings should we display the intro.
     *
    ...
    /**
     * Does an practicalabment have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype practicalabsubmission | practicalabfeedback
     * @param string $type
     * @return mixed practicalab_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('practicalab'));
     ...
     * @return mixed practicalab_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('practicalabfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed practicalab_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('practicalabsubmission', $type);
    }
    ...
    protected function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('practicalab'));
                $pluginclass = 'practicalab_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof practicalab_plugin) {
    ...
    /**
     * Display the practicalabment, used by view.php
     *
     * The practicalabment is displayed differently depending on your role,
     * the settings for the practicalabment and the status of the practicalabment.
     *
    ...
    public function view($action='') {
        ...
        if ($action == 'savesubmission') {
            ...
            if (optional_param('saveandshownext', null, PARAM_RAW)) {
                ...
                $action = 'grade';
                if ($this->process_save_grade($mform)) {
                    $message = get_string('gradingchangessaved', 'practicalab');
        ...
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/practicalab/view.php', $nextpageparams);
        ...
        } else if ($action == 'viewpluginpracticalabfeedback') {
            $o .= $this->view_plugin_content('practicalabfeedback');
        } else if ($action == 'viewpluginpracticalabsubmission') {
            $o .= $this->view_plugin_content('practicalabsubmission');
     ...
     *             when upgrading an old practicalabment to a new one (the plugins get called manually)
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->attemptreopenmethod = PRACTICALAB_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('practicalab', $update);
        $this->instance = $DB->get_record('practicalab', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        $DB->update_record('practicalab', $update);

        return $returnid;
    }

    /**
     * Delete all grades from the gradebook for this practicalabment.
     *
     * @return bool
     */
    protected function delete_grades() {
        ...
        $result = grade_update('mod/practicalab',
                               $this->get_course()->id,
                               'mod',
                               'practicalab',
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this practicalabment.
        $fs = get_file_storage();
        ...
        $DB->delete_records('practicalab_submission', array('practicalabment'=>$this->get_instance()->id));
        $DB->delete_records('practicalab_grades', array('practicalabment'=>$this->get_instance()->id));
        $DB->delete_records('practicalab_plugin_config', array('practicalabment'=>$this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('practicalab', array('id'=>$this->get_instance()->id));
     ...
     * practicalabment submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'practicalab');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_practicalab_submissions)) {
            // Delete files associated with this practicalabment.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'practicalab'),
            ...
            $practicalabssql = 'SELECT a.id
                             FROM {practicalab} a
                           WHERE a.course=:course';
            $params = array('course'=>$data->courseid);

            $DB->delete_records_select('practicalab_submission', "practicalabment IN ($practicalabssql)", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('practicalab_grades', "practicalabment IN ($practicalabssql)", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/practicalab/lib.php');
                practicalab_reset_gradebook($data->courseid);
            }
        }
        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            shift_course_mod_dates('practicalab',
     ...
     *
     * @param practicalab_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(practicalab_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this practicalabment.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this practicalabment
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/practicalab/lib.php');
        $practicalab = clone $this->get_instance();
        $practicalab->cmidnumber = $coursemoduleid;
        $param = null;
        if ($reset) {
            $param = 'reset';
        }

        return practicalab_grade_item_update($practicalab, $param);
    }
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('practicalab');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this practicalabment.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            ...
            $params = array('modulename'=>'practicalab', 'instance'=>$instance->id);
            ...
            if ($event->id) {
                ...
                $event->description = format_module_intro('practicalab', $instance, $coursemoduleid);
            ...
            $DB->delete_records('event', array('modulename'=>'practicalab', 'instance'=>$instance->id));
        }
    }
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('practicalab', $update);
        $this->instance = $DB->get_record('practicalab', array('id'=>$update->id), '*', MUST_EXIST);

        // Load the practicalabment so the plugins have access to it.
     ...
     *
     * @param practicalab_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(practicalab_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
        if ($plugin->is_visible()) {
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'practicalab'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'practicalab'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);
        /*
        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'practicalab'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'practicalab'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    protected function get_module_name() {
        ...
        self::$modulename = get_string('modulename', 'practicalab');
        return self::$modulename;
    }
    ...
    protected function get_module_name_plural() {
        ...
        self::$modulenameplural = get_string('modulenameplural', 'practicalab');
        return self::$modulenameplural;
    }

    /**
     * Has this practicalabment been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this practicalabment
     *
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('practicalab', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
            throw new coding_exception('Improper use of the practicalabment class. ' .
                                       'Cannot load the practicalabment record.');
        }
        return $this->instance;
    }
    ...
    public function get_course_module() {
        ...
        if ($this->context->contextlevel == CONTEXT_MODULE) {
            $this->coursemodule = get_coursemodule_from_id('practicalab',
        ...
        if ($this->get_instance()->grade >= 0) {
            ...
            if ($editing && $this->get_instance()->grade > 0) {
                ...
                $o .= '<label class="accesshide" for="quickgrade_' . $userid . '">' .
                       get_string('usergrade', 'practicalab') .
                       '</label>';
                $o .= '<input type="text"
    ...
    public function list_participants($currentgroup, $idsonly) {
        if ($idsonly) {
            return get_enrolled_users($this->context, 'mod/practicalab:submit', $currentgroup, 'u.id');
        } else {
            return get_enrolled_users($this->context, 'mod/practicalab:submit', $currentgroup);
        }
    }

    /**
     * Load a count of valid teams for this practicalabment.
     *
    ...
    public function count_participants($currentgroup) {
        return count_enrolled_users($this->context, 'mod/practicalab:submit', $currentgroup);
    }
    ...
    public function count_submissions_need_grading() {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group practicalabment because the submission is shared.
            return 0;
        }

        $currentgroup = groups_get_activity_group($this->get_course_module(), true);
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/practicalab:submit', $currentgroup, false);

        $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                 FROM {practicalab_submission} mxs
                                 WHERE mxs.practicalabment = :practicalabid2 GROUP BY mxs.userid';
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {practicalab_grades} mxg
                            WHERE mxg.practicalabment = :practicalabid3 GROUP BY mxg.userid';

        $params['practicalabid'] = $this->get_instance()->id;
        $params['practicalabid2'] = $this->get_instance()->id;
        $params['practicalabid3'] = $this->get_instance()->id;
        $params['submitted'] = PRACTICALAB_SUBMISSION_STATUS_SUBMITTED;
    ...
    public function count_grades() {
        ...
        $sql = 'SELECT COUNT(g.userid)
                   FROM {practicalab_grades} g
                   JOIN(' . $esql . ') e ON e.id = g.userid
                   WHERE g.practicalabment = :practicalabid';
    ...
    public function count_submissions_with_status($status) {
        ...
        if ($this->get_instance()->teamsubmission) {
            $maxattemptsql = 'SELECT mxs.groupid, MAX(mxs.attemptnumber) AS maxattempt
                              FROM {practicalab_submission} mxs
                              WHERE mxs.practicalabment = :practicalabid2 GROUP BY mxs.groupid';
    ...
    protected function get_grading_userid_list() {
        $filter = get_user_preferences('practicalab_filter', '');
        $table = new practicalab_grading_table($this, 0, $filter, 0, false);
    ...
    protected function pack_files($filesforzipping) {
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'practicalabment_');
        // Zip files.
    ...
    /**
     * Finds all practicalabment notifications that have yet to be mailed out, and mails them.
     *
    ...
    public static function cron() {
        ...
        $sql = 'SELECT a.course, a.name, a.blindmarking, a.revealidentities,
                 ...
                 FROM {practicalab} a
                 JOIN {practicalab_grades} g ON g.practicalabment = a.id
                 LEFT JOIN {practicalab_user_flags} uf ON uf.practicalabment = a.id AND uf.userid = g.userid
        ...
        mtrace('Processing ' . count($submissions) . ' practicalabment submissions ...');
        ...
        foreach ($submissions as $submission) {
            ...
            mtrace("Processing practicalabment submission $submission->id ...");
            ...
            if (!array_key_exists($submission->practicalabment, $modcache)) {
                $mod = get_coursemodule_from_instance('practicalab', $submission->practicalabment, $course->id);
                if (empty($mod)) {
                    mtrace('Could not find course module for practicalabment id ' . $submission->practicalabment);
                    continue;
                }
                $modcache[$submission->practicalabment] = $mod;
            } else {
                $mod = $modcache[$submission->practicalabment];
            }
            // Context lookups are already cached.
            $contextmodule = context_module::instance($mod->id);

            if (!$mod->visible) {
                // Hold mail notification for hidden practicalabments until later.
                continue;
            }

            // Need to send this to the student.
            $messagetype = 'feedbackavailable';
            $eventtype = 'practicalab_notification';
            $updatetime = $submission->lastmodified;
            $modulename = get_string('modulename', 'practicalab');

            $uniqueid = 0;
            if ($submission->blindmarking && !$submission->revealidentities) {
                $uniqueid = self::get_uniqueid_for_user_static($submission->practicalabment, $user->id);
            }
            $showusers = $submission->blindmarking && !$submission->revealidentities;
            self::send_practicalabment_notification($grader,
                                               $user,
                                               $messagetype,
                                               $eventtype,
                                               $updatetime,
            ...
            $flags = $DB->get_record('practicalab_user_flags', array('userid'=>$user->id, 'practicalabment'=>$submission->practicalabment));
            if ($flags) {
                $flags->mailed = 1;
                $DB->update_record('practicalab_user_flags', $flags);
            } else {
                $flags = new stdClass();
                $flags->userid = $user->id;
                $flags->practicalabment = $submission->practicalabment;
                $flags->mailed = 1;
        ...
        mtrace('Done processing ' . count($submissions) . ' practicalabment submissions');
    ...
    /**
     * Update user flags for this user in this practicalabment.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->practicalabment <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('practicalab_user_flags', $flags);
        return $result;
    }

    /**
     * Update a grade in the grade table for the practicalabment and in the gradebook.
     *
     * @param stdClass $grade a grade record keyed on id
     * @return bool true for success
     */
    public function update_grade($grade) {
        ...
        $result = $DB->update_record('practicalab_grades', $grade);
    ...
    protected function view_grant_extension($mform) {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/mod/practicalab/extensionform.php');
        ...
        $header = new practicalab_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'practicalab'));
        ...
        if (!$mform) {
            $formparams = array($this->get_course_module()->id,
                                ...
                                $this->get_instance(),
                                $data);
            $mform = new mod_practicalab_extension_form(null, $formparams);
        }
        $o .= $this->get_renderer()->render(new practicalab_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the practicalabment.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
    ...
    public function get_submission_group_members_who_have_not_submitted($groupid, $onlyids) {
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == PRACTICALAB_SUBMISSION_STATUS_SUBMITTED) {
                unset($members[$id]);
            } else {
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'practicalab') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('practicalabment'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        $submission = null;
        $submissions = $DB->get_records('practicalab_submission', $params, 'attemptnumber DESC', '*', 0, 1);
        if ($submissions) {
        ...
        if ($create) {
            ...
            $submission->practicalabment = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            $submission->status = PRACTICALAB_SUBMISSION_STATUS_DRAFT;
    ...
    /**
     * View a summary listing of all practicalabments in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('practicalab', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new practicalab_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['practicalab'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            if (has_capability('mod/practicalab:grade', $context)) {
                $submitted = $practicalabment->count_submissions_with_status(PRACTICALAB_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/practicalab:submit', $context)) {
                $usersubmission = $practicalabment->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'practicalab');
                } else {
                    $submitted = get_string('submissionstatus_', 'practicalab');
                }
            }
            $grading_info = grade_get_grades($course->id, 'mod', 'practicalab', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_practicalab_info($cm->id, $cm->name, $sectionname, $timedue, $submitted, $grade);
    ...
    /**
     * This is used for team practicalabments to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_TEXT);
        $item = null;
        if ($pluginsubtype == 'practicalabsubmission') {
            ...
            if ($item->userid != $USER->id) {
                require_capability('mod/practicalab:grade', $this->context);
            }
            $o .= $this->get_renderer()->render(new practicalab_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new practicalab_submission_plugin_submission($plugin,
                                                              $item,
                                                              practicalab_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            $logmessage = get_string('viewsubmissionforuser', 'practicalab', $item->userid);
            ...
            $o .= $this->get_renderer()->render(new practicalab_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new practicalab_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              practicalab_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));
            $logmessage = get_string('viewfeedbackforuser', 'practicalab', $item->userid);
     ...
     * @param practicalab_plugin $plugin - The practicalabment plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        ...
        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'practicalab');
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component) {
        ...
        if ($CFG->enableportfolios) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('practicalab_portfolio_caller', $portfolioparams, 'mod_practicalab');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new practicalab_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'practicalab')));
        $gradingresult = new practicalab_gradingmessage(get_string('savegradingresult', 'practicalab'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new practicalab_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'practicalab')));
        $gradingresult = new practicalab_gradingmessage(get_string('quickgradingresult', 'practicalab'),
                                                   $message,
                                                   $this->get_course_module()->id);
        $o .= $this->get_renderer()->render($gradingresult);
        $o .= $this->view_footer();
    ...
    /**
     * Does this user have grade permission for this practicalabment?
     *
     * @return bool
     */
    protected function can_grade() {
        // Permissions check.
        if (!has_capability('mod/practicalab:grade', $this->context)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all practicalabment submissions.
     *
    ...
    protected function download_submissions() {
        ...
        $students = get_enrolled_users($this->context, "mod/practicalab:submit");
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->get_instance()->teamsubmission) {
                    ...
                    if ($submissiongroup) {
                        ...
                        $groupname = get_string('defaultteam', 'practicalab') . '-';
                    }
                } else {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'practicalab'));
        ...
        if (count($filesforzipping) == 0) {
            $header = new practicalab_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'practicalab'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'practicalab'));
            $url = new moodle_url('/mod/practicalab/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            $this->add_to_log('download all submissions', get_string('downloadall', 'practicalab'));
     ...
     * @param string $url The url to the practicalab module instance.
     * @return void
     */
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_practicalab');
        return $this->output;
    }
     ...
     *
     * For team practicalabments there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('practicalabment'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('practicalabment'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('practicalab_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('practicalabment'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('practicalab_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->practicalabment = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('practicalab_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('practicalab_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->practicalabment   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('practicalab_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('practicalabment'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('practicalab_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grade_page($mform) {
        ...
        require_once($CFG->dirroot . '/mod/practicalab/gradeform.php');

        // Need submit permission to submit an practicalabment.
        require_capability('mod/practicalab:grade', $this->context);

        $header = new practicalab_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'practicalab'));
        $o .= $this->get_renderer()->render($header);
        ...
        $cache = cache::make_from_params(cache_store::MODE_SESSION, 'mod_practicalab', 'useridlist');
        if (!$userid) {
        ...
        if ($this->can_view_submission($userid)) {
            ...
            if ($teamsubmission) {
                $showsubmit = $showedit &&
                              $teamsubmission &&
                              ($teamsubmission->status == PRACTICALAB_SUBMISSION_STATUS_DRAFT);
            } else {
                $showsubmit = $showedit &&
                              $submission &&
                              ($submission->status == PRACTICALAB_SUBMISSION_STATUS_DRAFT);
            ...
            $submissionstatus = new practicalab_submission_status($instance->allowsubmissionsfromdate,
                                                             ...
                                                             $this->get_course_module()->id,
                                                             $this->get_course()->id,
                                                             practicalab_submission_status::GRADER_VIEW,
                                                             $showedit,
                                                             $showsubmit,
                                                             $viewfullnames,
                                                             $extensionduedate,
        ...
        if ($attemptnumber != -1) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'practicalab', $params);
        ...
        if (!$mform) {
            ...
            $mform = new mod_practicalab_grade_form(null,
                                               $formparams,
                                               'post',
                                               '',
        ...
        $o .= $this->get_renderer()->render(new practicalab_form('gradingform', $mform));

        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new practicalab_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    protected function view_reveal_identities_confirm() {
        ...
        require_capability('mod/practicalab:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/practicalab/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/practicalab/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'practicalab'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();
        $this->add_to_log('view', get_string('viewrevealidentitiesconfirm', 'practicalab'));
        return $o;
    }
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/practicalab/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this practicalabment.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/practicalab/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/practicalab/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/practicalab/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'practicalab');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/practicalab/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'practicalab');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/practicalab:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/practicalab/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'practicalab');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/practicalab/view.php' .
                           '?id=' .  $cmid .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'practicalab'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_practicalab', 'submissions');

        $perpage = get_user_preferences('practicalab_perpage', 10);
        $filter = get_user_preferences('practicalab_filter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller);
        $quickgrading = get_user_preferences('practicalab_quickgrading', false);
        ...
        $gradingoptionsform = new mod_practicalab_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_practicalab_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new practicalab_grading_table($this, $perpage, $filter, 0, true);
            $table = $this->get_renderer()->render($gradingtable);
            $quickformparams = array('cm'=>$this->get_course_module()->id, 'gradingtable'=>$table);
            $quickgradingform = new mod_practicalab_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new practicalab_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new practicalab_grading_table($this, $perpage, $filter, 0, false);
        ...
        if (count($users) != 0) {
            ...
            $practicalabform = new practicalab_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($practicalabform);
        }
        $practicalabform = new practicalab_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_practicalab.init_grading_options');
        $o .= $this->get_renderer()->render($practicalabform);
        return $o;
    }
    ...
    protected function view_grading_page() {
        ...
        $logmessage = get_string('viewsubmissiongradingtable', 'practicalab');
    ...
    /**
     * Message for students when practicalabment submissions have been closed.
     *
     * @return string
     */
    protected function view_student_error_message() {
        ...
        $o = '';
        // Need submit permission to submit an practicalabment.
        require_capability('mod/practicalab:submit', $this->context);

        $header = new practicalab_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('editsubmission', 'practicalab'));
        $o .= $this->get_renderer()->render($header);

        $o .= $this->get_renderer()->notification(get_string('submissionsclosed', 'practicalab'));
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/practicalab/submission_form.php');
        // Need submit permission to submit an practicalabment.
        require_capability('mod/practicalab:submit', $this->context);
        ...
        if (!$mform) {
            $mform = new mod_practicalab_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new practicalab_form('editsubmissionform', $mform));

        $o .= $this->view_footer();
        $this->add_to_log('view submit practicalabment form', get_string('viewownsubmissionform', 'practicalab'));

        return $o;
    }

    /**
     * See if this practicalabment has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/practicalab:submit', $this->context)) {
            return true;
        }
        if (has_capability('mod/practicalab:grade', $this->context)) {
            return true;
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_practicalab_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/practicalab/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_practicalab_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $submitforgradingpage = new practicalab_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        $logmessage = get_string('viewownsubmissionform', 'practicalab');
        $this->add_to_log('view confirm submit practicalabment form', $logmessage);
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            $showedit = has_capability('mod/practicalab:submit', $this->context) &&
            ...
            $gradingmanager = get_grading_manager($this->context, 'mod_practicalab', 'submissions');
            ...
            if ($teamsubmission && ($teamsubmission->status != PRACTICALAB_SUBMISSION_STATUS_DRAFT)) {
                $showsubmit = false;
            }
            if ($submission && ($submission->status != PRACTICALAB_SUBMISSION_STATUS_DRAFT)) {
                $showsubmit = false;
            }
            if (!$this->get_instance()->submissiondrafts) {
                $showsubmit = false;
            ...
            $submissionstatus = new practicalab_submission_status($instance->allowsubmissionsfromdate,
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $this->get_course()->id,
                                                              practicalab_submission_status::STUDENT_VIEW,
                                                              $showedit,
                                                              $showsubmit,
                                                              $viewfullnames,
                                                              $extensionduedate,
            ...
            $cangrade = has_capability('mod/practicalab:grade', $this->get_context());
            ...
            if ((!empty($gradebookgrade->grade) && ($cangrade || !$gradebookgrade->hidden)) ||
                ...
                $feedbackstatus = new practicalab_feedback_status($gradefordisplay,
                                                      $gradeddate,
                                                      $grader,
                                                      $this->get_feedback_plugins(),
                                                      $grade,
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('practicalab_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('practicalab_submission', $params, 'attemptnumber ASC');
    ...
    protected function view_submission_page() {
        ...
        $o = '';
        $o .= $this->get_renderer()->render(new practicalab_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id));

        if ($this->can_grade()) {
            $draft = PRACTICALAB_SUBMISSION_STATUS_DRAFT;
            $submitted = PRACTICALAB_SUBMISSION_STATUS_SUBMITTED;
            if ($instance->teamsubmission) {
                $summary = new practicalab_grading_summary($this->count_teams(),
                ...
                $summary = new practicalab_grading_summary($this->count_participants(0),
                                                      $instance->submissiondrafts,
                                                      $this->count_submissions_with_status($draft),
                                                      $this->is_any_submission_plugin_enabled(),
                                                      $this->count_submissions_with_status($submitted),
        ...
        $o .= $this->view_footer();
        $this->add_to_log('view', get_string('viewownsubmissionstatus', 'practicalab'));
        return $o;
    }
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        $practicalab = clone $this->get_instance();
        $practicalab->cmidnumber = $this->get_course_module()->id;

        return practicalab_grade_item_update($practicalab, $gradebookgrade);
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this practicalabment qualifies as submitted or draft.
        $team = $this->get_submission_group_members($submission->groupid, true);

        $allsubmitted = true;
        $anysubmitted = false;
        $result = true;
        if ($submission->status != PRACTICALAB_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                $membersubmission = $this->get_user_submission($member->id, false, $submission->attemptnumber);

                if (!$membersubmission || $membersubmission->status != PRACTICALAB_SUBMISSION_STATUS_SUBMITTED) {
                    $allsubmitted = false;
                    if ($anysubmitted) {
                        break;
                    }
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('practicalab_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = PRACTICALAB_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = PRACTICALAB_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('practicalab_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('practicalab_submission', $submission) && $result;
        }
    ...
    /**
     * Is this practicalabment open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the practicalabment locked?
     *
    ...
    public function submissions_open($userid = 0) {
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == PRACTICALAB_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the practicalabment.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        $files = $fs->get_area_files($this->get_context()->id,
                                     ...
                                     'timemodified',
                                     false);
        return $this->get_renderer()->practicalab_files($this->context, $submissionid, $area, $component);
    ...
    protected function get_graders($userid) {
        $potentialgraders = get_enrolled_users($this->context, 'mod/practicalab:grade');
     ...
     * @param string $practicalabmentname
     */
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext  = format_string($course->shortname, true, $formatparams) .
                     ...
                     $modulename .
                     ' -> ' .
                     format_string($practicalabmentname, true, $formatparams) . "\n";
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'practicalab', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/practicalab/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/practicalab/view.php?id=' . $coursemodule->id . '">' .
                     format_string($practicalabmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'practicalab', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_practicalabment_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        $info = new stdClass();
        if ($blindmarking) {
            $info->username = get_string('participant', 'practicalab') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'practicalab');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->practicalabment = format_string($practicalabmentname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/practicalab/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'practicalab', $info);
        ...
        $eventdata->component       = 'mod_practicalab';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->practicalabment;
    ...
    public function send_notification($userfrom,
                                      ...
                                      $eventtype,
                                      $updatetime) {
        self::send_practicalabment_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    protected function process_submit_for_grading($mform) {
        ...
        if (!$mform->is_cancelled()) {
            ...
            if ($submission->status != PRACTICALAB_SUBMISSION_STATUS_SUBMITTED) {
    ...
    protected function process_save_extension(& $mform) {
        ...
        // Need submit permission to submit an practicalabment.
        require_capability('mod/practicalab:grantextension', $this->context);
        ...
        $mform = new mod_practicalab_extension_form(null, array($this->get_course_module()->id,
                                                           $userid,
                                                           $batchusers,
                                                           $this->get_instance(),
                                                           null));
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'practicalab');
        }
        ...
        $params['practicalabid1'] = $this->get_instance()->id;
        $params['practicalabid2'] = $this->get_instance()->id;

        // Check them all for currency.
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {practicalab_grades} mxg
                            WHERE mxg.practicalabment = :practicalabid1 GROUP BY mxg.userid';
        ...
        $sql = 'SELECT u.id as userid, g.grade as grade, g.timemodified as lastmodified
                ...
                LEFT JOIN {practicalab_grades} g ON
                    u.id = g.userid AND
                    g.practicalabment = :practicalabid2 AND
                    g.attemptnumber = gmx.maxattempt
                WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'practicalab');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('practicalabfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/practicalab',
                                          $this->course->id,
                                          'mod',
                                          'practicalab',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'practicalab');
    }
    ...
    protected function process_reveal_identities() {
        ...
        require_capability('mod/practicalab:revealidentities', $this->context);
        if (!confirm_sesskey()) {
            return false;
        }

        // Update the practicalabment record.
        $update = new stdClass();
        $update->id = $this->get_instance()->id;
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $grades = $DB->get_records('practicalab_grades', array('practicalabment'=>$this->get_instance()->id));
        ...
        $this->add_to_log('reveal identities', get_string('revealidentities', 'practicalab'));
    }
    ...
    protected function process_save_grading_options() {
        ...
        $mform = new mod_practicalab_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('practicalab_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('practicalab_filter', $formdata->filter);
            }
            if ($showquickgrading) {
                set_user_preference('practicalab_quickgrading', isset($formdata->quickgrading));
            }
        }
    }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'practicalab', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'practicalab');
        }
        return $info;
    }
    ...
    protected function format_submission_for_log(stdClass $submission) {
        $info = '';
        $info .= get_string('submissionstatus', 'practicalab') .
                 ': ' .
                 get_string('submissionstatus_' . $submission->status, 'practicalab') .
                 '. <br>';
    ...
    /**
     * Copy the current practicalabment submission from the last submitted attempt.
     *
    ...
    protected function process_copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != PRACTICALAB_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'practicalab');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'practicalab');
            return false;
        }
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted practicalabments
    ...
    /**
     * Save practicalabment submission.
     *
     * @param  moodleform $mform
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        if ($data = $mform->get_data()) {
            ...
            // Get the flags to check if it is locked.
            if ($flags && $flags->locked) {
                print_error('submissionslocked', 'practicalab');
                return true;
            }
            ...
            if ($pluginerror || $allempty) {
                if ($allempty) {
                    $notices[] = get_string('submissionempty', 'mod_practicalab');
                }
                return false;
            }
     ...
     * This is specific to the practicalabment, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'practicalab', $this->get_instance()->grade);
                $gradingelement = $mform->addElement('text', 'grade', $name);
                $mform->addHelpButton('grade', 'gradeoutofhelp', 'practicalab');
        ...
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'practicalab'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'practicalab', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'practicalab'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'practicalab'));
            $mform->setDefault('applytoall', 1);
        }

        // Do not show if we are editing a previous attempt.
        if ($attemptnumber == -1 && $this->get_instance()->attemptreopenmethod != PRACTICALAB_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'practicalab'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'practicalab');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'practicalab'), $attemptreopenmethod);
            ...
            if ($maxattempts == PRACTICALAB_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'practicalab');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'practicalab'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'practicalab'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == PRACTICALAB_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == PRACTICALAB_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'practicalab'));
        ...
        $buttonarray=array();
        $name = get_string('savechanges', 'practicalab');
        $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
        if (!$last) {
            $name = get_string('savenext', 'practicalab');
        ...
        if ($rownum > 0) {
            $name = get_string('previous', 'practicalab');
            $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
        }

        if (!$last) {
            $name = get_string('nosavebutnext', 'practicalab');
    ...
    protected function process_save_grade(&$mform) {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            $maxattemptsreached = !empty($submission) &&
                                  ...
                                  $instance->maxattempts != PRACTICALAB_UNLIMITED_ATTEMPTS;
            $shouldreopen = false;
            if ($instance->attemptreopenmethod == PRACTICALAB_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
     ...
     * @return bool
     */
    public static function can_upgrade_practicalabment($type, $version) {
        $practicalabment = new practicalab(null, null, null);
        return $practicalabment->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an practicalabment from the 2.2 module.
     *
     * @param string $type The plugin type
     * @param int $version The plugin version
     * @return bool
    ...
    /**
     * Copy all the files from the old practicalabment files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old practicalabment context id
     * @param int $oldcomponent The old practicalabment component ('practicalabment')
     * @param int $oldfilearea The old practicalabment filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new practicalabment context id
     * @param int $newcomponent The new practicalabment component ('practicalabment')
     * @param int $newfilearea The new practicalabment filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group practicalabment
     * at most 1 time.
     *
    ...
    protected function process_add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == PRACTICALAB_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != PRACTICALAB_UNLIMITED_ATTEMPTS &&
        ...
        $submission->status = PRACTICALAB_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        global $DB, $CFG;
        $grades = array();
        $practicalabmentid = $this->get_instance()->id;
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('practicalabfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {practicalab_grades} mxg
                            WHERE mxg.practicalabment = :practicalabid2 GROUP BY mxg.userid';

        // When the gradebook asks us for grades - only return the last attempt for each user.
        $params = array('practicalabid1'=>$practicalabmentid,
                        'practicalabid2'=>$practicalabmentid,
                        'practicalabid3'=>$practicalabmentid,
                        'practicalabid4'=>$practicalabmentid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                LEFT JOIN ( ' . $grademaxattempt . ' ) gmx ON u.id = gmx.userid
                                                LEFT JOIN {practicalab_submission} s
                                                    ON u.id = s.userid and s.practicalabment = :practicalabid3 AND
                                                    s.attemptnumber = smx.maxattempt
                                                JOIN {practicalab_grades} g
                                                    ON u.id = g.userid and g.practicalabment = :practicalabid4 AND
                                                    g.attemptnumber = gmx.maxattempt' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - practicalab them a random id.
     *
     * @param int $practicalabid The practicalabid to lookup
     */
    public static function allocate_unique_ids($practicalabid) {
        global $DB;

        $cm = get_coursemodule_from_instance('practicalab', $practicalabid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/practicalab:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        $record = new stdClass();
        $record->practicalabment = $practicalabid;
        foreach ($users as $user) {
            $record = $DB->get_record('practicalab_user_mapping',
                                      array('practicalabment'=>$practicalabid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->userid = $user->id;
                $DB->insert_record('practicalab_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this practicalabment.
     *
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($practicalabid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('practicalabment'=>$practicalabid, 'userid'=>$userid);
        if ($record = $DB->get_record('practicalab_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($practicalabid);
        ...
        return $DB->insert_record('practicalab_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this practicalabment.
     *
     * @param int $practicalabid The id of the practicalabment this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($practicalabid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('practicalab_user_mapping',
                                      array('practicalabment'=>$practicalabid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
...
/**
 * Portfolio caller class for mod_practicalab.
 *
 * @package   mod_practicalab
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the practicalabment we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('practicalab', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the practicalabment type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_practicalab');
            }
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'practicalabment.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'practicalab' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return practicalab_submission_plugin
     */
    protected function get_submission_plugin() {
        ...
        return $practicalabment->get_submission_plugin_by_type($this->plugin);
    }
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/practicalab:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'practicalab');
    }
