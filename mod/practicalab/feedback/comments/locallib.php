 ...
 *
 * @package   practicalabfeedback_comments
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_feedback_comments extends practicalab_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'practicalabfeedback_comments');
    }
    ...
    public function get_feedback_comments($gradeid) {
        global $DB;
        return $DB->get_record('practicalabfeedback_comments', array('grade'=>$gradeid));
    }
    ...
    public function get_quickgrading_html($userid, $grade) {
        ...
        $pluginname = get_string('pluginname', 'practicalabfeedback_comments');
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'practicalabfeedback_comments'));
    }
    ...
    public function set_editor_text($name, $value, $gradeid) {
        ...
        if ($name == 'comments') {
            ...
            if ($feedbackcomment) {
                ...
                return $DB->update_record('practicalabfeedback_comments', $feedbackcomment);
            } else {
                $feedbackcomment = new stdClass();
                $feedbackcomment->commenttext = $value;
                $feedbackcomment->commentformat = FORMAT_HTML;
                $feedbackcomment->grade = $gradeid;
                $feedbackcomment->practicalabment = $this->practicalabment->get_instance()->id;
                return $DB->insert_record('practicalabfeedback_comments', $feedbackcomment) > 0;
            }
        }
    ...
    public function get_form_elements($grade, MoodleQuickForm $mform, stdClass $data) {
        if ($grade) {
            $feedbackcomments = $this->get_feedback_comments($grade->id);
            if ($feedbackcomments) {
                $data->practicalabfeedbackcomments_editor['text'] = $feedbackcomments->commenttext;
                $data->practicalabfeedbackcomments_editor['format'] = $feedbackcomments->commentformat;
            }
        }

        $mform->addElement('editor', 'practicalabfeedbackcomments_editor', html_writer::tag('span', $this->get_name(),
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        $feedbackcomment = $this->get_feedback_comments($grade->id);
        if ($feedbackcomment) {
            $feedbackcomment->commenttext = $data->practicalabfeedbackcomments_editor['text'];
            $feedbackcomment->commentformat = $data->practicalabfeedbackcomments_editor['format'];
            return $DB->update_record('practicalabfeedback_comments', $feedbackcomment);
        } else {
            $feedbackcomment = new stdClass();
            $feedbackcomment->commenttext = $data->practicalabfeedbackcomments_editor['text'];
    ...
    public function upgrade(context $oldcontext,
        ...
        $feedbackcomments->practicalabment = $this->practicalabment->get_instance()->id;
        if (!$DB->insert_record('practicalabfeedback_comments', $feedbackcomments) > 0) {
     ...
     * of the comment
     *
     * Only one feedback plugin can push comments to the gradebook and that is chosen by the practicalabment
     * settings page.
     *
     * @param stdClass $grade The grade
     * @return int
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('practicalabfeedback_comments',
