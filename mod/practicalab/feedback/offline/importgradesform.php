 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalabfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $renderer = $PAGE->get_renderer('practicalab');

        // Visible elements.
        $practicalabment = $params['practicalabment'];
        ...
        $scaleoptions = null;
        if ($practicalabment->get_instance()->grade < 0) {
            $scale = $DB->get_record('scale', array('id'=>-($practicalabment->get_instance()->grade)));
            if ($scale) {
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'practicalabfeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($practicalabment->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'practicalab') . $practicalabment->get_uniqueid_for_user($user->id);
            }

            $usergrade = $practicalabment->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $grade) {
                ...
                $skip = true;
            } else if ($practicalabment->grading_disabled($user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($practicalabment->get_instance()->grade > -1) &&
                      (($grade < 0) || ($grade > $practicalabment->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }
            ...
            if (!$skip) {
                $update = true;
                $updates[] = get_string('gradeupdate', 'practicalabfeedback_offline',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'practicalabfeedback_offline',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'practicalabfeedback_offline'));
        }

        $mform->addElement('hidden', 'id', $practicalabment->get_course_module()->id);
        ...
        $mform->addElement('hidden', 'pluginsubtype', 'practicalabfeedback');
