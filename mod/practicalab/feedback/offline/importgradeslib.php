 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalabfeedback_offline_grade_importer {
    ...
    /** @var practicalabment $practicalabment - the practicalabment class */
    private $practicalabment;
     ...
     * @param practicalab $practicalabment The current practicalabment
     */
    public function __construct($importid, practicalab $practicalabment) {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'practicalabfeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'practicalab');
        $strmodified = get_string('lastmodifiedgrade', 'practicalab');
        ...
        $this->validusers = $this->practicalabment->list_participants($groupid, false);
        return true;
    }
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'practicalab')));
            if ($userid = $this->practicalabment->get_user_id_for_uniqueid($id)) {
