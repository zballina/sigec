...
/**
 * This file contains the definition for the renderable classes for the practicalabment
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_submit_for_grading_page implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_gradingmessage implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_form implements renderable {
    /** @var moodleform $form is the edit submission form */
    public $form = null;
    /** @var string $classname is the name of the class to practicalab to the container */
    public $classname = '';
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_user_summary implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_feedback_plugin_feedback implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;

    /** @var practicalab_submission_plugin $plugin */
    public $plugin = null;
    /** @var stdClass $grade */
    public $grade = null;
    /** @var string $view */
     ...
     *
     * @param practicalab_feedback_plugin $plugin
     ...
     */
    public function __construct(practicalab_feedback_plugin $plugin,
                                stdClass $grade,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_submission_plugin_submission implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;
    ...
    /**
     * Constructor
     * @param practicalab_submission_plugin $plugin
     ...
     */
    public function __construct(practicalab_submission_plugin $plugin,
                                stdClass $submission,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_feedback_status implements renderable {
    ...
    /** @var stdClass practicalab_grade record */
    public $grade = null;
    /** @var int coursemoduleid */
    public $coursemoduleid = 0;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_submission_status implements renderable {
...
/**
 * Used to output the attempt history for a particular practicalabment.
 *
 * @package mod_practicalab
 * @copyright 2012 Davo Smith, Synergy Learning
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_attempt_history implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_header implements renderable {
    /** @var stdClass the practicalab record  */
    public $practicalab = null;
     ...
     * Constructor
     *
     * @param stdClass $practicalab  - the practicalab database record
     ...
     */
    public function __construct(stdClass $practicalab,
                                $context,
                                $showintro,
                                $coursemoduleid,
                                $subpage='',
                                $preface='') {
        $this->practicalab = $practicalab;
        $this->context = $context;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_grading_summary implements renderable {
    /** @var int participantcount - The number of users who can submit to this practicalabment */
    ...
    /** @var int duedate - The practicalabment due date (if one is set) */
    public $duedate = 0;
    /** @var int cutoffdate - The practicalabment cut off date (if one is set) */
    public $cutoffdate = 0;
    /** @var int coursemoduleid - The practicalabment course module id */
    public $coursemoduleid = 0;
    /** @var boolean teamsubmission - Are team submissions enabled for this practicalabment */
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_course_index_summary implements renderable {
    /** @var array practicalabments - A list of course module info and submission counts or statuses */
    public $practicalabments = array();
     ...
     * @param int $timedue - The due date for the practicalabment - may be 0 if no duedate
     * @param string $submissioninfo - A string with either the number of submitted practicalabments, or the
     ...
     */
    public function add_practicalab_info($cmid, $cmname, $sectionname, $timedue, $submissioninfo, $gradeinfo) {
        $this->practicalabments[] = array('cmid'=>$cmid,
                               'cmname'=>$cmname,
...
/**
 * An practicalab file class that extends rendererable class and is used by the practicalab module.
 *
 * @package   mod_practicalab
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_files implements renderable {
    /** @var context $context */
    public $context;
    /** @var string $context */
    public $dir;
    ...
    public function __construct(context $context, $sid, $filearea, $component) {
        ...
        if (!empty($CFG->enableportfolios)) {
            ...
            if (count($files) >= 1 &&
                    has_capability('mod/practicalab:exportownsubmission', $this->context)) {
                $button = new portfolio_add_button();
                $callbackparams = array('cmid' => $this->cm->id,
                                        'sid' => $sid,
                                        'area' => $filearea,
                                        'component' => $component);
                $button->set_callback_options('practicalab_portfolio_caller',
                                              $callbackparams,
                                              'mod_practicalab');
        ...
        if (!empty($CFG->enableplagiarism)) {
            ...
            $practicalabment = new practicalab($this->context, null, null);
            foreach ($files as $file) {
                ...
                $linkparams = array('userid' => $sid,
                                    ...
                                    'cmid' => $this->cm->id,
                                    'course' => $this->course,
                                    'practicalabment' => $practicalabment->get_instance());
    ...
    public function preprocess($dir, $filearea, $component) {
        ...
        foreach ($dir['files'] as $file) {
            ...
            if (!empty($CFG->enableportfolios)) {
                $button = new portfolio_add_button();
                if (has_capability('mod/practicalab:exportownsubmission', $this->context)) {
