...
M.mod_practicalab.init_tree = function(Y, expand_all, htmlid) {
    Y.use('yui2-treeview', function(Y) {
        var tree = new Y.YUI2.practicalab.TreeView(htmlid);
...
M.mod_practicalab.init_grading_table = function(Y) {
    Y.use('node', function(Y) {
        ...
        batchform.on('submit', function(e) {
            ...
            if (selectedusers.length == 0) {
                alert(M.str.practicalab.nousersselected);
                e.preventDefault();
            } else {
                ...
                if (action.indexOf(prefix) == 0) {
                    ...
                    confirmmessage = eval('M.str.practicalabfeedback_' + plugin + '.batchoperationconfirm' + action);
                } else {
                    confirmmessage = eval('M.str.practicalab.batchoperationconfirm' + operation.get('value'));
                }
...
M.mod_practicalab.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_practicalab.init_grade_change = function(Y) {
    ...
    if (gradenode) {
        ...
        gradenode.on('change', function() {
            if (gradenode.get('value') != originalvalue) {
                alert(M.str.mod_practicalab.changegradewarning);
            }
        });
    }
};

M.mod_practicalab.init_plugin_summary = function(Y, subtype, type, submissionid) {
