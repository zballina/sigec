 ...
 * @return bool
 */
function practicalabsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('practicalab_submission', array('id'=>$options->itemid))) {
    ...
    if ($practicalabment->get_instance()->id != $submission->practicalabment) {
        throw new comment_exception('invalidcontext');
    }
    if (!has_capability('mod/practicalab:grade', $context)) {
        if (!has_capability('mod/practicalab:submit', $context)) {
            throw new comment_exception('nopermissiontocomment');
        } else if ($practicalabment->get_instance()->teamsubmission) {
            $group = $practicalabment->get_submission_group($USER->id);
            $groupid = 0;
            if ($group) {
                $groupid = $group->id;
            }
 ...
 * @return array
 */
function practicalabsubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if ($practicalabment->get_instance()->teamsubmission &&
        !$practicalabment->can_view_group_submission($submission->groupid)) {
        return array('post' => false, 'view' => false);
    }

    if (!$practicalabment->get_instance()->teamsubmission &&
        !$practicalabment->can_view_submission($submission->userid)) {
        return array('post' => false, 'view' => false);
    }
 ...
 */
function practicalabsubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($practicalabment->is_blind_marking() && !empty($comments)) {
        // Blind marking is being used, may need to map unique anonymous ids to the comments.
        $usermappings = array();
        $hiddenuserstr = trim(get_string('hiddenuser', 'practicalab'));
        ...
        foreach ($comments as $comment) {
            ...
            if (empty($usermappings[$comment->userid])) {
                ...
                $anonid = $practicalabment->get_uniqueid_for_user($comment->userid);
                ...
                $usermappings[$comment->userid]->avatar = $practicalabment->get_renderer()->user_picture($commenter,
 ...
 */
function practicalabsubmission_comments_comment_add(stdClass $comment, stdClass $param) {

    global $DB;
    if ($comment->commentarea == 'submission_comments_upgrade') {
        $submissionid = $comment->itemid;
        $submission = $DB->get_record('practicalab_submission', array('id' => $submissionid));
