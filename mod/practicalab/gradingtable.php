...
/**
 * Extends table_sql to provide a table of practicalabment submissions
 *
 * @package   mod_practicalab
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class practicalab_grading_table extends table_sql implements renderable {
    /** @var practicalab $practicalabment */
    private $practicalabment = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the practicalabment class that is displaying this table
     *
     * @param practicalab $practicalabment The practicalabment class
     ...
     */
    public function __construct(practicalab $practicalabment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        global $CFG, $PAGE, $DB;
        parent::__construct('mod_practicalab_grading');
        ...
        $urlparams = array('action'=>'grading', 'id'=>$practicalabment->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/practicalab/view.php', $urlparams);
        ...
        $users = array_keys( $practicalabment->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['practicalabmentid1'] = (int)$this->practicalabment->get_instance()->id;
        $params['practicalabmentid2'] = (int)$this->practicalabment->get_instance()->id;
        $params['practicalabmentid3'] = (int)$this->practicalabment->get_instance()->id;
        $params['practicalabmentid4'] = (int)$this->practicalabment->get_instance()->id;
        $params['practicalabmentid5'] = (int)$this->practicalabment->get_instance()->id;

        $extrauserfields = get_extra_user_fields($this->practicalabment->get_context());
        ...
        $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                 ...
                                 WHERE mxs.practicalabment = :practicalabmentid4 GROUP BY mxs.userid';
        $grademaxattempt = 'SELECT mxg.userid, MAX(mxg.attemptnumber) AS maxattempt
                            FROM {practicalab_grades} mxg
                            WHERE mxg.practicalabment = :practicalabmentid5 GROUP BY mxg.userid';
        $from = '{user} u
                         ...
                         LEFT JOIN {practicalab_submission} s ON
                            u.id = s.userid AND
                            s.practicalabment = :practicalabmentid1 AND
                            s.attemptnumber = smx.maxattempt
                         LEFT JOIN {practicalab_grades} g ON
                            u.id = g.userid AND
                            g.practicalabment = :practicalabmentid2 AND
                            g.attemptnumber = gmx.maxattempt
                         LEFT JOIN {practicalab_user_flags} uf ON u.id = uf.userid AND uf.practicalabment = :practicalabmentid3';
        ...
        if ($this->practicalabment->is_any_submission_plugin_enabled()) {
            if ($filter == PRACTICALAB_FILTER_SUBMITTED) {
                $where .= ' AND s.timecreated > 0 ';
            }
            if ($filter == PRACTICALAB_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified > g.timemodified OR g.timemodified IS NULL))';
                $params['submitted'] = PRACTICALAB_SUBMISSION_STATUS_SUBMITTED;
            }
            if (strpos($filter, PRACTICALAB_FILTER_SINGLE_USER) === 0) {
        ...
        // User picture.
        if (!$this->practicalabment->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'practicalab');
            }
        ...
        // Submission status.
        if ($practicalabment->is_any_submission_plugin_enabled()) {
            $columns[] = 'status';
            $headers[] = get_string('status');
        }

        // Team submission columns.
        if ($practicalabment->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'practicalab');

            $columns[] = 'teamstatus';
            $headers[] = get_string('teamsubmissionstatus', 'practicalab');
        }
        ...
        if ($this->is_downloading()) {
            if ($this->practicalabment->get_instance()->grade >= 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'practicalab');
            } else {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'practicalab');
            }
        }
        if (!$this->is_downloading()) {
        ...
        if ($practicalabment->is_any_submission_plugin_enabled()) {
            ...
            $headers[] = get_string('lastmodifiedsubmission', 'practicalab');

            foreach ($this->practicalabment->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    if ($plugin->is_visible() && $plugin->is_enabled()) {
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'practicalab');

        // Feedback plugins.
        foreach ($this->practicalabment->get_feedback_plugins() as $plugin) {
            if ($this->is_downloading()) {
                if ($plugin->is_visible() && $plugin->is_enabled()) {
                    foreach ($plugin->get_editor_fields() as $field => $description) {
                        $index = 'plugin' . count($this->plugincache);
        ...
        $this->gradinginfo = grade_get_grades($this->practicalabment->get_course()->id,
                                              'mod',
                                              'practicalab',
                                              $this->practicalabment->get_instance()->id,
                                              $users);
        $this->hasgrantextension = has_capability('mod/practicalab:grantextension',
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this practicalabment.
     *
     * @return string
     */
    public function col_recordid(stdClass $row) {
        return get_string('hiddenuser', 'practicalab') .
               $this->practicalabment->get_uniqueid_for_user($row->userid);
    }
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id'=>-($this->practicalabment->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->practicalabment->get_instance()->grade >= 0) {
                ...
                // This is a custom scale.
                $scale = $this->practicalabment->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->practicalabment->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        return get_string('defaultteam', 'practicalab');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->practicalabment->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->practicalabment->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_teamstatus(stdClass $row) {
        ...
        return get_string('submissionstatus_' . $status, 'practicalab');
    }
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            $courseid = $this->practicalabment->get_course()->id;
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'practicalab', fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this practicalabment.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        return format_float($this->practicalabment->get_instance()->grade, 2);
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        $separator = '';
        $grade = '';
        $gradingdisabled = $this->practicalabment->grading_disabled($row->id);

        if (!$this->is_downloading()) {
            $name = fullname($row);
            if ($this->practicalabment->is_blind_marking()) {
                $name = get_string('hiddenuser', 'practicalab') .
                        $this->practicalabment->get_uniqueid_for_user($row->userid);
            }
            $icon = $this->output->pix_icon('gradefeedback',
                                            get_string('gradeuser', 'practicalab', $name),
                                            'mod_practicalab');
    ...
    public function col_status(stdClass $row) {
        ...
        $instance = $this->practicalabment->get_instance();

        if ($this->practicalabment->is_any_submission_plugin_enabled()) {

            $o .= $this->output->container(get_string('submissionstatus_' . $row->status, 'practicalab'),
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'practicalab');
                $o .= $this->output->container($lockedstr, 'lockedsubmission');
            }
            if ($row->grade !== null && $row->grade >= 0) {
                $o .= $this->output->container(get_string('graded', 'practicalab'), 'submissiongraded');
            }
            if (!$row->timesubmitted) {
                $now = time();
                ...
                if ($due && ($now > $due)) {
                    $overduestr = get_string('overdue', 'practicalab', format_time($now - $due));
                    $o .= $this->output->container($overduestr, 'overduesubmission');
                }
            }
            if ($row->extensionduedate) {
                $userdate = userdate($row->extensionduedate);
                $extensionstr = get_string('userextensiondate', 'practicalab', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'practicalab');
        }
        $actions[$url->out(false)] = $description;

        // Hide for offline practicalabments.
        if ($this->practicalabment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == PRACTICALAB_SUBMISSION_STATUS_DRAFT ||
                    !$this->practicalabment->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'practicalab');
                    $actions[$url->out(false)] = $description;
                } else {
                    $urlparams = array('id' => $this->practicalabment->get_course_module()->id,
                                       'userid'=>$row->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/practicalab/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'practicalab');
                    $actions[$url->out(false)] = $description;
                }
            }

            if (($this->practicalabment->get_instance()->duedate ||
                   $this->practicalabment->get_instance()->cutoffdate) &&
                ...
                $urlparams = array('id' => $this->practicalabment->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/practicalab/view.php', $urlparams);
                $description = get_string('grantextension', 'practicalab');
                $actions[$url->out(false)] = $description;
            }
        }
        if ($row->status == PRACTICALAB_SUBMISSION_STATUS_SUBMITTED &&
                $this->practicalabment->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->practicalabment->get_course_module()->id,
                               'userid'=>$row->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/practicalab/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'practicalab');
            $actions[$url->out(false)] = $description;
        }

        $ismanual = $this->practicalabment->get_instance()->attemptreopenmethod == PRACTICALAB_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != PRACTICALAB_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->practicalabment->get_instance()->maxattempts == PRACTICALAB_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->practicalabment->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->practicalabment->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/practicalab/view.php', $urlparams);
            $description = get_string('addattempt', 'practicalab');
            $actions[$url->out(false)] = $description;
        }
     ...
     *
     * @param practicalab_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(practicalab_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('practicalab')), 'practicalab');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'practicalabsubmission') {
                if ($this->practicalabment->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == PRACTICALAB_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->practicalabment->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->practicalabment = $this->practicalabment->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->practicalabment = $this->practicalabment->get_instance()->id;
     ...
     *
     * @return string the practicalabment name
     */
    public function get_practicalabment_name() {
        return $this->practicalabment->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->practicalabment->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->practicalabment->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->practicalabment->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this practicalabment accept submissions
     */
    public function submissions_enabled() {
        return $this->practicalabment->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->practicalabment->get_course_context();
