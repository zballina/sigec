 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_practicalab_grade_form extends moodleform {
    /** @var practicalabment $practicalabment */
    ...
    public function definition() {
        ...
        list($practicalabment, $data, $params) = $this->_customdata;
        // Visible elements.
        $this->practicalabment = $practicalabment;
        $practicalabment->add_grade_form_elements($mform, $data, $params);
    ...
    public function validation($data, $files) {
        ...
        if ($instance->grade > 0) {
            if (unformat_float($data['grade']) === null && (!empty($data['grade']))) {
                $errors['grade'] = get_string('invalidfloatforgrade', 'practicalab', $data['grade']);
            } else if (unformat_float($data['grade']) > $instance->grade) {
                $errors['grade'] = get_string('gradeabovemaximum', 'practicalab', $instance->grade);
            } else if (unformat_float($data['grade']) < 0) {
                $errors['grade'] = get_string('gradebelowzero', 'practicalab');
            }
        } else {
            // This is a scale.
            if ($scale = $DB->get_record('scale', array('id'=>-($instance->grade)))) {
                ...
                if (!array_key_exists((int)$data['grade'], $scaleoptions)) {
                    $errors['grade'] = get_string('invalidgradeforscale', 'practicalab');
                }
            }
        }
        return $errors;
