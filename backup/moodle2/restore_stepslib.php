...
class restore_section_structure_step extends restore_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('unidades', '/section/unidades');
    ...
    public function process_unidades($data) 
    {
        global $DB;
        $data = (object)$data;
        $oldid = $data->id;
        unset($data->id);
        $data->id = $this->task->get_sectionid();
        $data->nu_curso = $this->get_courseid();
        
        $unidad = new stdClass();
        $unidad->id = $data->id;
        $unidad->nu_curso = $data->nu_curso;
        $unidad->nu_unidad = $data->nu_unidad;
        
        if (!$secrec = $DB->get_record('course_unidades', (array)$unidad)) 
        {
            $newid = $data->id;
            $DB->insert_record_raw('course_unidades', $data, false, false, true);
            $this->set_mapping('course_unidades', $oldid, $newid);
        }                
    }
